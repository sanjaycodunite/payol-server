...
class Payout extends CI_Controller {    
	...
	{	
		...
		// datatable column index  => database column name
			...
			$sql = "SELECT a.* FROM tbl_user_fund_transfer as a INNER JOIN tbl_users as b ON b.id = a.user_id where a.account_id = '$account_id' AND a.user_id = '$loggedAccountID'";
    ...
    public function payoutTransferAuth()
	{
		...
		// save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Payout Open Api Called.]'.PHP_EOL;
        $this->User->generateLog($log_msg);

        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Payout Open Api Post Data - '.json_encode($post).'.]'.PHP_EOL;
        $this->User->generateLog($log_msg);
		...
		{	
			...
			$getMinWalletBal = $this->db->select('min_wallet_balance')->get_where('users',array('id'=>$loggedAccountID))->row_array();
			$min_wallet_balance = isset($getMinWalletBal['min_wallet_balance']) ? $getMinWalletBal['min_wallet_balance'] : 0 ;
			// get dmr surcharge
            $surcharge_amount = $this->User->get_xpress_payout_surcharge($amount,$loggedAccountID);
            // save system log
            ...
            $final_deduct_wallet_balance = $before_balance - $min_wallet_balance;  
            ...
            if($final_deduct_wallet_balance < $final_amount){
                ...
                {
                	$this->Az->redirect('portal/payout/payoutFundTransfer', 'system_message_error',lang('MIN_WALLET_BALANCE_ERROR'));
                }
                else
                {
                	$this->Az->redirect('portal/payout/payoutFundTransfer', 'system_message_error',lang('WALLET_BALANCE_ERROR'));
            	}
            }
			...
			$responseData = $this->Wallet_model->cibPayoutOpen($account_holder_name,$account_no,$ifsc,$amount,$transaction_id,$bankID,$txnType,$loggedAccountID,$account_id);
			...
			{
				$this->Az->redirect('portal/payout/payoutFundTransfer', 'system_message_error',lang('MANUAL_TRANSFER_SUCCESS'));
			...
			elseif(isset($responseData['status']) && $responseData['status'] == 3)
			{
				...
				$this->Az->redirect('portal/payout/payoutFundTransfer', 'system_message_error',sprintf(lang('MANUAL_TRANSFER_FAILED'),$apiMsg));
			}
