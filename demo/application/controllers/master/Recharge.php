...
class Recharge extends CI_Controller{
    ...
    {   
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Recharge Post Data - '.json_encode($post).']'.PHP_EOL;
        ...
        {   
            ...
            // save system log
            $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Wallet Balance - '.$chk_wallet_balance['wallet_balance'].' - Min Wallet Balance - '.$chk_wallet_balance['min_wallet_balance'].']'.PHP_EOL;
            $this->User->generateLog($log_msg);
            ...
            if($user_before_balance < $post['amount']){
                ...
                $this->Az->redirect('master/recharge/mobilePrepaid', 'system_message_error',lang('WALLET_ERROR'));
            ...
            if($user_before_balance < $final_deduct_wallet_balance){
                ...
                $this->Az->redirect('master/recharge/mobilePrepaid', 'system_message_error',lang('MIN_WALLET_ERROR'));
                
            }  
            
            
            $response = $this->User->generate_api_url($loggedUser['id'],$post['operator'],$post['amount']);

            // save system log
            $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Rechage API Generate Response - '.json_encode($response).']'.PHP_EOL;
            $this->User->generateLog($log_msg);
            if($response['status'] && $response['api_id'])
            {
                ...
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Rechage API Operator Code - '.$opt_code.']'.PHP_EOL;
                $this->User->generateLog($log_msg);
                ...
                $circle_code = 19;
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Rechage API Circle Code - '.$circle_code.']'.PHP_EOL;
                $this->User->generateLog($log_msg);

                // generate recharge unique id
                $recharge_unique_id = rand(1111,9999).time();
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Rechage Unique ID - '.$recharge_unique_id.']'.PHP_EOL;
                $this->User->generateLog($log_msg);
                ...
                if($response['is_instantpay_api'])
                {
                    ...
                    {
                        $this->Az->redirect('master/recharge/mobilePrepaid', 'system_message_error',sprintf(lang('RECHARGE_FAILED'),'Sorry ! Your eKyc not approved yet, please submit your eKyc.'));
                    }
                }
                ...
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Save Rechage Data System Recharge ID - '.$recharge_id.']'.PHP_EOL;
                $this->User->generateLog($log_msg);
                ...
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Recharge Wallet Amount Deducted]'.PHP_EOL;
                $this->User->generateLog($log_msg);
                ...
                if($response['is_instantpay_api'])
                {
                    ...
                    {
                        // call recharge API
                        $api_response = $this->User->instantpay_rechage_api($opt_code,$loggedUser['id'],$recharge_unique_id,$post['mobile'],$post['amount'],$response['api_id'],$loggedUser['user_code'],'MD');
                    }
                    else
                    {
                        $api_response = array(
                ...
                else
                {
                    ...
                    // save system log
                    $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Final API URL - '.$api_url.' - Post Data - '.json_encode($api_post_data).']'.PHP_EOL;
                    $this->User->generateLog($log_msg);

                    // call recharge API
                    $api_response = $this->User->prepaid_rechage_api($api_url,$api_post_data,$loggedUser['id'],$recharge_unique_id,$response['api_id'],$response['response_type'],$response['responsePara'],$response['seperator'],$response['header_data'],$loggedUser['user_code'],'MD');
                }

               
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - API Final Response - '.json_encode($api_response).']'.PHP_EOL;
                $this->User->generateLog($log_msg);
                ...
                if(isset($api_response['status']) && ($api_response['status'] == 1 || $api_response['status'] == 2))
                {
                    ...
                    if($api_response['status'] == 1){
                        ...
                        $this->Az->redirect('master/recharge/mobilePrepaid', 'system_message_error',lang('RECHARGE_PENDING'));
                    ...
                    elseif($api_response['status'] == 2)
                    {
                        ...
                        $this->Az->redirect('master/recharge/mobilePrepaid', 'system_message_error',lang('RECHARGE_SUCCESS'));
                ...
                {
                    // save system log
                    $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Recharge Failed]'.PHP_EOL;
                    $this->User->generateLog($log_msg);
                    // update recharge status
                    $this->db->where('id',$recharge_id);
                    $this->db->where('recharge_display_id',$recharge_unique_id);
                    ...
                    $this->Az->redirect('master/recharge/mobilePrepaid', 'system_message_error',sprintf(lang('RECHARGE_FAILED'),$api_response['opt_msg']));
            ...
            else
            {
                ...
                $this->Az->redirect('master/recharge/mobilePrepaid', 'system_message_error',lang('API_ERROR'));
    ...
    {   
        ...
        $post = $this->input->post();
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - DTH Recharge Post Data - '.json_encode($post).']'.PHP_EOL;
        $this->User->generateLog($log_msg);
        $this->load->library('form_validation');
        $this->form_validation->set_rules('operator', 'Operator', 'required');
        ...
        {   
            ...
            if($user_before_balance < $post['amount']){
                ...
                $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Insufficient Wallet Error]'.PHP_EOL;
                $this->User->generateLog($log_msg);
                $this->Az->redirect('master/recharge/dth', 'system_message_error',lang('WALLET_ERROR'));   
            ...
            if($is_cogent_instantpay_api)
            {
                ...
                if($admin_wallet_balance < $post['amount']){
                    // save system log
                    $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Insufficient Virtual Wallet Error]'.PHP_EOL;
                    $this->User->generateLog($log_msg);
                    
                    $this->Az->redirect('master/recharge/dth', 'system_message_error',lang('ADMIN_WALLET_BALANCE_ERROR'));
            ...
            if($user_before_balance < $final_deduct_wallet_balance){
                ...
                $this->Az->redirect('master/recharge/dth', 'system_message_error',lang('MIN_WALLET_ERROR'));
            ...
            if($response['status'] && $response['api_id'])
            {
                ...
                if($response['is_instantpay_api'])
                {
                    ...
                    {
                        // call recharge API
                        $api_response = $this->User->instantpay_rechage_api($opt_code,$loggedUser['id'],$recharge_unique_id,$post['cardNumber'],$post['amount'],$response['api_id'],$loggedUser['user_code'],'MD');
                    }
                    else
                    {
                        $api_response = array(
                ...
                if(isset($api_response['status']) && ($api_response['status'] == 1 || $api_response['status'] == 2))
                {
                    ...
                    if($api_response['status'] == 1){
                        ...
                        $this->Az->redirect('master/recharge/dth', 'system_message_error',lang('RECHARGE_PENDING'));   
                    ...
                    elseif($api_response['status'] == 2)
                    {
                        ...
                        $this->Az->redirect('master/recharge/dth', 'system_message_error',lang('RECHARGE_SUCCESS'));   
                ...
                else
                {
                    ...
                    $this->Az->redirect('master/recharge/dth', 'system_message_error',sprintf(lang('RECHARGE_FAILED'),$api_response['opt_msg']));
                         
                }
            }
            else
            {
                $this->Az->redirect('master/recharge/dth', 'system_message_error',lang('API_ERROR'));
            }
    ...
    {   
        ...
        $post = $this->input->post();
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Electicity Post Data - '.json_encode($post).']'.PHP_EOL;
        $this->User->generateLog($log_msg);
        $this->load->library('form_validation');
        $this->form_validation->set_rules('operator', 'Operator', 'required');
        ...
        else
        {
            ...
            {
                $this->Az->redirect('master/recharge/electricity', 'system_message_error',lang('OPERATOR_VALID_ERROR'));   
            }
            ...
            if($admin_wallet_balance < $post['amount']){
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Insufficient Wallet Error in Admin Account.]'.PHP_EOL;
                $this->User->generateLog($log_msg);
                $this->Az->redirect('master/recharge/electricity', 'system_message_error',lang('ADMIN_WALLET_BALANCE_ERROR'));
            } */
            ...
            if($user_before_balance < $post['amount']){
                ...
                $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Insufficient wallet error.]'.PHP_EOL;
                $this->User->generateLog($log_msg);
                $this->Az->redirect('master/recharge/electricity', 'system_message_error',lang('WALLET_ERROR'));   
            ...
            if($user_before_balance < $final_deduct_wallet_balance){
                ...
                $this->Az->redirect('master/recharge/electricity', 'system_message_error',lang('MIN_WALLET_ERROR'));
            ...
            // save system log
            $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Record Saved Successfully.]'.PHP_EOL;
            $this->User->generateLog($log_msg);
            ...
            if(isset($api_response['status']) && ($api_response['status'] == 1 || $api_response['status'] == 2))
            {
                ...
                if($api_response['status'] == 1){
                    ...
                    $this->db->where('recharge_display_id',$recharge_unique_id);
                    $this->db->update('recharge_history',array('txid'=>$api_response['txid'],'operator_ref'=>$api_response['operator_ref'],'api_response_id'=>$api_response['api_response_id']));
                    $this->Az->redirect('master/recharge/electricity', 'system_message_error',lang('RECHARGE_PENDING'));
                }
                else
                {
                    // update recharge status
                    ...
                    $this->Az->redirect('master/recharge/electricity', 'system_message_error',lang('RECHARGE_SUCCESS'));
                }
            }
            else
            {
                ...
                $this->db->where('recharge_display_id',$recharge_unique_id);
                $this->db->update('recharge_history',array('status'=>3));
                $this->Az->redirect('master/recharge/electricity', 'system_message_error',lang('RECHARGE_FAILED'));
            }
   ...
   public function getOperatorPlanList()
    ...
    public function complainAuth()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Az->redirect('master/report/recharge', 'system_message_error',lang('FORM_ERROR'));
        }
        else
        {   
            ...
            if(!$chk_recharge)
            {
                ...
                $this->Az->redirect('master/report/recharge', 'system_message_error',lang('AUTHORIZE_ERROR'));  
            ...
            if($status == true)
            {
                $this->Az->redirect('master/report/recharge', 'system_message_error',lang('COMPLAIN_SAVED'));
            }
            else
            {
                $this->Az->redirect('master/report/recharge', 'system_message_error',lang('COMMON_ERROR'));
            }
    ...
    public function ekycAuth()
    {
        ...
        {   
            ...
            if($response['status'] == 1)
            {
                ...
                $this->Az->redirect('master/recharge/ekycOtp/'.$response['otpReferenceID'], 'system_message_error',$msg);
            }
            else
            {
                ...
                $this->Az->redirect('master/recharge/ekyc', 'system_message_error',$msg);
            }
    ...
    public function ekycOtpAuth()
    {
        ...
        {   
            ...
            {
                $msg = '<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$response['msg'].'</div>';
                $this->Az->redirect('master/recharge/ekycOtp/'.$otpReferenceID, 'system_message_error',$msg);
            }
    ...
    public function mobileTransaction(){
        ...
        $data = array(
            ...
            'system_info' => $this->Az->getsystemMessageInfo(),
            'system_warning' => $this->Az->getSystemMessageWarning(),
            'content_block' => 'recharge/mobile-transaction'
        );

        $this->parser->parse('master/layout/column-1' , $data);
    }
    
    public function mobileRec(){
        ...
        $data = array(
            ...
            'system_info' => $this->Az->getsystemMessageInfo(),
            'system_warning' => $this->Az->getSystemMessageWarning(),
            'content_block' => 'recharge/mobile-rec'
        );
