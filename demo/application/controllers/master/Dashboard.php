...
class Dashboard extends CI_Controller{
    ...
    public function index($uname_prefix = '' , $username = ''){
        ...
        // get total member
        $totalMember = $this->db->where_in('role_id',array(4,5))->get_where('users',array('account_id'=>$domain_account_id,'created_by'=>$account_id))->num_rows();


        // get total distributor
        $totalDistributorMember = $this->db->where_in('role_id',array(4))->get_where('users',array('account_id'=>$domain_account_id,'created_by'=>$account_id))->num_rows();

        // get total retailer
        $totalRetailerMember = $this->db->where_in('role_id',array(5))->get_where('users',array('account_id'=>$domain_account_id,'created_by'=>$account_id))->num_rows();
    ...
    public function getDashboardSummary()
    {
        ...
        $get_success_recharge = $this->db->select('SUM(amount) as totalAmount,count(*) as totalRecord')->get_where('recharge_history',array('account_id'=>$account_id,'status'=>2,'member_id'=>$loggedAccountID,'DATE(created)'=>$today_date))->row_array();
        ...
        // get total success recharge
        $get_pending_recharge = $this->db->select('SUM(amount) as totalAmount,count(*) as totalRecord')->get_where('recharge_history',array('account_id'=>$account_id,'status'=>1,'member_id'=>$loggedAccountID,'DATE(created)'=>$today_date))->row_array();
        ...
        // get total success recharge
        $get_failed_recharge = $this->db->select('SUM(amount) as totalAmount,count(*) as totalRecord')->get_where('recharge_history',array('account_id'=>$account_id,'status'=>3,'member_id'=>$loggedAccountID,'DATE(created)'=>$today_date))->row_array();
