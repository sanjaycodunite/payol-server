...
class Society extends CI_Controller {   
    ...
    public function __construct() 
    {
		...
		$this->load->model('master/Society_model');	
		$this->lang->load('master/society', 'english');
	...
	{	
		...
		// datatable column index  => database column name
			...
			$sql = "SELECT a.*,b.title as tenureType FROM tbl_club_list as a INNER JOIN tbl_club_duration_type as b ON b.id = a.tenure_type where a.account_id = '$account_id' AND a.created_by = '$loggedAccountID'";
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData=array(); 
				$nestedData[] = $i;
				$nestedData[] = "<a href='".base_url('master/society/clubDetail/'.$list['id'])."' style='text-decoration:none;'>".$list['club_name']."</a>";
				...
				$nestedData[] = '<a title="edit" class="btn btn-primary btn-sm" href="'.base_url('master/society/editClub').'/'.$list['id'].'"><i class="fa fa-edit" aria-hidden="true"></i></a> <a title="delete" class="btn btn-danger btn-sm" href="'.base_url().'master/society/deleteClub/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to delete?\')"><i class="fa fa-trash" aria-hidden="true"></i></a>';
	...
	public function saveClub()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->Az->redirect('master/society', 'system_message_error',lang('CLUB_SAVE_SUCCESS'));
		}
	}
	...
	public function editClub($club_id = 0)
	{
		...
		$chkClub = $this->db->get_where('club_list',array('id'=>$club_id,'created_by'=>$loggedUser['id']))->num_rows();
		if(!$chkRound)
		{
			$this->Az->redirect('master/society', 'system_message_error',lang('MEMBER_ERROR'));
		}
	...
	public function updateClub()
	{
		...
		$id = $post['id'];
		// check round is valid or not
		$chkClub = $this->db->get_where('club_list',array('id'=>$id,'created_by'=>$loggedUser['id']))->num_rows();
		if(!$chkRound)
		{
			$this->Az->redirect('master/society', 'system_message_error',lang('MEMBER_ERROR'));
		}
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->Az->redirect('master/society', 'system_message_error',lang('CLUB_UPDATED'));
		}
	}
	...
	public function deleteClub($club_id = 0)
	{
		...
		$this->Az->redirect('master/society', 'system_message_error',lang('CLUB_DELETED'));
	...
	public function updateClubRoundAuth()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->Az->redirect('master/society/clubDetail/'.$club_id, 'system_message_error',lang('ROUND_UPDATED'));
		}
	}
	...
	public function requestAuth($requestID = 0, $club_id = 0, $status = 0)
	{
		...
		if($status == 1)
		{
			...
			$this->Az->redirect('master/society/clubDetail/'.$club_id, 'system_message_error',lang('REQUEST_APPROVED'));
		}
		elseif($status == 2)
		{
			$this->db->where('id',$requestID);
        	...
        	$this->db->insert('club_notification',$notificationData);
			$this->Az->redirect('master/society/clubDetail/'.$club_id, 'system_message_error',lang('REQUEST_DECLINED'));
		}
		elseif($status == 3)
		{
			$this->db->where('id',$requestID);
			$this->db->delete('club_member_request');
			$this->Az->redirect('master/society/clubDetail/'.$club_id, 'system_message_error',lang('REQUEST_DELETED'));
		}
		$this->Az->redirect('master/society/clubDetail/'.$club_id, 'system_message_error',lang('MEMBER_ERROR'));
    ...
    public function payWinAmountAuth($recordID = 0, $club_id = 0)
    {
		...
		if(!$chkRound)
		{
			$this->Az->redirect('master/society/clubDetail/'.$club_id, 'system_message_error',lang('ALREADY_PAID'));
		}
		...
		$this->Az->redirect('master/society/clubDetail/'.$club_id, 'system_message_error',lang('WIN_AMOUNT_PAID'));
