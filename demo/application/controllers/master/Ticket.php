...
class Ticket extends CI_Controller {    
    ...
    public function __construct() 
    {
        ...
        $this->load->model('master/Ticket_model');		
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData=array(); 
				$nestedData[] = $i;
				$nestedData[] = '<a href="'.base_url('master/ticket/viewTicket/'.$list['id']).'">'.$list['ticket_id'].'</a>';
	...
	public function saveTicketAuth()
	{
		...
		{	
			...
			if($_FILES['attachment']['name'])
			{
				...
				if($uploadError){
					$this->Az->redirect('master/ticket/create', 'system_message_error',$uploadError);
				}
				else
				{
			...
			$this->Az->redirect('master/ticket/ticketList', 'system_message_error',lang('TICKET_SAVED'));
	...
	public function viewTicket($ticket_id = 0){
		...
		if(!$chkMember)
		{
			$this->Az->redirect('master/ticket/ticketList', 'system_message_error',lang('MEMBER_ERROR'));
		}
	...
	public function ticketResponseAuth()
	{
		...
		{	
			...
			if($_FILES['attachment']['name'])
			{
				...
				$uploadError = $this->upload->display_errors();
				if($uploadError){
					$this->Az->redirect('master/ticket/viewTicket/'.$ticket_id, 'system_message_error',$uploadError);
				}
				else
				{
					$fileData = $this->upload->data();
			...
			$this->Az->redirect('master/ticket/viewTicket/'.$ticket_id, 'system_message_error',lang('TICKET_REPLY_SAVED'));
