    ...
    public function payoutTransferAuth()
	{
		...
		{	
            ...
            if($before_balance < $final_amount){
                ...
                $this->User->generateLog($log_msg);
                $this->Az->redirect('master/payout/payoutFundTransfer', 'system_message_error',lang('WALLET_BALANCE_ERROR'));
            }
			...
			{
				$this->Az->redirect('master/payout/payoutFundTransfer', 'system_message_error',lang('MANUAL_TRANSFER_SUCCESS'));
			...
			elseif(isset($responseData['status']) && $responseData['status'] == 3)
			{
				...
				$this->Az->redirect('master/payout/payoutFundTransfer', 'system_message_error',sprintf(lang('MANUAL_TRANSFER_FAILED'),$apiMsg));
			}
