    ...
    public function loginAuth(){
		...
		else
		{
			...
			if(!$chk_user_auth && !$chk_user_mobile)
			{
				...
				if($chk_user_auth)
				{
					$status = $this->db->select('id,account_id,role_id,user_code,name,is_active,email,mobile,wallet_balance,photo,fcm_id,device_id')->where_in('role_id',array(3,4,5,8))->get_where('users',array('username'=>$username,'password'=>$password,'account_id'=>$account_id))->row_array();
				}
				else
				{
					$status = $this->db->select('id,account_id,role_id,user_code,name,is_active,email,mobile,wallet_balance,photo,fcm_id,device_id')->where_in('role_id',array(3,4,5,8))->get_where('users',array('mobile'=>$username,'password'=>$password,'account_id'=>$account_id))->row_array();
				}
		...
		else
		{
			...
			$chk_otp = $this->db->get_where('users_otp',array('account_id'=>$account_id,'encrypt_otp_code'=>$encode_otp_code,'status'=>0,'otp_code'=>$otp_code))->row_array();
	        ...
	        if(!$chk_otp){
		        ...
		        $this->db->update('users_otp',array('status'=>1,'updated'=>date('Y-m-d H:i:s')));
