...
class Payout extends CI_Controller {    
    ...
    public function payoutTransferAuth()
	{
		...
		// save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Payout Open Api Called.]'.PHP_EOL;
        $this->User->generateLog($log_msg);

        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Payout Open Api Post Data - '.json_encode($post).'.]'.PHP_EOL;
        $this->User->generateLog($log_msg);
		...
		{	
            ...
            // save system log
	        $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Payout Open E-Wallet Balance - '.$chk_wallet_balance['wallet_balance'].'.]'.PHP_EOL;
	        $this->User->generateLog($log_msg);

            // get dmr surcharge
            $surcharge_amount = $this->User->get_dmr_surcharge($amount,$loggedAccountID);
            // save system log
            $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Payout Open Api - Payout Surcharge Amount - '.$surcharge_amount.']'.PHP_EOL;
            $this->User->generateLog($log_msg);
            ...
            if($before_balance < $final_amount){
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Payout Open Api - Insufficient Wallet Error]'.PHP_EOL;
                $this->User->generateLog($log_msg);
                $this->Az->redirect('retailer/payout/payoutFundTransfer', 'system_message_error',lang('WALLET_BALANCE_ERROR'));
            }
            ...
            // save system log
            $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Payout Open API - Member Wallet Deduction - Updated Balance - '.$after_wallet_balance.'.]'.PHP_EOL;
            $this->User->generateLog($log_msg);
			...
			// save system log
            $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Payout Open Api - Final Response - '.json_encode($responseData).']'.PHP_EOL;
            $this->User->generateLog($log_msg);
			...
			{
				$this->Az->redirect('retailer/payout/payoutFundTransfer', 'system_message_error',lang('MANUAL_TRANSFER_SUCCESS'));
			...
			elseif(isset($responseData['status']) && $responseData['status'] == 2)
			{
				...
				// save system log
	            $log_msg = '['.date('d-m-Y H:i:s').' - RT('.$loggedUser['user_code'].') - Payout Open Api - Distribute Commision/Surcharge Start]'.PHP_EOL;
	            $this->User->generateLog($log_msg);

				$this->User->distribute_payout_commision($recordID,$transaction_id,$amount,$loggedAccountID,$surcharge_amount,'RT',$loggedUser['user_code']);

				// save system log
	            $log_msg = '['.date('d-m-Y H:i:s').' - RT('.$loggedUser['user_code'].') - Payout Open Api - Distribute Commision/Surcharge End]'.PHP_EOL;
	            $this->User->generateLog($log_msg);
			...
			elseif(isset($responseData['status']) && $responseData['status'] == 3)
			{
				...
				$this->Az->redirect('retailer/payout/payoutFundTransfer', 'system_message_error',sprintf(lang('MANUAL_TRANSFER_FAILED'),$apiMsg));
			}
