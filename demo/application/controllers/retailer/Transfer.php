...
class Transfer extends CI_Controller {    
	...
	public function beneficiaryList()
    {
		...
		$benificaryList = $this->db->select('user_benificary.*,tbl_instantpay_aeps_bank_list.bank_name')->join('tbl_instantpay_aeps_bank_list','tbl_instantpay_aeps_bank_list.id = user_benificary.bankID')->get_where('user_benificary',array('user_benificary.account_id'=>$account_id,'user_benificary.user_id'=>$loggedAccountID))->result_array();
    ...
    public function benificaryAuth(){
        ...
        {   
        	...
        	$this->Az->redirect('retailer/transfer/beneficiaryList', 'system_message_error',lang('BENEFICIARY_SAVE_SUCCESS'));
        }
	...
	public function fundTransferAuth()
	{
		...
		{	
			...
			if(isset($responseData['statuscode']) && $responseData['statuscode'] == 'TXN' && $responseData['status'] == 'Transaction Successful')
			{
				...
				$this->User->distribute_money_transfer_commision($recordID,$transaction_id,$amount,$loggedAccountID,$surcharge_amount,'RT',$loggedUser['user_code']);
   ...
   /* public function payoutTransferAuth()
	...
	{	
		...
		$this->Az->redirect('retailer/transfer/beneficiaryList', 'system_message_error',lang('DELETE_SUCCESS'));
	}
	...
	public function updateBenificaryAuth(){
        ...
        if ($this->form_validation->run() == FALSE) {
    		...
    		$this->Az->redirect('retailer/transfer/beneficiaryList', 'system_message_error',$msg);
        }
        else
        {   
        	// check beneficiary valid or not
        	...
        	if($benificaryList)
        {
			...
			$str.='<tbody>';
                ...
                foreach($beneficiary_data as $key=>$list){
                	...
                	$str.='<td><a title="Fund Transfer" class="btn btn-primary btn-sm" href="'.base_url().'retailer/transfer/newPayoutFundTransfer/'.$list['id'].'"> Transfer</a>&nbsp;<a title="Fund Transfer" class="btn btn-danger btn-sm" href="'.base_url().'retailer/transfer/deleteBeneficiaryAccount/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to delete?\')" > Delete</a></td>';
