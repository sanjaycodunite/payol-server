...
class Iciciaeps extends CI_Controller {    
	...
	public function activeAeps(){
		...
		if($chk_kyc)
		{
			...
			$this->Az->redirect('distributor/iciciaeps', 'system_message_error',lang('AEPS_ACTIVE_SUCCESS'));
		}
	...
	public function activeAuth()
	{
		...
		{	
			...
			if($status == 1)
			{
				...
				$this->Az->redirect('distributor/iciciaeps/otpVerify/'.$otpReferenceID, 'system_message_error',lang('AEPS_OTP_SUCCESS'));
			}
			else
			{
	...
	public function otpVerify($otpReferenceID = ''){
		...
		if(!$chk_encode_id)
		{
			$this->Az->redirect('distributor/iciciaeps/activeAeps', 'system_message_error',lang('AEPS_ENCODED_ID_ERROR'));
		}
	...
	public function otpAuth()
	{
		...
		if($status == 1)
		{
			$this->Az->redirect('distributor/iciciaeps', 'system_message_error',lang('AEPS_OTP_VERIFY_SUCCESS'));
		}
		else
		{
			$this->Az->redirect('distributor/iciciaeps/activeAeps', 'system_message_error',sprintf(lang('AEPS_ACTIVE_FAILED'),$response['msg']));
	...
	public function resendOtp($encodeFPTxnId = ''){
		...
		if($status == 1)
		{
			$this->Az->redirect('distributor/iciciaeps/otpVerify/'.$encodeFPTxnId, 'system_message_error',lang('AEPS_OTP_RESEND_SUCCESS'));
		}
		else
		{
			$this->Az->redirect('distributor/iciciaeps/activeAeps', 'system_message_error',sprintf(lang('AEPS_ACTIVE_FAILED'),$response['msg']));
