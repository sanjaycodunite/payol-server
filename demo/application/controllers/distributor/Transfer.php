...
class Transfer extends CI_Controller {    
    ...
    public function benificaryAuth(){
        ...
        {   
        	...
        	$this->Az->redirect('distributor/transfer/beneficiaryList', 'system_message_error',lang('BENEFICIARY_SAVE_SUCCESS'));
        }
	...
	public function fundTransferAuth()
	{
		...
		{	
			/*if($post['amount'] > 25000){
				...
				$this->Az->redirect('distributor/transfer/beneficiaryList', 'system_message_error',lang('MAX_AMOUNT_ERROR'));
			}	*/
			...
			$chk_wallet_balance =$this->db->select('min_wallet_balance')->get_where('users',array('id'=>$loggedAccountID))->row_array();
            ...
            // save system log
	        $log_msg = '['.date('d-m-Y H:i:s').' - D('.$loggedUser['user_code'].') - DMT Api R-Wallet Balance - '.$before_balance.'.]'.PHP_EOL;
	        $this->User->generateLog($log_msg);
			...
			if(isset($responseData['statuscode']) && $responseData['statuscode'] == 'TXN' && $responseData['status'] == 'Transaction Successful')
			{
				...
				$this->User->distribute_money_transfer_commision($recordID,$transaction_id,$amount,$loggedAccountID,$surcharge_amount,'DT',$loggedUser['user_code']);
   ...
   /* public function payoutTransferAuth()
	...
	{	
		...
		$this->Az->redirect('distributor/transfer/beneficiaryList', 'system_message_error',lang('DELETE_SUCCESS'));
	}
	...
	public function updateBenificaryAuth(){
        ...
        if ($this->form_validation->run() == FALSE) {
    		...
    		$this->Az->redirect('distributor/transfer/beneficiaryList', 'system_message_error',$msg);
        }
        else
        {   
        	// check beneficiary valid or not
