...
class Pancard extends CI_Controller {    
    ...
    public function __construct() 
    {
        parent::__construct();
       	...
       	$this->load->model('distributor/Pancard_model');
	...
	public function pancardActiveAuth()
	{
		...
		{	
			...
			if(isset($_FILES['aadhar_card']['name']) && $_FILES['aadhar_card']['name']){
				...
				if($uploadError){
					$this->Az->redirect('distributor/pancard/activeService', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$uploadError.'</div>');
				}
				else
				{
			...
			if($status == 1)
			{
				$this->Az->redirect('distributor/pancard/activeService', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$response['msg'].'</div>');
			}
			else
			{
				$this->Az->redirect('distributor/pancard/activeService', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$response['msg'].'</div>');
			}
	...
	public function purchaseCouponAuth()
	{
		...
		{	
			...
			if($post['coupon'] < 1){
				$this->Az->redirect('distributor/pancard/activeService', 'system_message_error',lang('AUTHORIZE_ERROR'));
			}
        	...
        	if($wallet_balance < $final_deduct_wallet_balance){
				$this->Az->redirect('distributor/pancard/activeService', 'system_message_error',lang('WALLET_ERROR'));   
			}
			else{
				...
				{	
					...
					$this->Az->redirect('distributor/pancard/activeService', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Congratulations!! Coupon purchased successfully.</div>');
				}
				elseif($status == 1 && $message == 'Pending')
				{	
					$this->Az->redirect('distributor/pancard/activeService', 'system_message_error','<div class="alert alert-warning alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Coupon Purchase is under process, status will be updated soon.</div>');
				}
				...
				{	
					...
					$this->Az->redirect('distributor/pancard/activeService', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! Coupon purchase failed.</div>');
				}
				else
				{
					$this->Az->redirect('distributor/pancard/activeService', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! Coupon purchase failed.</div>');
    ...
    public function nsdlActive()
    {
		...
		if($isNsdlActive)
		{
			$this->Az->redirect('distributor/pancard/aaplyNsdl', 'system_message_error',lang('NSDL_ACTIVE_ALREADY_ERROR'));
		}
	...
	public function nsdlActiveAuth()
	{
		...
		{	
			...
			if($isNsdlActive)
			{
				$this->Az->redirect('distributor/pancard/nsdlActive', 'system_message_error',lang('NSDL_ACTIVE_ALREADY_ERROR'));
			}
            ...
            if($before_balance < $$total_wallet_deduct){
                
                $this->Az->redirect('distributor/pancard/nsdlActive', 'system_message_error',lang('WALLET_BALANCE_ERROR'));
		    ...
		    $after_balance = $before_balance - $total_wallet_deduct;  
			...
			if($status == 1)
			{
				...
				$this->Az->redirect('distributor/dashboard', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Congratulation ! Your Pancard Service activate successfully.</div>');
			}
			else
			{
				$this->Az->redirect('distributor/pancard/nsdlActive', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Something Went Wrong.</div>');
			}
	...
	public function nsdlProfile()
    {
		...
		if(!$isNsdlActive)
		{
			$this->Az->redirect('distributor/pancard/nsdlActive', 'system_message_error',lang('NSDL_ACTIVE_ERROR'));
		}
  ...
  "emailid": "sonujangid2011@gmail.com"
        ...
        if(isset($responseData['statuscode']) && $responseData['statuscode'] == "000"){
            ...
            $this->Az->redirect('distributor/pancard/nsdlProfile', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$responseData['message'].'</div>');
        }
    }
	...
	public function nsdlPan()
	{
		...
		// get commission
            ...
            $user_before_balance = $before_balance;
            if($before_balance < $charge){
                
                $this->Az->redirect('distributor/dashboard', 'system_message_error',lang('WALLET_BALANCE_ERROR'));
	...
	public function findPanAuth()
	{
		...
		{	
            ...
            if($before_balance < $total_wallet_deduct){
                ...
                $this->Az->redirect('distributor/pancard/findPan', 'system_message_error',lang('WALLET_BALANCE_ERROR'));                
            }
		    ...
		    $after_balance = $before_balance - $total_wallet_deduct;  
		            ...
		            //save gst data
						...
						if($status == 1)
						{
							...
							$this->Az->redirect('distributor/pancard/findPanList', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Congratulation ! Your  Request  successfully submitted.</div>');
						}
						else
						{
							$this->Az->redirect('distributor/pancard/findPanList', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Something Went Wrong.</div>');
						}
	...
	public function utiBalanceAuth()
	{
		...
		{	
            ...
            if($user_before_balance < $total_wallet_deduct){
                ...
                $this->Az->redirect('distributor/pancard/utiBalanceRequest', 'system_message_error',lang('WALLET_BALANCE_ERROR'));                
            }
		    ...
		    $after_balance = $user_before_balance - $total_wallet_deduct;  
			        ...
			        $status = $this->Pancard_model->utiBalanceRequest($post,$transaction_id);
						...
						if($status == 1)
						{
							...
							$this->Az->redirect('distributor/pancard/utiBalanceRequest', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Congratulation ! Your  Request  successfully submitted.</div>');
						}
						else
						{
							$this->Az->redirect('distributor/pancard/utiBalanceRequest', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Something Went Wrong.</div>');
						}
