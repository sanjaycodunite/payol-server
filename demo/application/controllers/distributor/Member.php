...
class Member extends CI_Controller {    
    ...
    public function __construct() 
    {
        ...
        $this->load->model('distributor/Member_model');		
        $this->lang->load('distributor/member', 'english');
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				/*$nestedData[] ='<a title="edit" class="btn btn-primary btn-sm" href="'.base_url('distributor/member/editMember').'/'.$list['id'].'"><i class="fa fa-edit" aria-hidden="true"></i></a>';*/
	...
	public function addMember()
    {
		...
		// get role list
		$roleList = $this->db->where_in('id',array(5,8))->get('user_roles')->result_array();
	...
	public function saveMember()
	{
		...
		{	
			...
			if($post['role_id'] == 1 || $post['role_id'] == 2 || $post['role_id'] == 3 || $post['role_id'] == 4 || $post['role_id'] == 6  || $post['role_id'] == 7){

				$this->Az->redirect('distributor/member/addMember', 'system_message_error',lang('ROLE_ERROR'));	
			...
			if($chk_user_mobile){
				...
				$this->Az->redirect('distributor/member/addMember', 'system_message_error',lang('MOBILE_ERROR'));	
			...
			if($status == true)
			{
				$this->Az->redirect('distributor/member/addMember', 'system_message_error',lang('MEMBER_SAVED'));
			}
			else
			{
				$this->Az->redirect('distributor/member/addMember', 'system_message_error',lang('MEMBER_ERROR'));
			}
    ...
    {    
    	$this->Az->redirect('distributor/member/memberList', 'system_message_error',lang('MEMBER_ERROR'));
    	$account_id = $this->User->get_domain_account();
    	$loggedUser = $this->User->getAdminLoggedUser(DISTRIBUTOR_SESSION_ID);
    	$loggedAccountID = $loggedUser['id'];
	...
	public function updateMember()
	{
		...
		{	
			if($post['role_id'] == 1 || $post['role_id'] == 2 || $post['role_id'] == 3 || $post['role_id'] == 4 || $post['role_id'] == 6  || $post['role_id'] == 7){
				...
				$this->Az->redirect('distributor/member/editMember/'.$post['id'], 'system_message_error',lang('ROLE_ERROR'));	
			...
			if($chk_user_mobile){
				...
				$this->Az->redirect('distributor/member/editMember/'.$post['id'], 'system_message_error',lang('MOBILE_ERROR'));	
			...
			if($status == true)
			{
				$this->Az->redirect('distributor/member/memberList', 'system_message_error',lang('MEMBER_UPDATED'));
			}
			else
			{
				$this->Az->redirect('distributor/member/memberList', 'system_message_error',lang('MEMBER_ERROR'));
