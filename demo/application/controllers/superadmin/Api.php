...
class Api extends CI_Controller {    
    ...
    public function __construct() 
    {
        ...
        $this->load->model('superadmin/Api_model');		
        $this->lang->load('superadmin/api', 'english');
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] ='<a title="edit" class="btn btn-primary btn-sm" href="'.base_url('superadmin/api/editApi').'/'.$list['id'].'"><i class="fa fa-edit" aria-hidden="true"></i></a> <a title="delete" class="btn btn-danger btn-sm" href="'.base_url().'superadmin/api/deleteApi/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to delete?\')"><i class="fa fa-trash" aria-hidden="true"></i></a> <a href="'.base_url().'superadmin/api/addApiOprator/'.$list['id'].'" class="btn btn-primary" style="padding: 7px 8px;font-size: 12px;">Add Operator</a> <a href="'.base_url().'superadmin/api/addApiCircle/'.$list['id'].'" class="btn btn-primary" style="padding: 7px 8px;font-size: 12px;">Add Circle</a>';
	...
	public function saveApiAuth(){
		...
		{	
		 	...
		 	$this->Az->redirect('superadmin/api/apiList', 'system_message_error',lang('API_SAVE_SUCCESS'));
		 }
	...
	public function editApi($api_id = 0){
		...
		if(!$chkMember)
		{
			$this->Az->redirect('superadmin/api/apiList', 'system_message_error',lang('MEMBER_ERROR'));
		}
	 	// get request type
	 	$requestTypeList = $this->db->get('api_request_type')->result_array();
	...
	public function deleteApi($api_id = 0){
        ...
        $this->db->where('api_id',$api_id);
      	...
      	$this->Az->redirect('superadmin/api/apiList', 'system_message_error',lang('API_DELETE_SUCCESS'));
	}
	...
	public function saveApiOprator(){
		...
		$this->Az->redirect('superadmin/api/apiList', 'system_message_error',lang('API_OPERATOR_SAVE_SUCCESS'));
	...
	public function saveApiCircle(){
		...
		$this->Az->redirect('superadmin/api/apiList', 'system_message_error',lang('API_CIRCLE_SAVE_SUCCESS'));
	...
	public function getMemberActiveAPIData($member_id = 0)
	{
		...
		$account_id = SUPERADMIN_ACCOUNT_ID;
	 	...
	 	$chk_member = $this->db->get_where('package',array('id'=>$member_id,'account_id'=>$account_id))->num_rows();
 		if(!$chk_member)
 		{
 			$is_error = 1;
	...
	public function changeApiAuth()
	{
		$account_id = SUPERADMIN_ACCOUNT_ID;
	 	$loggedUser = $this->User->getLoggedUser(SUPERADMIN_SESSION_ID);
 	 	...
 	 	if(!$post['api_id'])
 	 	{
 	 		$this->Az->redirect('superadmin/api/changeApi', 'system_message_error',lang('API_ERROR'));
 	 	}

 	 	if(!isset($post['optID']) || !$post['optID'])
 	 	{
 	 		$this->Az->redirect('superadmin/api/changeApi', 'system_message_error',lang('OPT_ERROR'));
 	 	}
		...
		if(!$chkMember)
		{
			$this->Az->redirect('superadmin/api/changeApi', 'system_message_error',lang('API_VALID_ERROR'));
		}
		...
		if(!$chk_member)
 		{
 			$this->Az->redirect('superadmin/api/changeApi', 'system_message_error',lang('MEMBER_ERROR'));
 		...
 		$this->Az->redirect('superadmin/api/changeApi', 'system_message_error',lang('API_CHANGE_SUCCESS'));
	}
