...
$ch = curl_init();
    ...
    public function cosmosUpiCallBack()
    {
    	...
    	/*$callbackData = '{"message":""}';*/
...
die();
    ...
    public function yesbankLiveUpi()
    {
		...
		echo $requestStr = 'YES0000011547194|671617||||||||||||NA|NA';
		$encryptData = $this->yesEncryptValue($requestStr,$enckey);
		...
		#echo $requestStr = 'YES0000000065149|Test1270707775|test|10|INR|P2P|Pay|1520||||||aamir@yesb|||||UPI| ABC||||||VPA|||||||||NA|NA';
		echo $requestStr = 'YES0000011547194|PHIC0991212123123|test|10.00|INR|P2P|Pay|1520||||||8104758957@ybl|||||UPI|Pushpender||||||VPA|||||||||NA|NA';
		$encryptData = $this->yesEncryptValue($requestStr,$enckey);
...
public function updateMemberFingpayAepsStatus()
	{
		$account_id = $this->User->get_domain_account();
		 $recordList = $this->db->get_where('users',array('account_id'=>$account_id,'fingpay_2fa_aeps_status'=>1,'fingpay_2fa_ap_status'=>1))->result_array();
        if($recordList)
        {
            foreach($recordList as $list)
            {
	...
	public function SettlementAepsTranscation()
	{
	    ...
	    $recordList = $this->db->where_in('service',array('aadharpay','balwithdraw'))->get_where('member_aeps_transaction',array('account_id'=>$account_id,'status'=>2,'is_settlement'=>0))->result_array();
