...
class Website extends CI_Controller {    
    ...
    public function __construct() 
    {
        ...
        $this->lang->load('admin/website', 'english');
	...
	public function saveContact()
	{
		...
		if($status == true)
		{
			$this->Az->redirect('admin/website/contact', 'system_message_error',lang('SAVE_SUCCESS'));
		}
		else
		{
			$this->Az->redirect('admin/website/contact', 'system_message_error',lang('DB_ERROR'));
		}
	...
	public function saveAccount()
	{
		...
		if($status == true)
		{
			$this->Az->redirect('admin/website/account', 'system_message_error',lang('SAVE_SUCCESS'));
		}
		else
		{
			$this->Az->redirect('admin/website/account', 'system_message_error',lang('DB_ERROR'));
		}
	...
	public function savePrivacy()
	{
		...
		$this->Az->redirect('admin/website/privacy', 'system_message_error',lang('SAVE_SUCCESS'));
	...
	public function saveTerms()
	{
		...
		$this->Az->redirect('admin/website/terms', 'system_message_error',lang('SAVE_SUCCESS'));
	...
	public function saveService()
	{
		...
		{	
            ...
            if ($_FILES['image']['name'] != '') {
                ...
                if ($uploadError) {
                    $this->Az->redirect('admin/website/addService', 'system_message_error', $uploadError);
                } else {
			...
			if($status == true)
			{
				$this->Az->redirect('admin/website/addService', 'system_message_error',lang('SAVE_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('admin/website/addService', 'system_message_error',lang('DB_ERROR'));
			}
	...
	public function updateService()
	{
		...
		if(!$chkService)
		{
			$this->Az->redirect('admin/website/serviceList', 'system_message_error',lang('DB_ERROR'));
		}
		$this->load->library('form_validation');
		$this->form_validation->set_rules('title', 'Title', 'required|xss_clean');
        $this->form_validation->set_rules('description', 'Description', 'required');
		...
		{	
            ...
            if ($_FILES['image']['name'] != '') {
                ...
                $uploadError = $this->upload->display_errors();
                if ($uploadError) {
                    $this->Az->redirect('admin/website/editService/'.$post['id'], 'system_message_error', $uploadError);
                } else {
			...
			if($status == true)
			{
				$this->Az->redirect('admin/website/service', 'system_message_error',lang('UPDATE_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('admin/website/service', 'system_message_error',lang('DB_ERROR'));
			}
	...
	{	
		...
		$this->Az->redirect('admin/website/service', 'system_message_error',lang('DELETE_SUCCESS'));
	}
	...
	public function saveSlider()
	{
		...
		if(!isset($_FILES['image']['name']) || !$_FILES['image']['name']){
            ...
            $this->Az->redirect('admin/website/addSlider', 'system_message_error',lang('IMAGE_ERROR'));
            ...
            if ($_FILES['image']['name'] != '') {
                ...
                $uploadError = $this->upload->display_errors();
                if ($uploadError) {
                    $this->Az->redirect('admin/website/slider', 'system_message_error', $uploadError);
                } else {
			...
			if($status == true)
			{
				$this->Az->redirect('admin/website/slider', 'system_message_error',lang('SAVE_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('admin/website/slider', 'system_message_error',lang('DB_ERROR'));
			}
	...
	{	
		...
		$this->Az->redirect('admin/website/slider', 'system_message_error',lang('DELETE_SUCCESS'));
	}
	...
	public function saveTestimonial()
	{
		...
		{	
			...
			if($status == true)
			{
				$this->Az->redirect('admin/website/testimonial', 'system_message_error',lang('SAVE_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('admin/website/testimonial', 'system_message_error',lang('DB_ERROR'));
			}
	...
	{	
		...
		$this->Az->redirect('admin/website/testimonial', 'system_message_error',lang('DELETE_SUCCESS'));
	}
	...
	public function saveNews()
	{
		...
		{	
			...
			if($status == true)
			{
				$this->Az->redirect('admin/website/news', 'system_message_error',lang('SAVE_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('admin/website/news', 'system_message_error',lang('DB_ERROR'));
			}
	...
	public function updateNews()
	{
		...
		{	
			...
			if($status == true)
			{
				$this->Az->redirect('admin/website/news', 'system_message_error',lang('UPDATE_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('admin/website/news', 'system_message_error',lang('DB_ERROR'));
	...
	{	
		...
		$this->Az->redirect('admin/website/news', 'system_message_error',lang('DELETE_SUCCESS'));
	}
	...
	public function saveBlog()
	{
		...
		{	
            ...
            if ($_FILES['image']['name'] != '') {
                ...
                $uploadError = $this->upload->display_errors();
                if ($uploadError) {
                    $this->Az->redirect('admin/website/addBlog', 'system_message_error', $uploadError);
                } else {
			...
			if($status == true)
			{
				$this->Az->redirect('admin/website/blogList', 'system_message_error',lang('BLOG_SAVE_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('admin/website/addBlog', 'system_message_error',lang('DB_ERROR'));
			}
	...
	public function updateBlog()
	{
		...
		if(!$chkBlog)
		{
			$this->Az->redirect('admin/website/blogList', 'system_message_error',lang('DB_ERROR'));
		}
		$this->load->library('form_validation');
		$this->form_validation->set_rules('title', 'Title', 'required|xss_clean');
        $this->form_validation->set_rules('description', 'Description', 'required');
		...
		{	
			...
			if($status == true)
			{
				$this->Az->redirect('admin/website/blogList', 'system_message_error',lang('BLOG_UPDATE_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('admin/website/editBlog', 'system_message_error',lang('DB_ERROR'));
			}
	...
	{	
		...
		$this->Az->redirect('admin/website/blogList', 'system_message_error',lang('BLOG_DELETE_SUCCESS'));
	}
	...
	public function saveFeature()
	{
		...
		{	
            ...
            if ($_FILES['image']['name'] != '') {
                ...
                $uploadError = $this->upload->display_errors();
                if ($uploadError) {
                    $this->Az->redirect('admin/website/addFeature', 'system_message_error', $uploadError);
                } else {
			...
			if($status == true)
			{
				$this->Az->redirect('admin/website/featureList', 'system_message_error',lang('FEATURE_SAVE_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('admin/website/addFeature', 'system_message_error',lang('DB_ERROR'));
			}
	...
	public function updateFeature()
	{
		...
		if(!$chkFeature)
		{
			$this->Az->redirect('admin/website/featureList', 'system_message_error',lang('DB_ERROR'));
		}
		$this->load->library('form_validation');
		$this->form_validation->set_rules('title', 'Title', 'required|xss_clean');
		...
		{	
            ...
            if ($_FILES['image']['name'] != '') {
                ...
                $uploadError = $this->upload->display_errors();
                if ($uploadError) {
                    $this->Az->redirect('admin/website/editFeature/'.$post['id'], 'system_message_error', $uploadError);
                } else {
			...
			if($status == true)
			{
				$this->Az->redirect('admin/website/featureList', 'system_message_error',lang('FEATURE_UPDATE_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('admin/website/editFeature', 'system_message_error',lang('DB_ERROR'));
			}
	...
	{	
		...
		$this->Az->redirect('admin/website/featureList', 'system_message_error',lang('FEATURE_DELETE_SUCCESS'));
	}
	...
	{	
		...
		$this->Az->redirect('admin/website/enquiryList', 'system_message_error',lang('ENQUIRY_DELETE_SUCCESS'));
	}
	...
	public function saveAppSlider()
	{
		...
		if(!isset($_FILES['image']['name']) || !$_FILES['image']['name']){
            ...
            $this->Az->redirect('admin/website/addAppSlider', 'system_message_error',lang('IMAGE_ERROR'));
            ...
            if ($_FILES['image']['name'] != '') {
                ...
                $uploadError = $this->upload->display_errors();
                if ($uploadError) {
                    $this->Az->redirect('admin/website/appSlider', 'system_message_error', $uploadError);
                } else {
			...
			if($status == true)
			{
				$this->Az->redirect('admin/website/appSlider', 'system_message_error',lang('SAVE_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('admin/website/appSlider', 'system_message_error',lang('DB_ERROR'));
			}
    ...
    public function updateAppSlider()
	{
        ...
        if ($_FILES['image']['name'] != '') {
            ...
            $uploadError = $this->upload->display_errors();
            if ($uploadError) {
                $this->Az->redirect('admin/website/editAppSlider/'.$sliderID, 'system_message_error', $uploadError);
            } else {
	...
	{	
		...
		$this->Az->redirect('admin/website/appSlider', 'system_message_error',lang('DELETE_SUCCESS'));
	}
    ...
    public function savePage()
	{
		...
		{	
			...
			if($status == true)
			{
				$this->Az->redirect('admin/website/pages', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Page added successfully.</div>');
			}
			else
			{
				$this->Az->redirect('admin/website/pages', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! something went wrong.</div>');
			}
    ...
    public function updatePage()
	{
		...
		{	
			...
			if($status == true)
			{
				$this->Az->redirect('admin/website/pages', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Page updated successfully.</div>');
			}
			else
			{
				$this->Az->redirect('admin/website/pages', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! something went wrong.</div>');
	...
	public function deletePage($page_id = 0){
        ...
        if(!$chk_page){
        	...
        	$this->Az->redirect('admin/website/pages', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Page deleted successfully.</div>');
