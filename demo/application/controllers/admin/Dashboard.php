...
class Dashboard extends CI_Controller{
	...
	public function index($uname_prefix = '' , $username = ''){
        ...
        $totalEmpMember = $this->db->where_in('role_id',array(9))->get_where('users',array('account_id'=>$domain_account_id))->num_rows();
    ...
    public function notificationAuth()
    {
     ...
     public function sendWebNotification(){
        ...
        $data = array(
            ...
            'system_info' => $this->Az->getsystemMessageInfo(),
            'system_warning' => $this->Az->getSystemMessageWarning(),
            'content_block' => 'sendWebNotification'
        );
    ...
    public function webNotificationAuth()
    {
        $account_id = $this->User->get_domain_account();
        $loggedUser = $this->User->getAdminLoggedUser(ADMIN_SESSION_ID);
        $accountData = $this->User->get_account_data($account_id);
        ...
        {   
            ...
            $status = $this->db->insert('web_notification',$post_data);
            ...
            if($status)
            {
                ...
                $this->Az->redirect('admin/dashboard/sendWebNotification', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Notification sent successfully.</div>');
            }
            else
            {
                // save system log
                ...
                $this->Az->redirect('admin/dashboard/sendWebNotification', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! Notification failed due to some reason.</div>');
            }
