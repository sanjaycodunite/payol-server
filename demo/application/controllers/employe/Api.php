...
class Api extends CI_Controller {    
    ...
    public function __construct() 
    {
        ...
        $this->load->model('employe/Api_model');		
        $this->lang->load('employe/api', 'english');
        
    }

    public function apiList(){

    	if(!$this->User->admin_menu_permission(9,1) || !$this->User->admin_menu_permission(65,2)){
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] ='<a title="edit" class="btn btn-primary btn-sm" href="'.base_url('employe/api/editApi').'/'.$list['id'].'"><i class="fa fa-edit" aria-hidden="true"></i></a> <a title="delete" class="btn btn-danger btn-sm" href="'.base_url().'employe/api/deleteApi/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to delete?\')"><i class="fa fa-trash" aria-hidden="true"></i></a> <a href="'.base_url().'employe/api/addApiOprator/'.$list['id'].'" class="btn btn-primary" style="padding: 7px 8px;font-size: 12px;">Add Operator</a> <a href="'.base_url().'employe/api/addApiCircle/'.$list['id'].'" class="btn btn-primary" style="padding: 7px 8px;font-size: 12px;">Add Circle</a>';
	...
	public function addApi(){
		...
		if(!$this->User->admin_menu_permission(9,1) || !$this->User->admin_menu_permission(62,2)){
		...
		$account_id = $this->User->get_domain_account();
	 	...
	 	// save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - Account - '.$account_id.' - IP - '.$user_ip_address.' - User('.$loggedUser['user_code'].') - Open Employe Add Api Page.]'.PHP_EOL;
        $this->User->generateAccountActivityLog($log_msg);
	 	...
	 	if($accountData['is_api_active'] == 0){
	        ...
	        $this->User->generateAccountActivityLog($log_msg);
	 		...
	 		$this->Az->redirect('employe/api/apiList', 'system_message_error',lang('MEMBER_ERROR'));
	 	}
	...
	public function saveApiAuth(){
		...
		{	
	        ...
	        $this->User->generateAccountActivityLog($log_msg);
		 	$this->Az->redirect('employe/api/apiList', 'system_message_error',lang('API_SAVE_SUCCESS'));
		 }
	...
	public function editApi($api_id = 0){
			...
			if(!$this->User->admin_menu_permission(9,1) || !$this->User->admin_menu_permission(63,2)){
	...
	public function deleteApi($api_id = 0){
		...
		if(!$this->User->admin_menu_permission(9,1) || !$this->User->admin_menu_permission(64,2)){
        ...
        $this->User->generateAccountActivityLog($log_msg);
      	...
      	$this->Az->redirect('employe/api/apiList', 'system_message_error',lang('API_DELETE_SUCCESS'));
	}
	...
	public function saveApiOprator(){
		...
		$this->Az->redirect('employe/api/apiList', 'system_message_error',lang('API_OPERATOR_SAVE_SUCCESS'));
	...
	public function saveApiCircle(){
		...
		$this->Az->redirect('employe/api/apiList', 'system_message_error',lang('API_CIRCLE_SAVE_SUCCESS'));
	...
	public function changeApi(){
		...
		if(!$this->User->admin_menu_permission(9,1) || !$this->User->admin_menu_permission(66,2)){
	...
	public function changeApiAuth()
	{
        ...
        $this->User->generateAccountActivityLog($log_msg);
 	 	if(!$post['api_id'])
 	 	{
 	 		$this->Az->redirect('employe/api/changeApi', 'system_message_error',lang('API_ERROR'));
 	 	}

 	 	if(!isset($post['optID']) || !$post['optID'])
 	 	{
 	 		$this->Az->redirect('employe/api/changeApi', 'system_message_error',lang('OPT_ERROR'));
 	 	}
		...
		if(!$chkMember)
		{
			...
			$this->Az->redirect('employe/api/changeApi', 'system_message_error',lang('API_VALID_ERROR'));
		}
		...
		if(!$chk_member)
 		{
 			$this->Az->redirect('employe/api/changeApi', 'system_message_error',lang('MEMBER_ERROR'));
        ...
        $this->User->generateAccountActivityLog($log_msg);
 		...
 		$this->Az->redirect('employe/api/changeApi', 'system_message_error',lang('API_CHANGE_SUCCESS'));
	}

	public function amountFilter(){

		if(!$this->User->admin_menu_permission(9,1) || !$this->User->admin_menu_permission(67,2)){
	...
	public function amountFilterAuth(){
		...
		{	
	        ...
	        $this->User->generateAccountActivityLog($log_msg);
		 	$this->Az->redirect('employe/api/amountFilter', 'system_message_error',lang('AMOUNT_FILTER_SAVE'));
		 }
	...
	public function updateAmountFilter(){
        ...
        if ($this->form_validation->run() == FALSE) {
	        ...
	        $this->User->generateAccountActivityLog($log_msg);
			$this->Az->redirect('employe/api/amountFilter', 'system_message_error',lang('MEMBER_ERROR'));
		}
		else
		{	
			$chk_member = $this->db->get_where('amount_active_api',array('id'=>$recordID,'account_id'=>$account_id))->num_rows();
	...
	public function deleteAmountFilter($recordID = 0)
	{
        ...
        $this->User->generateAccountActivityLog($log_msg);
 		...
 		$this->Az->redirect('employe/api/amountFilter', 'system_message_error',lang('AMOUNT_FILTER_DELETE_SUCCESS'));
	}
