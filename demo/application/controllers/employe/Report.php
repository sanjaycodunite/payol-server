...
class Report extends CI_Controller {    
	...
	public function recharge($status = 0){
		...
		if(!$this->User->admin_menu_permission(3,1) || !$this->User->admin_menu_permission(17,2)){
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] = "<a href=".base_url('employe/report/rechargeInvoice/').$list['recharge_display_id']." style='text-decoration:none;' target='_blank'>Invoice</a>";	
				...
				if($list['status'] == 1) {
					...
					{
						$nestedData[] = '<a href="'.base_url('employe/report/refundRecharge').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to refund this recharge?\')" class="btn btn-sm btn-primary">Refund</a> <a href="'.base_url('employe/report/successRecharge').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to success this recharge?\')" class="btn btn-sm btn-primary">Success</a>';
					}
				}
				elseif($list['status'] == 2) {
					...
					else
					{
						...
						{
							$nestedData[] = '<a href="'.base_url('employe/report/refundRecharge').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to refund this recharge?\')" class="btn btn-sm btn-primary">Refund</a>';
						}
					}
				}
	...
	public function refundRecharge($recharge_id = 0)
	{
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/recharge', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/recharge', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Recharge Already Refunded.</div>');
		}
		...
		$this->Az->redirect('employe/report/recharge', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Recharge refunded successfully.</div>');
	}
	...
	public function successRecharge($recharge_id = 0)
	{
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/recharge', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Recharge Status Already Updated.</div>');
		}
		...
		$this->Az->redirect('employe/report/recharge', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Recharge got success.</div>');
	}
	...
	public function refundPayout($recharge_id = 0)
	{
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/moneyTransfer', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/moneyTransfer', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Transaction Already Refunded/Success.</div>');
		}
		...
		$this->Az->redirect('employe/report/moneyTransfer', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction refunded successfully.</div>');
	}
	...
	public function successPayout()
	{
		...
		if(!$bank_rrn)
		{
			$this->Az->redirect('employe/report/moneyTransfer', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Please enter Bank RRN.</div>');
		}
		// check member
		$chkMember = $this->db->get_where('user_fund_transfer',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		...
		$this->Az->redirect('employe/report/moneyTransfer', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction successfully Credited.</div>');
	}
	...
	public function moneyTransferHistory(){
		...
		if(!$this->User->admin_menu_permission(3,1) || !$this->User->admin_menu_permission(20,2)){
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				if($list['invoice_no']){
				 ...
				 $nestedData[] = '<a href="'.base_url('employe/report/transferInvoice/'.$list['id'].'').'" target="_blank">'.$list['invoice_no'].'</a>';
				...
				if($list['status'] == 2) {
					$nestedData[] = '<a href="'.base_url('employe/report/refundMoneyTransfer').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to refund this transaction?\')" class="btn btn-sm btn-primary">Refund</a> <a href="#" onclick="successMoneyTranfer('.$list['id'].'); return false;" class="btn btn-sm btn-success">Success</a>';
				}
				elseif($list['status'] == 3) {
					$nestedData[] = 'Not Allowed';
				}
	...
	public function refundMoneyTransfer($recharge_id = 0)
	{
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/moneyTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/moneyTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Transaction Already Refunded/Success.</div>');
		}
		...
		$this->Az->redirect('employe/report/moneyTransferHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction refunded successfully.</div>');
	}
	...
	public function successMoneyTransfer()
	{
		...
		if(!$bank_rrn)
		{
			$this->Az->redirect('employe/report/moneyTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Please enter Bank RRN.</div>');
		}
		// check member
		$chkMember = $this->db->get_where('user_money_transfer',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		...
		$this->Az->redirect('employe/report/moneyTransferHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction successfully Credited.</div>');
	}
	...
	public function rechargeCommision(){
		...
		if(!$this->User->admin_menu_permission(3,1) || !$this->User->admin_menu_permission(24,3)){
	...
	public function bbpsCommision(){
		...
		if(!$this->User->admin_menu_permission(3,1) || !$this->User->admin_menu_permission(20,3)){
	...
	public function liveRecharge(){
        ...
        $this->parser->parse('employe/layout/column-3' , $data);
    }
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] = "<a href=".base_url('employe/report/bbpsLiveInvoice/').$list['recharge_display_id']." style='text-decoration:none;' target='_blank'>Invoice</a>";	
				...
				if($list['status'] == 1) {
					...
					{
						$nestedData[] = '<a href="'.base_url('employe/report/refundBbps').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to refund this recharge?\')" class="btn btn-sm btn-primary">Refund</a> <a href="'.base_url('admin/report/successBbps').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to success this recharge?\')" class="btn btn-sm btn-primary">Success</a>';
					}
				}
				elseif($list['status'] == 2) {
					$nestedData[] = '<font color="green">Success</font>';
					...
					else
					{
						...
						{
							$nestedData[] = '<a href="'.base_url('employe/report/refundBbps').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to refund this recharge?\')" class="btn btn-sm btn-primary">Refund</a>';
						}
					}
				}
				elseif($list['status'] == 3) {
	...
	public function refundBbps($recharge_id = 0)
	{
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/bbpsHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/bbpsHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Bill Payment Already Refunded.</div>');
		}
		...
		$this->Az->redirect('employe/report/bbpsHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Bill Payment refunded successfully.</div>');
	}
	...
	public function successBbps($recharge_id = 0)
	{
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/bbpsHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Bill Payment Status Already Updated.</div>');
		}
		...
		$this->Az->redirect('employe/report/bbpsHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Bill Payment got success.</div>');
	}
	...
	{	
		...
		// datatable column index  => database column name
			...
			// getting total number records without any search
			$sql = "SELECT a.*, b.user_code as user_code FROM tbl_aeps_member_kyc as a INNER JOIN tbl_users as b ON b.id = a.member_id  where a.account_id = '$account_id'";
			
			$totalData = $this->db->query($sql)->num_rows();
			$totalFiltered = $totalData;  // when there is no search parameter then total number rows = total number filtered rows.
		
		
			$sql = "SELECT a.*, b.user_code as user_code,c.state as state_name,d.city_name FROM tbl_aeps_member_kyc as a INNER JOIN tbl_users as b ON b.id = a.member_id LEFT JOIN tbl_aeps_state as c ON c.id = a.state_id LEFT JOIN tbl_city as d ON d.city_id = a.city_id where a.account_id = '$account_id'";
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				if($list['service'] == 'balwithdraw' && ($list['status'] == 1 || $list['status'] == 3) && date('Y-m-d',strtotime($list['created'])) == date('Y-m-d')) {
					$nestedData[] = '<a href="'.base_url('employe/report/checkAepsStatus').'/'.$list['id'].'" class="btn btn-sm btn-primary">Check Status</a>';
				}
				else
				{
					$nestedData[] = 'Not Allowed';
				...
				$nestedData[] = "<a href=".base_url('employe/report/aepsInvoice/').$list['id']." style='text-decoration:none;' target='_blank'>Invoice</a>";
	...
	public function checkAepsStatus($recharge_id = 0)
	{
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/aepsHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/aepsHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Transaction Status Already Updated.</div>');
		}
		...
		if($response == true)
		{
			$this->Az->redirect('employe/report/aepsHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction Status Updated successfully.</div>');
		}
		else
		{
			$this->Az->redirect('employe/report/aepsHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Your transaction got failed.</div>');
		}
	}
	...
	public function upiCollectionReport(){
		...
		if(!in_array(5, $activeService)){
			$this->Az->redirect('employe/dashboard', 'system_message_error',lang('AUTHORIZE_ERROR'));
		}
	...
	public function checkUpiColStatus($recharge_id = 0)
	{
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/upiCollectionReport', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/upiCollectionReport', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Transaction Status Already Updated.</div>');
		}
		...
		{
			$this->Az->redirect('employe/report/upiCollectionReport', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction Status Updated successfully.</div>');
		}
		else
		{
			$this->Az->redirect('employe/report/upiCollectionReport', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Transaction failed due to '.$response['message'].'</div>');
		}
	}
        ...
        if($get_filter_data){
            ...
            foreach($get_filter_data as $list){
                ...
                if($list['status'] == 1){
                    ...
                    $nestedData[] = '<a href="'.base_url('employe/report/approveRequest/'.$list['user_id']).'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to approve request?\')" title="Approve Request" class="btn btn-success btn-sm"><i class="fa fa-check" aria-hidden="true"></i></a>
                    <a href="'.base_url('employe/report/rejectRequest/'.$list['user_id']).'/'.$list['id'].'" title="Reject Request" onclick="return confirm(\'Are you sure you want to reject request?\')" class="btn btn-danger btn-sm"><i class="fa fa-times" aria-hidden="true"></i></a>';
                }   
                else{
	...
	{	
     ...
     public function approveRequest($user_id = 0, $id=0){
        ...
        if(!$chk_request){
            ...
            $this->Az->redirect('employe/report/changeAccountList', 'system_message_error',lang('MEMBER_ERROR'));
            ...
            $this->Az->redirect('employe/report/changeAccountList', 'system_message_error',lang('REQUEST_APPROVE_SUCCESS'));
        ...
        if(!$chk_request){
            ...
            $this->Az->redirect('employe/report/changeAccountList', 'system_message_error',lang('REQUEST_REJECT_SUCCESS'));
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] = "<a href=".base_url('employe/report/cashDepositeInvoice/').$list['id']." style='text-decoration:none;' target='_blank'>Invoice</a>";
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				if($list['status'] == 2) {
					$nestedData[] = '<a href="'.base_url('employe/report/checkDmtStatus').'/'.$list['id'].'" class="btn btn-sm btn-primary">Check Status</a>';
				}
				elseif($list['status'] == 3) {
					$nestedData[] = 'Not Allowed';
				}
	...
	public function checkDmtStatus($recharge_id = 0)
	{
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/dmtHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/dmtHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Transaction Status Already Updated.</div>');
		}
		...
		$this->Az->redirect('employe/report/dmtHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction Status Updated successfully.</div>');
	}
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				if($list['status'] == 1) {
					...
					$nestedData[] = '<font color="red">Deactive</font>'.'<a title="delete" class="btn btn-danger btn-sm" href="'.base_url('employe/report/deleteKyc').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to delete?\')"><i class="fa fa-trash" aria-hidden="true"></i></a>';
...
public function newAepsHistory(){
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				if($list['status'] == 2) {
					$nestedData[] = '<a href="'.base_url('employe/report/refundNewPayout').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to refund this transaction?\')" class="btn btn-sm btn-primary">Refund</a> <a href="#" onclick="successNewPayout('.$list['id'].'); return false;" class="btn btn-sm btn-success">Success</a>';
				}
				elseif($list['status'] == 3 && $list['is_refund_by_callback'] == 0) {
					$nestedData[] = 'Not Allowed';
				}
				elseif($list['status'] == 3 && $list['is_refund_by_callback'] == 1) {
					$nestedData[] = '<a href="'.base_url('employe/report/refundNewPayout').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to refund this transaction?\')" class="btn btn-sm btn-primary">Refund</a>';
				}
				elseif($list['status'] == 4) {
					$nestedData[] = 'Not Allowed';
				}
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				{
					$nestedData[] = '<a href="'.base_url('employe/report/failedIciciAepsTxn').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to failed this transaction?\')" class="btn btn-sm btn-primary">Failed</a> <a href="'.base_url('employe/report/successIciciAepsTxn').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to success this transaction?\')" class="btn btn-sm btn-success">Success</a>';
				}
				else
				{
					$nestedData[] = 'Not Allowed';
				...
				$nestedData[] = "<a href=".base_url('employe/report/iciciAepsInvoice/').$list['id']." style='text-decoration:none;' target='_blank'>Invoice</a>";
	...
	public function successIciciAepsTxn($recharge_id = 0)
	{
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/iciciAepsHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/iciciAepsHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Transaction Already Failed.</div>');
		}
		...
		$this->Az->redirect('employe/report/iciciAepsHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction successfully Credited.</div>');
	}
	...
	public function failedIciciAepsTxn($recharge_id = 0)
	{
		...
		$this->Az->redirect('employe/report/iciciAepsHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction failed successfully.</div>');
	}
        ...
        if($get_filter_data){
            ...
            foreach($get_filter_data as $list){
                ...
                if($list['status'] == 1){
                    ...
                    $nestedData[] = '<a href="'.base_url('employe/report/approveIciciAccountRequest/'.$list['user_id']).'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to approve request?\')" title="Approve Request" class="btn btn-success btn-sm"><i class="fa fa-check" aria-hidden="true"></i></a>
                    <a href="'.base_url('employe/report/rejectIciciAccountRequest/'.$list['user_id']).'/'.$list['id'].'" title="Reject Request" onclick="return confirm(\'Are you sure you want to reject request?\')" class="btn btn-danger btn-sm"><i class="fa fa-times" aria-hidden="true"></i></a>';
                }   
                else{
    ...
    public function approveIciciAccountRequest($user_id = 0, $id=0){
        ...
        if(!$chk_request){
            ...
            $this->Az->redirect('employe/report/iciciChangeAccountList', 'system_message_error',lang('MEMBER_ERROR'));
            ...
            $this->Az->redirect('employe/report/iciciChangeAccountList', 'system_message_error',lang('REQUEST_APPROVE_SUCCESS'));
        ...
        if(!$chk_request){
            ...
            $this->Az->redirect('employe/report/iciciChangeAccountList', 'system_message_error',lang('REQUEST_REJECT_SUCCESS'));
      ...
      public function tdsReport(){
		...
		$data = array(
            ...
            'system_info' => $this->Az->getsystemMessageInfo(),
            'system_warning' => $this->Az->getSystemMessageWarning(),
            'content_block' => 'employe/tds-report'
        );
        $this->parser->parse('employe/layout/column-1' , $data);
...
public function getTdsReportList()
	...
	public function refundNewPayout($recharge_id = 0)
	{
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/newMoneyTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/newMoneyTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Transaction Already Refunded/Success.</div>');
		}
		...
		$this->Az->redirect('employe/report/newMoneyTransferHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction refunded successfully.</div>');
	}
	...
	public function successNewPayout()
	{
		...
		if(!$bank_rrn)
		{
			$this->Az->redirect('employe/report/newMoneyTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Please enter Bank RRN.</div>');
		}
		// check member
		$chkMember = $this->db->get_where('user_new_fund_transfer',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		...
		$this->Az->redirect('employe/report/newMoneyTransferHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction successfully Credited.</div>');
	}
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				if($list['status'] == 2) {
					$nestedData[] = '<a href="'.base_url('employe/report/refundUpiPayout').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to refund this transaction?\')" class="btn btn-sm btn-primary">Refund</a> <a href="#" onclick="successUpiPayout('.$list['id'].'); return false;" class="btn btn-sm btn-success">Success</a>';
				}
				elseif($list['status'] == 3) {
					$nestedData[] = 'Not Allowed';
				}
	...
	public function refundUpiPayout($recharge_id = 0)
	{
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/upiTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/upiTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Transaction Already Refunded/Success.</div>');
		}
		...
		$this->Az->redirect('employe/report/upiTransferHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction refunded successfully.</div>');
	}
	...
	public function successUpiPayout()
	{
		...
		if(!$bank_rrn)
		{
			$this->Az->redirect('employe/report/upiTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Please enter Bank RRN.</div>');
		}
		// check member
		$chkMember = $this->db->get_where('user_new_fund_transfer',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		...
		$this->Az->redirect('employe/report/upiTransferHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction successfully Credited.</div>');
	}
...
public function iciciAepsInvoice($id = ''){
	...
	public function deleteKyc($id)
	{
		...
		$this->Az->redirect('employe/report/newAepsKyc', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Kyc delete successfully.</div>');
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				if($list['status'] == 1)
				{
					$nestedData[] ='<a title="edit" class="btn btn-primary btn-sm" href="'.base_url('employe/report/refundNsdlPan').'/'.$list['id'].'">Refund</a>';
				}
				else
				{
					$nestedData[] = 'Not Allowed';
    ...
    public function refundNsdlPan($recharge_id = 0)
	{
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/nsdlPanList', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}
		...
		if(!$chkMember)
		{
			$this->Az->redirect('employe/report/nsdlPanList', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Transaction Already Refunded/Success.</div>');
		}
		...
		$this->Az->redirect('employe/report/nsdlPanList', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction refunded successfully.</div>');
	}
...
public function getGstReportList()
	...
	public function updateFindPanImage(){
      ...
      $post = $this->input->post();
     	...
     	$pan_photo = '';
			if(isset($_FILES['pan_photo']['name']) && $_FILES['pan_photo']['name']){
				...
				if($uploadError){
					$this->Az->redirect('employe/report/findPanReport', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$uploadError.'</div>');
				}
				else
				{
      ...
      $this->Az->redirect('employe/report/findPanReport', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>PAN Image Update successfully.</div>');
	...
	public function updateUtiBalanceAuth($requestID = 0, $status = 0)
	{
		...
		if(!$chk_request_id)
		{
			$this->Az->redirect('employe/report/utiBalanceReport', 'system_message_error',lang('WALLET_ERROR'));
		}
		...
		else
		{
            ...
            $this->Az->redirect('employe/report/utiBalanceReport', 'system_message_error',lang('REQUEST_APPROVE_SUCCESS'));
		}
	...
	public function rejectUtiBalance(){
      ...
      $this->Az->redirect('employe/report/utiBalanceReport', 'system_message_error',lang('REQUEST_REJECT_SUCCESS'));
