...
class Aeps extends CI_Controller {    
    ...
    public function __construct() 
    {
        ...
        $this->load->model('user/Aeps_model');		
        $this->lang->load('user/aeps', 'english');
	...
	public function index(){
		...
		if(!$user_aeps_status){
			$this->Az->redirect('user/dashboard', 'system_message_error',lang('AEPS_ACTIVE_ERROR'));
		}
	...
	public function activeAeps(){
		...
		if($user_aeps_status)
		{
			$this->Az->redirect('user/dashboard', 'system_message_error',lang('AEPS_MEMBER_ERROR'));
		}
		...
		if($chk_kyc)
		{
			...
			$this->Az->redirect('user/aeps', 'system_message_error',lang('AEPS_ACTIVE_SUCCESS'));
		}
	...
	public function activeAuth()
	{
		...
		{	
			...
			if(isset($_FILES['aadhar_photo']['name']) && $_FILES['aadhar_photo']['name']){
				...
				if($uploadError){
					$this->Az->redirect('user/aeps/activeAeps', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$uploadError.'</div>');
				}
				else
				{
			...
			if($status == 1)
			{
				...
				$this->Az->redirect('user/aeps/otpVerify/'.$encodeFPTxnId, 'system_message_error',lang('AEPS_OTP_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('user/aeps/activeAeps', 'system_message_error',sprintf(lang('AEPS_ACTIVE_FAILED'),$response['msg']));
			}
	...
	public function otpVerify($encodeFPTxnId = ''){
		...
		if(!$chk_encode_id)
		{
			$this->Az->redirect('user/aeps/activeAeps', 'system_message_error',lang('AEPS_ENCODED_ID_ERROR'));
		}
	...
	public function otpAuth()
	{
		...
		{
			$this->Az->redirect('user/aeps/otpVerify/'.$encodeFPTxnId, 'system_message_error',lang('AEPS_OTP_ERROR'));
		}
		...
		if($status == 1)
		{
			$this->Az->redirect('user/aeps/capture/'.$encodeFPTxnId, 'system_message_error',lang('AEPS_OTP_VERIFY_SUCCESS'));
		}
		else
		{
			$this->Az->redirect('user/aeps/activeAeps', 'system_message_error',sprintf(lang('AEPS_ACTIVE_FAILED'),$response['msg']));
	...
	public function resendOtp($encodeFPTxnId = ''){
		...
		if($status == 1)
		{
			$this->Az->redirect('user/aeps/otpVerify/'.$encodeFPTxnId, 'system_message_error',lang('AEPS_OTP_RESEND_SUCCESS'));
		}
		else
		{
			$this->Az->redirect('user/aeps/activeAeps', 'system_message_error',sprintf(lang('AEPS_ACTIVE_FAILED'),$response['msg']));
