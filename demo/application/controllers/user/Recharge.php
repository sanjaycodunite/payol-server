...
class Recharge extends CI_Controller{
    ...
    public function __construct() {
        ...
        $this->load->model('user/Master_model');      
    ...
    {   
        ...
        {   
            ...
            if($user_before_balance < $post['amount']){
                ...
                $this->Az->redirect('user/recharge/mobilePrepaid', 'system_message_error',lang('WALLET_ERROR'));
           ...
           /* $is_cogent_instantpay_api = $this->User->get_admin_instant_cogent_api($account_id);
            if($is_cogent_instantpay_api)
            {
                ...
                if($admin_wallet_balance < $post['amount']){
                    ...
                    $this->Az->redirect('user/recharge/mobilePrepaid', 'system_message_error',lang('ADMIN_WALLET_BALANCE_ERROR'));
            ...
            if($user_before_balance < $final_deduct_wallet_balance){
                ...
                $this->Az->redirect('user/recharge/mobilePrepaid', 'system_message_error',lang('MIN_WALLET_ERROR'));
            ...
            if($response['status'] && $response['api_id'])
            {
                ...
                if($response['is_instantpay_api'])
                {
                    ...
                    {
                        $this->Az->redirect('user/recharge/mobilePrepaid', 'system_message_error',sprintf(lang('RECHARGE_FAILED'),'Sorry ! Your eKyc not approved yet, please submit your eKyc.'));
                    }
                }
                ...
                if(isset($api_response['status']) && ($api_response['status'] == 1 || $api_response['status'] == 2))
                {
                    ...
                    if($api_response['status'] == 1){
                        ...
                        $this->Az->redirect('user/recharge/mobilePrepaid', 'system_message_error',lang('RECHARGE_PENDING'));
                    ...
                    elseif($api_response['status'] == 2)
                    {
                        ...
                        $this->Az->redirect('user/recharge/mobilePrepaid', 'system_message_error',lang('RECHARGE_SUCCESS'));
                ...
                else
                {
                    ...
                    $this->Az->redirect('user/recharge/mobilePrepaid', 'system_message_error',sprintf(lang('RECHARGE_FAILED'),$api_response['opt_msg']));
            ...
            else
            {
                ...
                $this->Az->redirect('user/recharge/mobilePrepaid', 'system_message_error',lang('API_ERROR'));
    ...
    {   
        ...
        {   
            ...
            if($chk_wallet_balance['wallet_balance'] < $post['amount']){
                ...
                $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Insufficient Wallet Error]'.PHP_EOL;
                $this->User->generateLog($log_msg);
                $this->Az->redirect('user/recharge/dth', 'system_message_error',lang('WALLET_ERROR'));   
            ...
            if($user_before_balance < $final_deduct_wallet_balance){
                ...
                $this->Az->redirect('user/recharge/dth', 'system_message_error',lang('MIN_WALLET_ERROR'));
            ...
            if($is_cogent_instantpay_api)
            {
                ...
                if($admin_wallet_balance < $post['amount']){
                    ...
                    $this->Az->redirect('user/recharge/dth', 'system_message_error',lang('ADMIN_WALLET_BALANCE_ERROR'));
            ...
            if($response['status'] && $response['api_id'])
            {
                ...
                if(isset($api_response['status']) && ($api_response['status'] == 1 || $api_response['status'] == 2))
                {
                    ...
                    if($api_response['status'] == 1){
                        ...
                        $this->Az->redirect('user/recharge/dth', 'system_message_error',lang('RECHARGE_PENDING'));   
                    ...
                    elseif($api_response['status'] == 2)
                    {
                        ...
                        $this->Az->redirect('user/recharge/dth', 'system_message_error',lang('RECHARGE_SUCCESS'));   
                ...
                else
                {
                    ...
                    $this->Az->redirect('user/recharge/dth', 'system_message_error',sprintf(lang('RECHARGE_FAILED'),$api_response['opt_msg']));
                         
                }
            }
            else
            {
                $this->Az->redirect('user/recharge/dth', 'system_message_error',lang('API_ERROR'));
            }
    ...
    {   
        ...
        else
        {
            ...
            {
                $this->Az->redirect('user/recharge/electricity', 'system_message_error',lang('OPERATOR_VALID_ERROR'));   
            }
            ...
            if($admin_wallet_balance < $post['amount']){
                ...
                $this->User->generateLog($log_msg);
                $this->Az->redirect('user/recharge/electricity', 'system_message_error',lang('ADMIN_WALLET_BALANCE_ERROR'));
            } */
            ...
            if($chk_wallet_balance['wallet_balance'] < $post['amount']){
                ...
                $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Insufficient wallet error.]'.PHP_EOL;
                $this->User->generateLog($log_msg);
                $this->Az->redirect('user/recharge/electricity', 'system_message_error',lang('WALLET_ERROR'));   
            ...
            if($user_before_balance < $final_deduct_wallet_balance){
                ...
                $this->Az->redirect('user/recharge/electricity', 'system_message_error',lang('MIN_WALLET_ERROR'));
            ...
            if(isset($api_response['status']) && ($api_response['status'] == 1 || $api_response['status'] == 2))
            {
                ...
                if($api_response['status'] == 1){
                    ...
                    $this->db->where('recharge_display_id',$recharge_unique_id);
                    $this->db->update('recharge_history',array('txid'=>$api_response['txid'],'operator_ref'=>$api_response['operator_ref'],'api_response_id'=>$api_response['api_response_id']));
                    $this->Az->redirect('user/recharge/electricity', 'system_message_error',lang('RECHARGE_PENDING'));
                }
                else
                {
                    // update recharge status
                    ...
                    $this->User->generateLog($log_msg);
                    $this->Az->redirect('user/recharge/electricity', 'system_message_error',lang('RECHARGE_SUCCESS'));
                }
            }
            else
            {
                ...
                $this->db->where('recharge_display_id',$recharge_unique_id);
                $this->db->update('recharge_history',array('status'=>3));
                $this->Az->redirect('user/recharge/electricity', 'system_message_error',lang('RECHARGE_FAILED'));
            }
   ...
   public function getOperatorPlanList()
    ...
    public function complainAuth()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Az->redirect('user/report/recharge', 'system_message_error',lang('FORM_ERROR'));
        }
        else
        {   
            ...
            if(!$chk_recharge)
            {
                ...
                $this->Az->redirect('user/report/recharge', 'system_message_error',lang('AUTHORIZE_ERROR'));  
            ...
            if($status == true)
            {
                $this->Az->redirect('user/report/recharge', 'system_message_error',lang('COMPLAIN_SAVED'));
            }
            else
            {
                $this->Az->redirect('user/report/recharge', 'system_message_error',lang('COMMON_ERROR'));
            }
    ...
    public function ekycAuth()
    {
        ...
        {   
            ...
            if($response['status'] == 1)
            {
                ...
                $this->Az->redirect('user/recharge/ekycOtp/'.$response['otpReferenceID'], 'system_message_error',$msg);
            }
            else
            {
                ...
                $this->Az->redirect('user/recharge/ekyc', 'system_message_error',$msg);
            }
    ...
    public function ekycOtpAuth()
    {
        ...
        {   
            ...
            {
                $msg = '<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$response['msg'].'</div>';
                $this->Az->redirect('user/recharge/ekycOtp/'.$otpReferenceID, 'system_message_error',$msg);
            }
