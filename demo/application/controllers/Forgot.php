...
class Forgot extends CI_Controller{
    ...
    public function resendOtpAuth($encode_opt_code = ''){
		...
		if(isset($decodeResponse['type']) && $decodeResponse['type'] == 'success')
	    {
			...
			$this->Az->redirect('forgot/otp/'.$encrypt_otp_code, 'system_message_error', '<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Otp sent to your registered mobile number. Please Verify.</div>');
		}
		else{

			$error = $decodeResponse['message'];
			$this->Az->redirect('forgot/otp/'.$encrypt_otp_code, 'system_message_error', '<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$error.'</div>');
	...
	public function transcationPasswordAuth(){
        ...
        $response = array();
        $this->load->library('form_validation');
        $this->form_validation->set_rules('new_password', 'New Transcation Password', 'required|xss_clean|min_length[6]');
