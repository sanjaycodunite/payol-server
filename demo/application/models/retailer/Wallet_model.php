...
class Wallet_model extends CI_Model {
    ...
    {       
            ...
            // save system log
            $log_msg = '['.date('d-m-Y H:i:s').' - RT('.$loggedUser['user_code'].') - Credit/Debit Wallet Member #'.$member_code.' Amount '.$type_title.'.]'.PHP_EOL;
            ...
            // save system log
            $log_msg = '['.date('d-m-Y H:i:s').' - DT('.$loggedUser['user_code'].') - Wallet Amount Debited from  Retailer Account.]'.PHP_EOL;
                $this->User->generateLog($log_msg);
    ...
    public function cibPayout($account_holder_name,$account_no,$ifsc,$amount,$transaction_id,$bankID,$txnType)
    {
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Payout API Post Data - '.$plainText.']'.PHP_EOL;
        $this->User->generateLog($log_msg);
        ...
        curl_close ($curl);
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Payout API Encode Response - '.$result.']'.PHP_EOL;
        $this->User->generateLog($log_msg);
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Payout API Response - '.$finalResponse.']'.PHP_EOL;
        $this->User->generateLog($log_msg);
    ...
    public function cibMoneyTransfer($account_holder_name,$account_no,$ifsc,$amount,$transaction_id,$bankID,$txnType)
    {
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - DMT API Post Data - '.$plainText.']'.PHP_EOL;
        $this->User->generateLog($log_msg);
        ...
        curl_close ($curl);
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - DMT API Encode Response - '.$result.']'.PHP_EOL;
        $this->User->generateLog($log_msg);
        /*$result = '""';*/
        ...
        $decodeResponse = json_decode($finalResponse,true);
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - DMT API Response - '.$finalResponse.']'.PHP_EOL;
        $this->User->generateLog($log_msg);
    ...
    public function mapCashQrName($ref_id,$loggedAccountID)
    {
     ...
     public function moveWalletBalance($post)
    {       
            $account_id = $this->User->get_domain_account();
            $loggedUser = $this->User->getAdminLoggedUser(RETAILER_SESSION_ID);
            $loggedAccountID = $loggedUser['id'];
            
            $before_balance = $this->db->get_where('users',array('id'=>$loggedAccountID,'account_id'=>$account_id))->row_array();
            $member_code = $before_balance['user_code'];
            $member_name = $before_balance['name'];
            ...
            //move to disstributor 
             ...
             $distributor_before_balance = $this->db->get_where('users',array('id'=>$post['member'],'account_id'=>$account_id))->row_array();

             $distributor_wallet_balance = $this->User->getMemberWalletBalanceSP($post['member']);
