...
<div class="card shadow ">
            ...
            <div class="card-body">
            <?php echo form_open_multipart('distributor/settlement/fundTransferAuth', array('id' => 'admin_profile'),array('method'=>'post')); ?>
              ...
              <select class="form-control selectpicker" name="bene_id" id="bene_id" data-live-search="true">
                ...
                <?php if($benList){ ?>
                  <?php foreach($benList as $list){ ?>
                    <option value="<?php echo $list['ben_id']; ?>" <?php if($bene_id == $list['ben_id']){ ?> selected="selected" <?php } ?>><?php echo $list['account_holder_name'].' ('.$list['account_no'].')'; ?></option>
                  <?php } ?>
                <?php } ?>
              </select>
