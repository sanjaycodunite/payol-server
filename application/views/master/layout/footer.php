    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <a class="btn btn-primary btn-sm" href="{site_url}master/dashboard/logout">Logout</a>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="modal fade" id="instantLoanModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <?php $loggedUser = $this->User->getAdminLoggedUser(MASTER_DIST_SESSION_ID); ?>
  ...
  <script src="{site_url}skin/admin/js/master-custom.js"></script>
 <?php
   if($content_block == 'aeps/capture' || $content_block == 'aeps/list'){
  ?>
   <script src="{site_url}skin/admin/js/master-aeps-custom.js"></script>
    ...
    <script src="{site_url}skin/admin/js/master-new-aeps-custom.js"></script>
  <?php }  elseif($content_block == 'fingpayaeps/capture'  || $content_block == 'fingpayaeps/capture' || $content_block == 'fingpayaeps/capture' || $content_block == 'fingpayaeps/list'  || $content_block == 'fingpayaeps/member-register' || $content_block == 'fingpayaeps/member-login' ) { ?>
       <script src="{site_url}skin/admin/js/master-fingpay-aeps-custom.js"></script>
      
      <?php } else { ?>
        
        <script src="{site_url}skin/admin/js/master-instantpay-aeps-custom.js"></script>

      <?php } ?>
   
  <script src="{site_url}skin/admin/js/master-datatable.js"></script>
...
$("#special_price_from").datetimepicker({
    ...
    $('[data-countdown]').each(function() {
        ...
        $this.html(event.strftime('%D Day %H h %M m %S s'));
          ...
          var siteUrl = $("#siteUrl").val();
          $("#club-live-bid-btn").html('<a class="blink_me" href="'+siteUrl+'master/saving/clubChatLiveAuth/'+club_id+'/'+requestID+'"><i class="fa fa-comment"></i>View Live Bidding</a>');
...
function getDashboardSummary()
{
  ...
  var siteUrl = $("#siteUrl").val();
  $.ajax({                
    url:siteUrl+'master/dashboard/getDashboardSummary',                        
    success:function(r){
...
function clubRoundSuperTimer()
{
  ...
  var requestID = $("#requestID").val();
  $.ajax({                
    url:siteUrl+'master/saving/clubRoundSuperTimer/'+club_id+'/'+requestID+'/'+roundNo,                        
    success:function(r){
...
function getClubLiveMember()
{
  ...
  var requestID = $("#requestID").val();
  $.ajax({                
    url:siteUrl+'master/saving/getClubLiveMembers/'+club_id+'/'+requestID,                        
    success:function(r){
...
function getClubChatList()
{
  ...
  var requestID = $("#requestID").val();
  $.ajax({                
    url:siteUrl+'master/saving/getClubChatList/'+club_id+'/'+requestID+'/'+roundNo,                        
    success:function(r){
...
function getClubRoundStatus()
{
  ...
  var requestID = $("#requestID").val();
  $.ajax({                
    url:siteUrl+'master/saving/getClubRoundStatus/'+club_id+'/'+requestID+'/'+roundNo,                        
    success:function(r){
...
$('#update-task-message').keypress(function(event) {
    if (event.keyCode == 13) {
        ...
        if($("#update-task-message").val() != '')
        {
          ...
          $.ajax({                
            type:'POST',
            url:siteUrl+'master/saving/clubChatAuth/'+club_id+'/'+requestID+'/'+roundNo,
...
$("#club-bid-btn").click(function(){
  ...
  if($("#update-task-message").val() != '')
  {
    ...
    $.ajax({                
      type:'POST',
      url:siteUrl+'master/saving/clubBidAuth/'+club_id+'/'+requestID+'/'+roundNo,
      data:{'message':$("#update-task-message").val()},
      success:function(r){
