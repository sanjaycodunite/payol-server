      ...
      <!-- Logout Modal-->
      <div class="modal fade" id="instantLoanModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
          aria-hidden="true">
          <div class="modal-dialog" role="document">
              <div class="modal-content">
                  <div class="modal-header">
                  ...
                  <div class="modal-body" style="text-align: center;"><img
                          src="https://chart.googleapis.com/chart?chs=400x400&cht=qr&chl=<?php echo urlencode($instantLoanText); ?>&choe=UTF-8" />
                  </div>
                  <?php } else { ?>
                  <div class="modal-body">We are not able to offer Loan to you at this point. Please make daily
                      transaction till 1 Lakh for 3 Months. We are working hard to include you soon.</div>
                  <?php } ?>
                  <div class="modal-footer">
      ...
      <script src="{site_url}skin/admin/full-loader/please-wait-fullpage-loader.js"></script>
      ...
      <script type="text/javascript" language="javascript"
          src="{site_url}skin/admin/vendor/datatables/dataTables.buttons.min.js"></script>
      <script type="text/javascript" language="javascript"
          src="{site_url}skin/admin/vendor/datatables/buttons.flash.min.js"></script>
      <script type="text/javascript" language="javascript" src="{site_url}skin/admin/vendor/datatables/jszip.min.js">
      </script>
      <script type="text/javascript" language="javascript" src="{site_url}skin/admin/vendor/datatables/pdfmake.min.js">
      </script>
      <script type="text/javascript" language="javascript"
          src="{site_url}skin/admin/vendor/datatables/buttons.html5.min.js"></script>
  ...
  ?>
      ...
      <?php } elseif($content_block == 'newaeps/capture' || $content_block == 'newaeps/list'  || $content_block == 'newaeps/member-registration' || $content_block == 'newaeps/member-login' || $content_block == 'newaeps/cash-deposite' ) { ?>
        ...
        success: function(r) {
            ...
            if (data["status"] == 1) {
                ...
                if (data['lastChatDatetime'] != lastChatDatetime) {
                    $("#club-chat-block").animate({
                        scrollTop: $('#club-chat-block').prop("scrollHeight")
                    }, 100);
                }
...
$('#update-task-message').keypress(function(event) {
    if (event.keyCode == 13) {
        ...
        if ($("#update-task-message").val() != '') {
            $("#update-task-comment-loader").html('<img src="' + siteUrl +
                'skin/admin/images/small-loading.gif" />');
            $.ajax({
                type: 'POST',
                url: siteUrl + 'retailer/saving/clubChatAuth/' + club_id + '/' + requestID + '/' +
                    roundNo,
                data: {
                    'message': $("#update-task-message").val()
                },
                success: function(r) {
                    ...
                    if (data["status"] == 1) {
                        ...
                        $("#update-task-comment-loader").html('<font color="red">' + data['msg'] +
                            '</font>');
                    }
    ...
    $.ajax({
        ...
        success: function(r) {
            ...
            if (data["status"] == 1) {
                ...
                $("#totalBbpsSuccessBlock").html(data['successBbpsAmount'] + ' / ' + data[
                    'successBbpsRecord']);
                $("#totalBbpsPendingBlock").html(data['pendingBbpsAmount'] + ' / ' + data[
                    'pendingBbpsRecord']);
                $("#totalBbpsFailedBlock").html(data['failedBbpsAmount'] + ' / ' + data[
                    'failedBbpsRecord']);

                $("#totalAepsSuccessBlock").html(data['successAepsAmount'] + ' / ' + data[
                    'successAepsRecord']);
                $("#totalAepsFailedBlock").html(data['failedAepsAmount'] + ' / ' + data[
                    'successAepsRecord']);


                $("#totalMoneySuccessBlock").html(data['successMoneyAmount'] + ' / ' + data[
                    'successMoneyRecord']);
                $("#totalMoneyPendingBlock").html(data['pendingMoneyAmount'] + ' / ' + data[
                    'pendingMoneyRecord']);
                $("#totalMoneyFailedBlock").html(data['failedMoneyAmount'] + ' / ' + data[
                    'failedMoneyRecord']);


                $("#totalMoneyTransferSuccessBlock").html(data['successMoneyTransferAmount'] + ' / ' + data[
                    'successMoneyTransferRecord']);
                $("#totalMoneyTransferPendingBlock").html(data['pendingMoneyTransferAmount'] + ' / ' + data[
                    'pendingMoneyTransferRecord']);
                $("#totalMoneyTransferFailedBlock").html(data['failedMoneyTransferAmount'] + ' / ' + data[
                    'failedMoneyTransferRecord']);
