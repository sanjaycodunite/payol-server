...
class Saving extends CI_Controller {    
    ...
    public function __construct() 
    {
        ...
        $this->load->model('master/Saving_model');		
        $this->lang->load('master/saving', 'english');
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] = '<a href="'.base_url().'master/saving/clubLiveAuth/'.$list['id'].'/'.$requestID.'"><button type="button" class="btn btn-success">View LIVE</button></a>';
    ...
    public function clubRequestAuth()
    {
		...
		if(!$chkClub)
		{
			$this->Az->redirect('master/saving/clubList', 'system_message_error',lang('AUTHORIZE_ERROR'));
		}
		...
		if($chkMemberAction)
		{
			$this->Az->redirect('master/saving/clubList', 'system_message_error',lang('REQUEST_ALREADY_FOUND'));
		}

		if(!isset($post['is_agree']) && isset($post['accept']))
		{
			$this->Az->redirect('master/saving/clubList', 'system_message_error',lang('TERMS_ERROR'));
		}
		...
		{
			$this->Az->redirect('master/saving/clubList', 'system_message_error',lang('REQUEST_SENT_SUCCESS'));
		}
		else
		{
			$this->Az->redirect('master/saving/clubList', 'system_message_error',lang('REQUEST_SENT_DECLINE'));
		}
    }
    ...
    public function clearDuePayment($club_id = 0, $requestID = 0)
    {
		...
		if(!$isDueAmount)
		{
			$this->Az->redirect('master/saving/clubLiveAuth/'.$club_id.'/'.$requestID, 'system_message_error',lang('NO_CLUB_DUE_AMOUNT'));
		}
		...
		{
			$this->Az->redirect('master/saving/clubLiveAuth/'.$club_id.'/'.$requestID, 'system_message_error',lang('WALLET_BAL_ERROR'));
		}
        ...
        $this->Az->redirect('master/saving/clubLiveAuth/'.$club_id.'/'.$requestID, 'system_message_error',lang('DUE_CLEAR_SUCCESS'));
    }
