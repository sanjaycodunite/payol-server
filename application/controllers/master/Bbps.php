...
class Bbps extends CI_Controller{
    ...
    public function __construct() {
        ...
        $this->load->model('master/Bbps_model');      
        $this->lang->load('master/bbps', 'english');
	...
	public function index(){
        ...
        $boradbandPostpaidBillerList = $this->User->get_bbps_biller_list(8);
        ...
        if($billerParams)
        {
           foreach($billerParams as $pkey=>$plist)
           {
            ...
            if($plist['type']!=1)
              ...
              $str.='<label>'.$plist['paramDisplayName'].'*</label>';
              $str.='<input class="form-control" name="params[]" placeholder="Enter '.$plist['paramDisplayName'].'" type="text" />';
              $str.='</div>';     
               }
               else
               {
                   
                   $str.=  '<select class="form-control"  name="params[]">';
					$str.='<option value="">Select</option>';

					$type = explode(',',$plist['paramName']);
    ...
    {   
        ...
        else
        {
            ...
            // save system log
            $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Mobile Postpaid Post Data - '.json_encode($post).']'.PHP_EOL;
            ...
            {   
                ...
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - E-Wallet Balance - '.$user_before_balance.']'.PHP_EOL;
                $this->User->generateBBPSLog($log_msg);
                ...
                if($user_before_balance < $final_deduct_wallet_balance){
                    ...
                    // save system log
                    $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Wallet Insufficient Error.]'.PHP_EOL;
                    $this->User->generateBBPSLog($log_msg);
                }
                elseif($is_cogent_instantpay_api && $admin_wallet_balance < $post['amount']){
                    ...
                    // save system log
                    $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Admin Wallet Insufficient Error.]'.PHP_EOL;
                    $this->User->generateBBPSLog($log_msg);
                }
                else
                {
                    ...
                    // save system log
                    $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Bill Pay Final Response - '.json_encode($response).']'.PHP_EOL;
                    $this->User->generateBBPSLog($log_msg);
    ...
    {   
        ...
        else
        {
            ...
            // save system log
            $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Electricity Post Data - '.json_encode($post).']'.PHP_EOL;
            $this->User->generateBBPSLog($log_msg);
    ...
    {   
        ...
        else
        {
            ...
            // save system log
            $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - DTH Post Data - '.json_encode($post).']'.PHP_EOL;
            $this->User->generateBBPSLog($log_msg);
    ...
    {   
        ...
        else
        {
            ...
            // save system log
            $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Master Post Data - '.json_encode($post).']'.PHP_EOL;
            $this->User->generateBBPSLog($log_msg);
