    ...
    public function fundTransferAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($responseData['statuscode']) && $responseData['statuscode'] == 'TXN' && $responseData['status'] == 'Transaction Successful') {
                ...
                $this->Az->redirect('master/transfer/beneficiaryList', 'system_message_error', lang('MANUAL_TRANSFER_FAILED', $api_msg));
            } else {
...
*/
    ...
    public function upiPayoutTransferAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($amount < 0) {
                $this->Az->redirect(
                    'master/transfer/upiPayoutBeneficiaryList',
            ...
            if (isset($response_data[5]) && $response_data[5] == 'Transaction success') {
                ...
                $this->Az->redirect('master/transfer/upiPayoutBeneficiaryList', 'system_message_error', lang('MANUAL_TRANSFER_FAILED', $api_msg));
            } else {
                $this->Az->redirect('master/transfer/upiPayoutBeneficiaryList', 'system_message_error', lang('MANUAL_TRANSFER_SUCCESS'));
            }
        }
    ...
    public function upiOpenPayoutTransferAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($response_data[5]) && $response_data[5] == 'Transaction success') {
                ...
                $this->db->where('account_id', $account_id);
                $this->db->where('user_id', $loggedAccountID);
                $this->db->update('user_money_transfer', ['api_response' => json_encode($response), 'status' => 4, 'updated' => date('Y-m-d H:i:s')]);
            ...
            if ($beneficiary_data) {
                ...
                foreach ($beneficiary_data as $key => $list) {
                    ...
                    $str .=
                        ...
                        base_url() .
                        'master/transfer/newPayoutFundTransfer/' .
                        $list['id'] .
                        '"> Transfer</a>&nbsp;<a title="Fund Transfer" class="btn btn-danger btn-sm" href="' .
                        base_url() .
                        'master/transfer/deleteBeneficiaryAccount/' .
                        $list['id'] .
    ...
    public function getUpiBankBeneficiary()
    {
        ...
        $benificaryList = $this->db->get_where('instantpay_upi_payout_user_benificary', ['account_id' => $account_id, 'user_id' => $loggedAccountID, 'instantpay_upi_payout_user_benificary.is_delete' => 0])->num_rows();

        if ($benificaryList) {
            // check recharge status
            $beneficiary_data = $this->db->get_where('instantpay_upi_payout_user_benificary', ['account_id' => $account_id, 'user_id' => $loggedAccountID, 'instantpay_upi_payout_user_benificary.is_delete' => 0])->result_array();
