...
class Wallet extends CI_Controller {    
	...
	public function saveWallet()
	{
		...
		// save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Credit/Debit Wallet Post Data - '.json_encode($post).']'.PHP_EOL;
		...
		{	
			...
			{
				// save system log
		        $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Credit/Debit Wallet Member Valid Error.]'.PHP_EOL;
		        $this->User->generateLog($log_msg);
				$this->Az->redirect('master/wallet/walletList', 'system_message_error',lang('WALLET_ERROR'));
			}
			$post['type'] = 1;
			$type = $post['type'];
			if($type == 1)
			{
				...
				// save system log
		        $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Credit/Debit Member Wallet Balance - '.$wallet_balance.'.]'.PHP_EOL;
		        $this->User->generateLog($log_msg);
				if($post['amount'] > $final_deduct_wallet_balance)
				{
					// save system log
			        $log_msg = '['.date('d-m-Y H:i:s').' - MD('.$loggedUser['user_code'].') - Credit/Debit Member Insufficient Wallet Error.]'.PHP_EOL;
			        $this->User->generateLog($log_msg);
					$this->Az->redirect('master/wallet/walletList', 'system_message_error',lang('WALLET_BALANCE_ERROR'));
				}
			}
			else
			{
			...
			if($status == true)
			{
				$this->Az->redirect('master/wallet/walletList', 'system_message_error',lang('WALLET_SAVED'));
			}
			else
			{
				$this->Az->redirect('master/wallet/walletList', 'system_message_error',lang('WALLET_ERROR'));
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] = ($list['is_upi'] == 1) ? 'Yes' : 'No';
				$nestedData[] = $list['upi_id'];
				...
				if($list['status'] == 1)
				{
					...
					$nestedData[] ='<a title="Approve" class="btn btn-success btn-sm" href="'.base_url('master/wallet/updateRequestAuth').'/'.$list['id'].'/1" onclick="return confirm(\'Are you sure you want to approve this request?\')"><i class="fa fa-check" aria-hidden="true"></i></a> <a title="Reject" class="btn btn-danger btn-sm" href="'.base_url('master/wallet/updateRequestAuth').'/'.$list['id'].'/2" onclick="return confirm(\'Are you sure you want to reject this request?\')"><i class="fa fa-times" aria-hidden="true"></i></a>';
				}
	...
	public function updateRequestAuth($requestID = 0, $status = 0)
	{
		...
		if(!$chk_request_id)
		{
			$this->Az->redirect('master/wallet/requestList', 'system_message_error',lang('WALLET_ERROR'));
		}

		if($status == 1)
		{
			// check account wallet balance
			$wallet_balance = $this->User->getMemberWalletBalance($loggedAccountID);
			...
			{
				$this->Az->redirect('master/wallet/requestList', 'system_message_error',lang('WALLET_BALANCE_ERROR'));
			}
		}
			
		$this->Wallet_model->updateRequestAuth($requestID,$status);
		if($status == 1){
			$this->Az->redirect('master/wallet/requestList', 'system_message_error',lang('REQUEST_APPROVE_SUCCESS'));
		}
		else
		{
			$this->Az->redirect('master/wallet/requestList', 'system_message_error',lang('REQUEST_REJECT_SUCCESS'));
		}
	}
    ...
    public function requestAuth()
	{
		...
		else
		{
			...
			if($status == true)
			{
				$this->Az->redirect('master/wallet/myRequestList', 'system_message_error',lang('REQUEST_GENERATE_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('master/wallet/myRequestList', 'system_message_error',lang('WALLET_ERROR'));
			}
	...
	public function paymentResponse()
	{
		...
		if (empty($_POST['razorpay_payment_id']) === false)
		{
			...
			catch(SignatureVerificationError $e)
			{
				...
				$this->Az->redirect('master/wallet/topup', 'system_message_error',$e->getMessage());
			}
		}
		...
		{	
            ...
            $this->Az->redirect('master/wallet/topup', 'system_message_error',lang('PAYMENT_SUCCESS'));
		...
		{	
			...
			$this->Az->redirect('master/wallet/topup', 'system_message_error',lang('PAYMENT_FAILED'));
		}
	...
	public function walletTransferAuth()
	{
		...
		{	
			...
			if(!$chk_member)
			{
		        ...
		        $this->User->generateLog($log_msg);
				$this->Az->redirect('master/wallet/walletTransfer', 'system_message_error','<div class="alert alert-danger alert-dismissable"> <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><b>Sorry !!</b> MemberID not exists.</div>');
			}
			...
			if(do_hash($post['transaction_password']) != $chk_txn_password['transaction_password']){
				...
				$this->Az->redirect('master/wallet/walletTransfer', 'system_message_error','<div class="alert alert-danger alert-dismissable"> <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><b>Sorry !!</b> Transaction password is wrong.</div>');
			...
			if($wallet_balance < $post['amount']){
				...
				$this->Az->redirect('master/wallet/walletTransfer', 'system_message_error','<div class="alert alert-danger alert-dismissable"> <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><b>Sorry !!</b> you have insufficient balance in your wallet.</div>');
			...
			if($status == true)
			{
				$this->Az->redirect('master/wallet/walletTransfer', 'system_message_error',lang('WALLET_SAVED'));
			}
			else
			{
				$this->Az->redirect('master/wallet/walletTransfer', 'system_message_error',lang('WALLET_ERROR'));
			}
	...
	{	
		...
		{	
			...
			else
			{
				...
				$this->Az->redirect('master/wallet/dynamicQr', 'system_message_error',$error);
			}
	...
	public function activeQr()
    {
		...
		{
			$error = '<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$responseData['message'].'</div>';
			$this->Az->redirect('master/wallet/addFund', 'system_message_error',$error);
		}
    }
	...
	public function mapQr()
    {
		...
		// check qr is active or not
	 	...
	 	if($chk_qr_status)
		{
			$this->Az->redirect('master/wallet/addFund', 'system_message_error',lang('QR_ALREDY_ACTIVE_ERROR'));
		}
	...
	{	
		...
		{	
			...
			if(!$chk_txn_id)
			{
				$this->Az->redirect('master/wallet/mapQr', 'system_message_error',lang('MAP_TXN_ERROR'));
			}
			...
			if($chk_txn_id)
			{
				$this->Az->redirect('master/wallet/mapQr', 'system_message_error',lang('MAP_ALREADY_ERROR'));
			}
			...
			$this->Az->redirect('master/wallet/addFund', 'system_message_error',lang('MAP_SUCCESS'));
	...
	{	
		...
		{	
			...
			{
				$error = '<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$responseData['message'].'</div>';
				$this->Az->redirect('master/wallet/dynamicCashQr', 'system_message_error',$error);
			}
	...
	public function activeCashQr()
    {
		...
		{
			$error = '<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$responseData['message'].'</div>';
			$this->Az->redirect('master/wallet/addCashFund', 'system_message_error',$error);
		}
    }
	...
	public function mapCashQr()
    {
		...
		// check qr is active or not
	 	...
	 	if($chk_qr_status)
		{
			$this->Az->redirect('master/wallet/addCashFund', 'system_message_error',lang('QR_ALREDY_ACTIVE_ERROR'));
		}
	...
	{	
		...
		{	
			...
			if(!$chk_txn_id)
			{
				$this->Az->redirect('master/wallet/mapCashQr', 'system_message_error',lang('MAP_TXN_ERROR'));
			}
			...
			if($chk_txn_id)
			{
				$this->Az->redirect('master/wallet/mapCashQr', 'system_message_error',lang('MAP_ALREADY_ERROR'));
			}
			...
			$this->Az->redirect('master/wallet/addCashFund', 'system_message_error',lang('MAP_SUCCESS'));
    ...
    public function payolWalletAuth()
	{
		...
		{	
			if($post['amount'] < 1)
			{
				$this->Az->redirect('master/wallet/payolTransfer', 'system_message_error',lang('AMOUNT_VALID_ERROR'));
			}
			...
			if(!$chk_member)
			{
				$this->Az->redirect('master/wallet/payolTransfer', 'system_message_error',lang('MEMBER_VALID_ERROR'));
			}
			...
			if($post['amount'] > $wallet_balance)
			{
				$this->Az->redirect('master/wallet/payolTransfer', 'system_message_error',lang('WALLET_BALANCE_ERROR'));
			}
			...
			{
				$this->Az->redirect('master/wallet/payolTransfer', 'system_message_error',lang('SELF_TRANSFER_ERROR'));
			}
			...
			$this->Az->redirect('master/wallet/payolTransferVerify/'.$encode_txnid, 'system_message_error','');
	...
	public function payolTransferVerify($encode_txnid = '')
    {
		...
		if(!$chk_member)
		{
			$this->Az->redirect('master/wallet/payolTransfer', 'system_message_error',lang('WALLET_ERROR'));
		}
    ...
    public function payolWalletPinAuth()
	{
		...
		{	
			...
			if(!$chk_member)
			{
				$this->Az->redirect('master/wallet/payolTransferVerify/'.$encode_txnid, 'system_message_error',lang('TXN_PIN_ERROR'));
			}
			...
			$this->Az->redirect('master/wallet/payolTransfer', 'system_message_error',lang('PAYOL_TRANSFER_SUCCESS'));
