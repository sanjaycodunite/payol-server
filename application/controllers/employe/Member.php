...
class Member extends CI_Controller {    
	...
	public function deactiveMemberList(){
		...
		if(!$this->User->admin_menu_permission(2,1) || !$this->User->admin_menu_permission(134,2)){
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				{
					$nestedData[] ='<a title="edit" class="btn btn-success btn-sm" href="'.base_url('employe/member/activeNsdlAeps').'/'.$list['id'].'">Active Aeps</a>';
	...
	{	
		...
		$this->Az->redirect('employe/member/memberList', 'system_message_error',lang('AEPS_ACTIVE_SUCCESS'));
	}
	...
	public function portalKyc(){
        ...
        $loggedUser = $this->User->getAdminLoggedUser(ADMIN_EMPLOYE_SESSION_ID);
         ...
         if(!$this->User->admin_menu_permission(2,1) || !$this->User->admin_menu_permission(138,2)){
        ...
        if($get_filter_data){
            ...
            foreach($get_filter_data as $list){
                ...
                if($list['status'] == 1){
                    ...
                    $nestedData[] = '<a href="'.base_url('employe/member/approveRequest/'.$list['id']).'" onclick="return confirm(\'Are you sure you want to approve request?\')" title="Approve Request" class="btn btn-success btn-sm"><i class="fa fa-check" aria-hidden="true"></i></a>
                    <a href="'.base_url('employe/member/rejectRequest/'.$list['id']).'" title="Reject Request" onclick="return confirm(\'Are you sure you want to reject request?\')" class="btn btn-danger btn-sm"><i class="fa fa-times" aria-hidden="true"></i></a>';
                }   
                else{
    ...
    public function approveRequest($id = 0){
        ...
        $chk_request = $this->db->get_where('portal_kyc',array('account_id'=>$account_id,'id'=>$id))->row_array();
         ...
         if(!$this->User->admin_menu_permission(2,1) || !$this->User->admin_menu_permission(138,2)){
	        ...
	        if(!$chk_request){
	            ...
	            $this->Az->redirect('employe/member/portalKyc', 'system_message_error',lang('MEMBER_ERROR'));
            ...
            $this->Az->redirect('employe/member/portalKyc', 'system_message_error',lang('REQUEST_APPROVE_SUCCESS'));
        ...
        if(!$chk_request){
            ...
            $this->Az->redirect('employe/member/portalKyc', 'system_message_error',lang('REQUEST_REJECT_SUCCESS'));
