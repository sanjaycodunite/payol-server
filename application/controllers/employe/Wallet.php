...
class Wallet extends CI_Controller {    
	...
	public function upiWalletTransferAuth()
	{
		...
		{	
			...
			if($post['amount'] < 0){
				$this->Az->redirect('employe/wallet/upiWalletTransfer', 'system_message_error',lang('WALLET_AMOUNT_VALID_ERROR'));
			}
			$member_before_balance = $this->User->getMemberUpiWalletBalanceSP($post['member'],1);
			if($member_before_balance < $post['amount']){
				$this->Az->redirect('employe/wallet/upiWalletTransfer', 'system_message_error',lang('MEMBER_WALLET_BALANCE_ERROR'));
			}
			...
			if($status == true)
			{
				$this->Az->redirect('employe/wallet/upiWalletList', 'system_message_error',lang('WALLET_SAVED'));
			}
			else
			{
				$this->Az->redirect('employe/wallet/upiWalletList', 'system_message_error',lang('WALLET_ERROR'));
			}
	...
	public function oldWalletList(){
		...
		if(!$this->User->admin_menu_permission(6,1) || !$this->User->admin_menu_permission(135,2)){
