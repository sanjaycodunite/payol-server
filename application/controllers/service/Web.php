...
class Web extends CI_Controller{
    ...
    //         	$check_txn_password = $this->db->select('decoded_transaction_password')->get_where('users',array('id'=>$loggedAccountID,'account_id'=>$account_id))->row_array();
   ...
   public function complainAuth(){
	...
	public function getBbpsServiceList(){
		...
		$countryList = $this->db->order_by('title','asc')->get_where('mobikwik_bbps_service')->result_array();
		$data = array();
		if($countryList)
		{
			foreach($countryList as $key=>$list)
	...
	{
		// get state list
  		$bankList = $this->db->order_by('bank_name','DESC')->get('aeps_bank_list')->result_array();
  ...
  public function aepsOtpAuth(){
    ...
    public function aepsKycBioAuth(){
        ...
        else
        {
			...
			{
			   	log_message('debug', 'AEPS KYC Bio Auth API Post Data 1');	
				$userID = $post['user_id'];
				$encodeFPTxnId = $post['encodeFPTxnId'];
...
-----END RSA PRIVATE KEY-----';
	 ...
	 public function FinoCms()
   {
    ...
    public function iciciAepsActiveAuth(){
        ...
        $this->form_validation->set_rules('aadhar_photo', 'Aadhar Photo', 'required|xss_clean');
        $this->form_validation->set_rules('pancard_photo', 'Pancard Photo', 'required|xss_clean');
        $this->form_validation->set_rules('account_no', 'Account Number', 'required|xss_clean');
	...
	public function checkVpaAuth(){
		...
		{	
				...
				$enckey = '7153f272dbdc71b459c6b49551988767';
					...
					$requestStr = 'YES0000011547194|YESD04F53227F184F6C88741779F8C|'.$vpa_id.'|T|com.msg.app|0.0 ,0.0 |Mumbai|172.16.50.65|MOB|'.$transid.'|Android7.0|351898082074677|89914902900059967808|4e9389eadeea5b7c|02:00:00:00:00:00|02:00:00:00:00:00|||||||||NA|NA';
					$encryptData = $this->User->yesEncryptValue($requestStr,$enckey);
	...
	public function scanPayAuth(){
		...
		$this->form_validation->set_rules('vpa_name', 'VPD ID', 'required|xss_clean');
		$this->form_validation->set_rules('amount', 'Amount ', 'required|xss_clean|callback_maximumCheck|numeric');
				...
				if(isset($response_data[5]) && $response_data[5] == 'Transaction success')
						{
							...
							$this->db->where('user_id',$loggedAccountID);
							$this->db->where('id',$recordID);
							$this->db->update('user_scan_pay_transfer',array('api_response'=>$output,'rrn'=>$rrno,'status'=>3,'updated'=>date('Y-m-d H:i:s')));
							...
							$response = array(
							 ...
							 'rrn_no' => $rrno,
							 'transaction_id'=>$transaction_id
                             //'date_time' => date('Y-m-d H:i:s')->format('h:i:s A \O\n d-m-Y')
							);
    					...
    					$this->db->where('account_id',$account_id);
    					$this->db->where('user_id',$loggedAccountID);
    					$this->db->update('user_scan_pay_transfer',array('api_response'=>$output,'status'=>4,'updated'=>date('Y-m-d H:i:s')));
			...
			$activeService = $this->User->account_active_service($loggedAccountID);
				...
				{	
					...
					elseif($loggedAccountID == $to_member_id)
					{
						...
						$from_member = $this->db->select('name,mobile')->get_where('users',array('id'=>$loggedAccountID))->row_array();
			            $from_member_name = $from_member['name'];
			            $from_mobile = str_repeat('X', 6) . substr($from_member['mobile'], 6);
			            $to_member = $this->db->select('name,mobile')->get_where('users',array('id'=>$to_member_id))->row_array();
