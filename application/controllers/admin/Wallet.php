...
class Wallet extends CI_Controller {    
		...
		{	
			...
			if($post['type'] == 2)
			{
				...
				if($member_before_balance < $post['amount']){
					$this->Az->redirect('admin/wallet/addWallet', 'system_message_error',lang('MEMBER_WALLET_BALANCE_ERROR'));
				}
			}
			...
			if($accountData['account_type'] == 2){
				...
				if($post['type'] == 1)
				{
					if($admin_wallet_balance < $post['amount']){
						...
						$this->Az->redirect('admin/wallet/addWallet', 'system_message_error',lang('WALLET_BALANCE_ERROR'));
					}
				}
			}
	...
	public function upiWalletTransfer()
    {
		$loggedUser = $this->User->getAdminLoggedUser(ADMIN_SESSION_ID);
		$account_id = $this->User->get_domain_account();
		// get users list
        ...
        $data = array(
            ...
            'meta_keywords' => lang('SITE_NAME'),
            'meta_description' => lang('SITE_NAME'),
            'content_block' => 'wallet/upiWalletTransfer',
            'memberList'	=> $memberList,
            'manager_description' => lang('SITE_NAME'),
          	'system_message' => $this->Az->getSystemMessageError(),
            'system_info' => $this->Az->getSystemMessageInfo(),
    ...
    // save member
	public function upiWalletTransferAuth()
	{
		$account_id = $this->User->get_domain_account();
		$accountData = $this->User->get_account_data($account_id);
		$loggedUser = $this->User->getAdminLoggedUser(ADMIN_SESSION_ID);
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->upiWalletTransfer();
		}
		else
		{	
			
			if($post['amount'] < 0){
				$this->Az->redirect('admin/wallet/upiWalletTransfer', 'system_message_error',lang('WALLET_AMOUNT_VALID_ERROR'));
			}
			$member_before_balance = $this->User->getMemberUpiWalletBalanceSP($post['member'],1);
			if($member_before_balance < $post['amount']){
				$this->Az->redirect('admin/wallet/upiWalletTransfer', 'system_message_error',lang('MEMBER_WALLET_BALANCE_ERROR'));
			}
			
			$status = $this->Wallet_model->transferUpiWallet($post);
			
			if($status == true)
			{
				$this->Az->redirect('admin/wallet/upiWalletList', 'system_message_error',lang('WALLET_SAVED'));
			}
			else
			{
				$this->Az->redirect('admin/wallet/upiWalletList', 'system_message_error',lang('WALLET_ERROR'));
			}
	...
	public function getMemberUpiWalletBalance($memberID = 0)
	{
		$account_id = $this->User->get_domain_account();
		echo json_encode(array(
			'status' => 1,
			'balance' => $this->User->getMemberUpiWalletBalanceSP($memberID)
		));
	}
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				if($list['status'] == 1)
				{
					...
					$nestedData[] ='<a title="Approve" class="btn btn-success btn-sm" href="'.base_url('admin/wallet/updateRequestAuth').'/'.$list['id'].'/1" onclick="return confirm(\'Are you sure you want to approve this request?\')"><i class="fa fa-check" aria-hidden="true"></i></a> <a title="Reject" class="btn btn-danger btn-sm" href="'.base_url('admin/wallet/updateRequestAuth').'/'.$list['id'].'/2" onclick="return confirm(\'Are you sure you want to reject this request?\')"><i class="fa fa-times" aria-hidden="true"></i></a>';
				}
	...
	public function updateRequestAuth($requestID = 0, $status = 0)
	{
		...
		if(!$chk_request_id)
		{
			$this->Az->redirect('admin/wallet/requestList', 'system_message_error',lang('WALLET_ERROR'));
		}
		...
		if($accountData['account_type'] == 2){
			if($status == 1)
			{
				...
				{
					$this->Az->redirect('admin/wallet/requestList', 'system_message_error',lang('WALLET_BALANCE_ERROR'));
				}
			}
		}
		...
		if($status == 1){
			$this->Az->redirect('admin/wallet/requestList', 'system_message_error',lang('REQUEST_APPROVE_SUCCESS'));
		}
		else
		{
			$this->Az->redirect('admin/wallet/requestList', 'system_message_error',lang('REQUEST_REJECT_SUCCESS'));
		}
	}
    ...
    public function requestAuth()
	{
		...
		else
		{
			...
			if($status == true)
			{
				$this->Az->redirect('admin/wallet/myRequestList', 'system_message_error',lang('REQUEST_GENERATE_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('admin/wallet/myRequestList', 'system_message_error',lang('WALLET_ERROR'));
			}
			...
			if($user){
				  ...
				  if($total_deduct_user){
					 ...
					 $status = $this->db->insert('wallet_deduction_history',$deduction_data);
				   	...
				   	$this->Az->redirect('admin/wallet/rwalletDeduct', 'system_message_error',lang('USER_WALLET_ERROR'));
					...
					if($status == true)
					{
						$this->Az->redirect('admin/wallet/rwalletDeduct', 'system_message_error',lang('R-WALLET_DEDUCT_SUCCESS'));
					}
					else
					{
						$this->Az->redirect('admin/wallet/rwalletDeduct', 'system_message_error',lang('WALLET_ERROR'));
					}
			}
			else{
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[]= '<a title="View" href="'.'https://paymyrecharge.in/documentmem/RwalletData/'.$list['image_url'].'" target="_blank">View</a>';
				$nestedData[] = date('d-m-Y',strtotime($list['created']));
				
				if($list['status'] == 1)
				{
					$nestedData[] = '<font color="black">Pending</font>';
					$nestedData[] ='<a title="Approve" class="btn btn-success btn-sm" href="'.base_url('admin/wallet/updateApiFundRequestAuth').'/'.$list['id'].'/1" onclick="return confirm(\'Are you sure you want to approve this request?\')"><i class="fa fa-check" aria-hidden="true"></i></a>';
				}
				elseif($list['status'] == 2)
				{
					$nestedData[] = '<font color="green">Approved</font>';
	...
	public function saveApiFundRequest()
	{
		...
		{	
			...
			if(!$chk_member)
			{
				$this->Az->redirect('admin/wallet/apiFundRequestList', 'system_message_error',lang('WALLET_ERROR'));
			}
			...
			if($status == true)
			{
				$this->Az->redirect('admin/wallet/apiFundRequestList', 'system_message_error',lang('WALLET_SAVED'));
			}
			else
			{
				$this->Az->redirect('admin/wallet/apiFundRequestList', 'system_message_error',lang('WALLET_ERROR'));
	...
	public function rejectApiFundRequest($requestID =0)
	{
        ...
        $chk_request_id = $this->db->get_where('api_member_fund_request',array('id'=>$requestID,'status'=>1,'account_id'=>$account_id))->num_rows();
         ...
         // update request status
            ...
            $api_url = 'https://paymyrecharge.in/myapi/dmr/MorningwalletRequest.aspx?transcionid='.$txnid.'&utrno='.$utrno.'&status=FAILED&memberID='.$member_id.'&Amount='.$amount.'&reason='.urlencode($post['description']);
        ...
        log_message('debug', 'PMR Request Reject Api response - '.json_encode($output));   
            ...
            $this->Az->redirect('admin/wallet/apiFundRequestList', 'system_message_error',lang('REQUEST_REJECT_SUCCESS'));
	...
	{	
		...
		if($extra_search)
		{
			...
			$keyword = isset($filterData[1]) ? trim($filterData[1]) : '';
			$member_id = isset($filterData[2]) ? trim($filterData[2]) : '';
			$fromDate = isset($filterData[3]) ? trim($filterData[3]) : '';
            $toDate = isset($filterData[4]) ? trim($filterData[4]) : '';
		}
		...
		// datatable column index  => database column name
		    ...
		    $this->db->query("SET SESSION group_concat_max_len = 1000000;");
			$sql = "SELECT a.*, b.user_code as user_code, b.name as name FROM tbl_member_wallet_till_oct_2023 as a INNER JOIN tbl_users as b ON b.id = a.member_id  where a.wallet_type = 1 AND a.account_id = '$account_id' AND b.role_id > 2 AND DATE(a.created) >= '".date('Y-m-d',strtotime($fromDate))."' AND DATE(a.created) <= '".date('Y-m-d',strtotime($toDate))."'";
			...
			if($status)
            {
                $sql2Filter.=" AND type = '$status'";
                
            }
            

			$sql2 = "SELECT SUM(COALESCE(CASE WHEN a.type = 1 THEN a.amount END,0)) totalCreditAmount,count( case when a.type=1 then 1 else NULL end) totalCreditRecord, SUM(COALESCE(CASE WHEN a.type = 2 THEN a.amount END,0)) totalDebitAmount,count( case when a.type=2 then 1 else NULL end) totalDebitRecord FROM tbl_member_wallet_till_oct_2023 as a INNER JOIN tbl_users as b ON b.id = a.member_id WHERE a.wallet_type = 1 AND b.role_id > 2 AND a.account_id = '$account_id'".$sql2Filter;
	...
	public function upiWalletList(){
		...
		$data = array(
            ...
            'system_info' => $this->Az->getsystemMessageInfo(),
            'system_warning' => $this->Az->getSystemMessageWarning(),
            'content_block' => 'wallet/upiWalletList'
        );
        $this->parser->parse('admin/layout/column-1' , $data);
	...
	public function getUpiWalletList()
	{	
		$account_id = $this->User->get_domain_account();
		$requestData= $this->input->get();
		$extra_search = $requestData['extra_search'];	
		...
		// datatable column index  => database column name
		    ...
		    $this->db->query("SET SESSION group_concat_max_len = 1000000;");
			$sql = "SELECT a.*, b.user_code as user_code, b.name as name FROM tbl_member_upi_wallet as a INNER JOIN tbl_users as b ON b.id = a.member_id  where a.wallet_type = 1 AND a.account_id = '$account_id' AND b.role_id > 2 AND DATE(a.created) >= '".date('Y-m-d',strtotime($fromDate))."' AND DATE(a.created) <= '".date('Y-m-d',strtotime($toDate))."'";
			...
			$sql2 = "SELECT SUM(COALESCE(CASE WHEN a.type = 1 THEN a.amount END,0)) totalCreditAmount,count( case when a.type=1 then 1 else NULL end) totalCreditRecord, SUM(COALESCE(CASE WHEN a.type = 2 THEN a.amount END,0)) totalDebitAmount,count( case when a.type=2 then 1 else NULL end) totalDebitRecord FROM tbl_member_upi_wallet as a INNER JOIN tbl_users as b ON b.id = a.member_id WHERE a.wallet_type = 1 AND b.role_id > 2 AND a.account_id = '$account_id'".$sql2Filter;
	...
	public function addUpiWallet()
    {
        ...
        $data = array(
            ...
            'meta_keywords' => lang('SITE_NAME'),
            'meta_description' => lang('SITE_NAME'),
            'content_block' => 'wallet/addUpiWallet',
            'memberList'	=> $memberList,
            'manager_description' => lang('SITE_NAME'),
          	'system_message' => $this->Az->getSystemMessageError(),
            'system_info' => $this->Az->getSystemMessageInfo(),
	...
	public function saveUpiWallet()
	{
		...
		{	
			...
			if($post['type'] == 2)
			{
				$member_before_balance = $this->User->getMemberUpiWalletBalanceSP($post['member'],1);
				if($member_before_balance < $post['amount']){
					$this->Az->redirect('admin/wallet/addUpiWallet', 'system_message_error',lang('MEMBER_WALLET_BALANCE_ERROR'));
				}
			}

			if($accountData['account_type'] == 2){

				$admin_wallet_balance = $this->User->getMemberUpiWalletBalanceSP($admin_id,1);
				if($post['type'] == 1)
				{
					if($admin_wallet_balance < $post['amount']){
			...
			$status = $this->Wallet_model->saveUpiWallet($post);
