...
class Report extends CI_Controller {
	...
	public function getRechargeList(){
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$queryData = $this->db->get_where('tbl_recharge_type', array('id' => $list['recharge_type']));
	...
	public function getMoneyTransferList(){
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				if($list['status'] == 2) {
					$nestedData[] = '<a href="'.base_url('admin/report/refundMoneyTransfer').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to refund this transaction?\')" class="btn btn-info btn-sm btn-primary py-1" style="margin-bottom:3px !important; padding: 1px !important;">Refund</a>
					<a href="#" onclick="successMoneyTranfer('.$list['id'].'); return false;" class="btn btn-sm btn-success" style="padding: 1px !important;">Success</a>';
				}
				elseif($list['status'] == 3) {
					$nestedData[] = 'Not Allowed';
				}
	...
	public function bbpsHistory($status = 0){
		...
		$loggedUser = $this->User->getAdminLoggedUser(ADMIN_SESSION_ID);
        $user_type = $this->db->where_in('id',array(3,4,5,6))->get('user_roles')->result_array();
        $user = $this->db->get_where('users',array('is_active'=>1,'role_id >'=>3))->result_array();
		$siteUrl = base_url();
...
public function getGstReportList()
	...
	public function upiCheckStatusAuth($recordID = 0)
	{
		...
		$account_id = $this->User->get_domain_account();
		// check member
		$chkMember = $this->db->get_where('upi_dynamic_qr',array('id'=>$recordID,'is_callback'=>0))->num_rows();
		if(!$chkMember)
		{
			$response = array(
				'status' => 0,
		...
		{
			$recordData = $this->db->get_where('upi_dynamic_qr',array('id'=>$recordID,'is_callback'=>0))->row_array();
            ...
            $get_member_role_id = $this->db->select('role_id')->get_where('users',array('account_id'=>$account_id,'id'=>$loggedAccountID))->row_array();
	...
	public function getFingpayAepsHistoryList()
	{
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				if(($list['api_response'] == 0 || $list['api_response'] == NULL) && $list['force_status'] == 0){
					$nestedData[] = '<a href="'.base_url('admin/report/failedFingpayAeps3Txn').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to failed this transaction?\')" class="btn btn-sm btn-primary">Failed</a> <a href="'.base_url('admin/report/successIciciAeps3Txn').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to success this transaction?\')" class="btn btn-sm btn-success">Success</a>';
				}else{
					$nestedData[] = 'Not Allowed';
				}
	...
	public function getFingpayReconList()
	{
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				{
					$get_data = $this->db->like('post_data',$list['txnID'])->get_where('recon_aeps_api_response',array('account_id'=>$account_id,'user_id'=>$list['member_id']))->row_array();
...
//release upi balance
    ...
    public function getReleaseUpiBalanceReport()
	{
		...
		if($get_filter_data){
			...
			$get_today_transfer_amount = $this->db->select('SUM(transfer_amount) as transfer_amount')->get_where('upi_wallet_transfer',array('account_id'=>$account_id,'member_id'=>$list['id'],'DATE(created) <'=>$today_date))->row_array();
	        ...
	        $get_today_upi_collection = $this->db->select('SUM(credit_amount) as collection_amount')->get_where('upi_transaction',array('account_id'=>$account_id,'member_id'=>$list['id'],'DATE(created)'=>$today_date))->row_array();
		...
		//today transfer_amount
			$get_today_transfer_amount = $this->db->select('SUM(transfer_amount) as transfer_amount')->get_where('upi_wallet_transfer',array('account_id'=>$account_id,'member_id'=>$member_id,'DATE(created) <'=>$today_date))->row_array();

	        $today_transfer_amount = isset($get_today_transfer_amount['transfer_amount']) ? $get_today_transfer_amount['transfer_amount'] : 0 ;

	        $get_today_upi_collection = $this->db->select('SUM(credit_amount) as collection_amount')->get_where('upi_transaction',array('account_id'=>$account_id,'member_id'=>$member_id,'DATE(created)'=>$today_date))->row_array();
	...
	public function upiChargeBackAuth($recordID = 0)
	{
		...
		$account_id = $this->User->get_domain_account();
		// check member
		$chkMember = $this->db->get_where('upi_transaction',array('id'=>$recordID,'account_id'=>$account_id,'status'=>2))->num_rows();
		if(!$chkMember)
		{
			$response = array(
				'status' => 0,
		...
		{
			$recordData = $this->db->get_where('upi_transaction',array('id'=>$recordID,'account_id'=>$account_id,'status'=>2))->row_array();
			$loggedAccountID = $recordData['member_id'];
			$txnid = $recordData['txnid'];
			$bank_rrno = $recordData['bank_rrno'];
			$amount = $recordData['amount'];
			...
			$get_chargeback_amount = $this->db->select('chargeback_charge')->get_where('users',array('account_id'=>$account_id,'id'=>$loggedAccountID))->row_array();
	        ...
	        $this->db->where('id',$recordID);
	        $this->db->update('upi_transaction',array('status'=>4,'updated'=>date('Y-m-d H:i:s'),'updated_by'=>2,'force_chargeback'=>1));
	...
	public function getUserList($user_type = 0)
	{
		...
		{
			// get city list
			$user = $this->db->get_where('users',array('account_id'=>$account_id,'role_id'=>$user_type,'is_active'=>1))->result_array();
			if($user)
			{
...
// 			die;
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				if($list['status'] == 2) {
					$nestedData[] = '<a href="'.base_url('admin/report/refundScanPay').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to refund this transaction?\')" class="btn btn-sm btn-primary">Refund</a> <a href="#" onclick="successScanPay('.$list['id'].'); return false;" class="btn btn-sm btn-success">Success</a>';
					//$nestedData[] = 'Not Allowed';
				}
				elseif($list['status'] == 3 && $list['is_refund_by_callback'] == 0) {
					$nestedData[] = 'Not Allowed';
	...
	public function getOpenMoneyTransferList()
	{
		...
		//$sql = "SELECT a.* , c.name as member_name FROM tbl_open_money_payout as a INNER JOIN tbl_users as b ON b.id = a.user_id LEFT JOIN tbl_users as c ON c.id = a.user_id where a.account_id = '$account_id' AND a.txnType!= 'UPI'";
          $sql = "SELECT DISTINCT(a.transaction_id), a.*,b.account_holder_name,b.account_no,b.mobile as benficry_mobile_no,c.name,c.user_code,c.mobile FROM tbl_open_money_payout as a INNER JOIN tbl_open_money_payout_user_benificary as b ON a.ben_id = b.ben_id INNER JOIN tbl_users as c ON a.user_id = c.id  WHERE a.account_id = '$account_id' AND  a.id > 0 ";
        if($fromDate && $toDate)
            {
                $sql.=" AND DATE(a.created) >= '".date('Y-m-d',strtotime($fromDate))."' AND DATE(a.created) <= '".date('Y-m-d',strtotime($toDate))."'";
            }
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				if($list['status'] == 2) {
					$nestedData[] = '<a href="'.base_url('admin/report/refundOpenMoneyPayout').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to refund this transaction?\')" class="btn btn-sm btn-danger">Refund</a> <a href="#" onclick="successOpenMoneyPayout('.$list['id'].'); return false;" class="btn btn-sm btn-success">Success</a>';
					//$nestedData[] = 'Not Allowed';
				}
				elseif($list['status'] == 3 && $list['is_refund_by_callback'] == 0) {
					$nestedData[] = 'Not Allowed';
				...
				elseif($list['status'] == 3 && $list['is_refund_by_callback'] == 1) {
					$nestedData[] = '<a href="'.base_url('admin/report/refundOpenMoneyPayout').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to refund this transaction?\')" class="btn btn-sm btn-primary">Refund</a>';
					//$nestedData[] = 'Not Allowed';
				}
				elseif($list['status'] == 4) {
					$nestedData[] = 'Not Allowed';
	...
	public function getSettlementTransferList(){
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] = date('d-M-Y H:i:s',strtotime($list['created']));
				if($list['status'] == 2) {
					$nestedData[] = '<a href="'.base_url('admin/report/refundSettlementMoneyTransfer').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to refund this transaction?\')" class="btn btn-sm btn-primary">Refund</a> <a href="#" onclick="successSettlementMoneyTransfer('.$list['id'].'); return false;" class="btn btn-sm btn-success">Success</a>';
					$nestedData[] = 'Not Allowed';
				}elseif($list['status'] == 3 && $list['is_refund_by_callback'] == 0) {
					$nestedData[] = 'Not Allowed';
				}elseif($list['status'] == 3 && $list['is_refund_by_callback'] == 1) {
	...
	public function refundOpenMoneyPayout($recharge_id = 0)
	{
		$account_id = $this->User->get_domain_account();
		// check member
		$chkMember = $this->db->get_where('open_money_payout',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		{
			$this->Az->redirect('admin/report/openMoneyTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}

		// check member
		$chkMember = $this->db->where_in('status',array(2,3))->get_where('open_money_payout',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		{
			$this->Az->redirect('admin/report/openMoneyTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Transaction Already Refunded/Success.</div>');
		}

		// check recharge status
		$get_recharge_data = $this->db->get_where('open_money_payout',array('id'=>$recharge_id,'account_id'=>$account_id))->row_array();
		...
		$this->db->where('user_id',$loggedAccountID);
		$this->db->where('transaction_id',$transaction_id);
		$this->db->update('open_money_payout',array('status'=>4,'force_status'=>1,'updated'=>date('Y-m-d H:i:s')));
        ...
        $get_role_id = $this->db->select('role_id,open_payout_call_back_url,user_code')->get_where('users',array('id'=>$loggedAccountID,'account_id'=>$account_id))->row_array();
		$user_role_id = isset($get_role_id['role_id']) ? $get_role_id['role_id'] : 0 ;
		$api_member_code = isset($get_role_id['user_code']) ? $get_role_id['user_code'] : 0 ;
		...
		$this->Az->redirect('admin/report/openMoneyTransferHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction refunded successfully.</div>');
	}
	...
	public function successOpenMoneyPayout()
	{
		...
		if(!$bank_rrn)
		{
			$this->Az->redirect('admin/report/openMoneyTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Please enter Bank RRN.</div>');
		}
		// check member
		$chkMember = $this->db->get_where('open_money_payout',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		...
		// check member
		$chkMember = $this->db->where_in('status',array(2))->get_where('open_money_payout',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		{
			$this->Az->redirect('admin/report/openMoneyTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Transaction Already Refunded/Success.</div>');
		}
		...
		$this->db->where('user_id',$loggedAccountID);
		$this->db->where('transaction_id',$transaction_id);
		$this->db->update('open_money_payout',array('op_txn_id'=>$optxid,'rrn'=>$bank_rrn,'status'=>3,'force_status'=>1,'updated'=>date('Y-m-d H:i:s')));
		...
		$this->Az->redirect('admin/report/openMoneyTransferHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction successfully Credited.</div>');
	}
	...
	public function getOpenMoneyPayoutData($recharge_id = 0)
	{
        ...
        $chk_txn_id = $this->db->get_where('open_money_payout',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
        if($chk_txn_id)
        {

			// check recharge status
			$get_recharge_data = $this->db->get_where('open_money_payout',array('id'=>$recharge_id))->row_array();
	...
	public function getSettlementMoneyTransferData($recharge_id = 0)
	{
        ...
        $chk_txn_id = $this->db->get_where('settlement_open_money_payout',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
        if($chk_txn_id)
        {

			// check recharge status
			$get_recharge_data = $this->db->get_where('settlement_open_money_payout',array('id'=>$recharge_id))->row_array();
	...
	public function refundSettlementMoneyTransfer($recharge_id = 0)
	{
		$account_id = $this->User->get_domain_account();
		// check member
		$chkMember = $this->db->get_where('settlement_open_money_payout',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		{
			$this->Az->redirect('admin/report/settlementMoneyTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}

		// check member
		$chkMember = $this->db->where_in('status',array(2))->get_where('settlement_open_money_payout',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		{
			$this->Az->redirect('admin/report/settlementMoneyTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Transaction Already Refunded/Success.</div>');
		}

		// check recharge status
		$get_recharge_data = $this->db->get_where('settlement_open_money_payout',array('id'=>$recharge_id,'account_id'=>$account_id))->row_array();
		...
		$this->db->where('user_id',$loggedAccountID);
		$this->db->where('transaction_id',$transaction_id);
		$this->db->update('settlement_open_money_payout',array('status'=>4,'force_status'=>1,'updated'=>date('Y-m-d H:i:s')));
		...
		$this->Az->redirect('admin/report/settlementMoneyTransferHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction refunded successfully.</div>');
	}
	...
	public function successSettlementMoneyTransfer()
	{
		...
		if(!$bank_rrn)
		{
			$this->Az->redirect('admin/report/settlementMoneyTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Please enter Bank RRN.</div>');
		}
		// check member
		$chkMember = $this->db->get_where('settlement_open_money_payout',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		...
		$this->db->where('user_id',$loggedAccountID);
		$this->db->where('transaction_id',$transaction_id);
		$this->db->update('settlement_open_money_payout',array('rrn'=>$bank_rrn,'status'=>3,'force_status'=>1,'updated'=>date('Y-m-d H:i:s')));
		$this->Az->redirect('admin/report/settlementMoneyTransferHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction successfully Credited.</div>');
	}
	...
	public function getScanPayData($recharge_id = 0)
	{
        ...
        $chk_txn_id = $this->db->get_where('user_scan_pay_transfer',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
        if($chk_txn_id)
        {

			// check recharge status
			$get_recharge_data = $this->db->get_where('user_scan_pay_transfer',array('id'=>$recharge_id))->row_array();
	...
	public function refundScanPay($recharge_id = 0)
	{
		$account_id = $this->User->get_domain_account();
		// check member
		$chkMember = $this->db->get_where('user_scan_pay_transfer',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		{
			$this->Az->redirect('admin/report/scanPayTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}

		// check member
		$chkMember = $this->db->where_in('status',array(2))->get_where('user_scan_pay_transfer',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		{
			$this->Az->redirect('admin/report/scanPayTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Transaction Already Refunded/Success.</div>');
		}

		// check recharge status
		$get_recharge_data = $this->db->get_where('user_scan_pay_transfer',array('id'=>$recharge_id,'account_id'=>$account_id))->row_array();
		...
		$this->db->where('user_id',$loggedAccountID);
		$this->db->where('transaction_id',$transaction_id);
		$this->db->update('user_scan_pay_transfer',array('status'=>4,'force_status'=>1,'updated'=>date('Y-m-d H:i:s')));
		...
		$this->Az->redirect('admin/report/scanPayTransferHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction refunded successfully.</div>');
	}
	...
	public function successScanPay()
	{
		...
		if(!$bank_rrn)
		{
			$this->Az->redirect('admin/report/scanPayTransferHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Please enter Bank RRN.</div>');
		}
		// check member
		$chkMember = $this->db->get_where('user_scan_pay_transfer',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		...
		$this->db->where('user_id',$loggedAccountID);
		$this->db->where('transaction_id',$transaction_id);
		$this->db->update('user_scan_pay_transfer',array('op_txn_id'=>$transaction_id,'rrn'=>$bank_rrn,'status'=>3,'force_status'=>1,'updated'=>date('Y-m-d H:i:s')));

		$this->Az->redirect('admin/report/scanPayTransferHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction successfully Credited.</div>');
	}
	...
	public function getUtrData($recordID = 0)
	{
		...
		$account_id = $this->User->get_domain_account();
		$accountData = $this->User->get_account_data($account_id);
		$chk_member = $this->db->get_where('open_money_payout',array('id'=>$recordID,'account_id'=>$account_id))->num_rows();
 		if(!$chk_member)
 		{
 			$response = array(
 				'status' => 0,
 		...
 		{
 			$dmrData = $this->db->get_where('open_money_payout',array('id'=>$recordID,'account_id'=>$account_id))->row_array();

 			$merchant_txn_id = $dmrData['transaction_id'];

 			// call open money get trascation

 			$chk_dmr_data = $this->db->like('post_data',$merchant_txn_id)->get_where('open_money_api_response',array('account_id'=>$account_id))->row_array();
  ...
  //       $by = isset($post['by']) ? trim($post['by']) : '';
        ...
        if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				echo "$i,$list[memberID],$list[member_name],$list[account_holder_name],$list[account_no],$list[ifsc],$list[transfer_amount],$list[transfer_charge_amount],$list[transaction_id],$list[rrn],$status,".date('d-M-Y H:i:s',strtotime($list['created'])).",".PHP_EOL;
	...
	public function failedFingpayAeps3Txn($recharge_id = 0){
		...
		if(!$chkMember)
		{
			$this->Az->redirect('admin/report/fingpayAepsHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}

		// check member
		$chkMember = $this->db->where_in('status',array(3))->get_where('member_aeps_transaction',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		{
			$this->Az->redirect('admin/report/fingpayAepsHistory', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Transaction Already Failed.</div>');
		}


		$this->db->where('id',$recharge_id);
		$this->db->where('account_id',$account_id);
		$this->db->update('member_aeps_transaction',array('status'=>3,'force_status'=>1,'message'=>'Manually Failed','updated'=>date('Y-m-d H:i:s'),'updated_by'=>$account_id));

		$this->Az->redirect('admin/report/fingpayAepsHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction failed successfully.</div>');
	}
	...
	public function successIciciAeps3Txn($recharge_id = 0)
	{
		...
		$this->db->where('id',$recharge_id);
		$this->db->where('account_id',$account_id);
		$this->db->update('member_aeps_transaction',array('status'=>2,'force_status'=>1,'message'=>'Manually Success','updated'=>date('Y-m-d H:i:s'),'updated_by'=>$account_id));
		...
		$this->Az->redirect('admin/report/fingpayAepsHistory', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transaction successfully Credited.</div>');
	}
	...
	public function getUserByType() {
		...
		$html =  "";
		$getAllUsers = $this->db->get_where('users',array('account_id'=>$account_id,'is_active'=>1,'role_id >'=>2))->result_array();
