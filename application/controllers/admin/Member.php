...
class Member extends CI_Controller {    
	...
	{	
		...
		// datatable column index  => database column name
			...
			$sql = "SELECT a.*,b.package_name FROM tbl_users as a LEFT JOIN tbl_package as b ON b.id = a.package_id where a.role_id IN (3,4,5,6,8) AND a.account_id = '$account_id' AND a.is_active = 1";
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				{
					$str.='<tr><td><b>Sponser </b></td><td><a href="'.base_url('admin/member/viewUpline/'.$sponserData['id']).'">'.$sponserData['name'].' ('.$sponserData['user_code'].')</a></td></tr>';
				}
				...
				if(count($downline_id)){
					$nestedData[]='<a href="'.base_url('admin/member/viewDownline/'.$list['id']).'" title="View Downline">'.count($downline_id).'</a>';
				}
				else
				{
				...
				$str = '<table class="table">';
				if($list['instantpay_aeps_status'] == 1)
				{
				$str.='<tr><td><b style="color:green">ICICI AEPS </b></td></tr>';	
				}
				
				if($list['new_aeps_status'] == 1)
				{
				$str.='<tr><td><b style="color:green">NSDL AEPS </b></td></tr>';	
				}
				
				if($list['fingpay_aeps_status'] == 1)
				{
				$str.='<tr><td><b style="color:green"> AEPS 3 </b></td></tr>';	
				}
				...
				$nestedData[] ='<a title="edit" class="btn btn-primary btn-sm" href="'.base_url('admin/member/editMember').'/'.$list['id'].'"><i class="fa fa-edit" aria-hidden="true"></i></a>';
				if($list['new_aeps_status'] == 0)
				{
					$nestedData[] ='<a title="edit" class="btn btn-success btn-sm" href="'.base_url('admin/member/activeNsdlAeps').'/'.$list['id'].'">Active Aeps</a>';
	...
	public function saveMember()
	{
		...
		{	
			...
			if($post['role_id'] == 1 || $post['role_id'] == 2  || $post['role_id'] == 7){
				...
				$this->Az->redirect('admin/member/addMember', 'system_message_error',lang('ROLE_ERROR'));	
			...
			if($chk_user_mobile){
				...
				$this->Az->redirect('admin/member/addMember', 'system_message_error',lang('MOBILE_ERROR'));	
			...
			if(!$chk_package){
				...
				$this->Az->redirect('admin/member/addMember', 'system_message_error',lang('PACKAGE_ERROR'));	
			...
			if($status == true)
			{
				$this->Az->redirect('admin/member/addMember', 'system_message_error',lang('MEMBER_SAVED'));
			}
			else
			{
				$this->Az->redirect('admin/member/addMember', 'system_message_error',lang('MEMBER_ERROR'));
			}
	...
	public function updateMember()
	{
		...
		{	
			if($post['role_id'] == 1 || $post['role_id'] == 2  || $post['role_id'] == 7){
				...
				$this->Az->redirect('admin/member/editMember/'.$post['id'], 'system_message_error',lang('ROLE_ERROR'));	
			...
			if(!$chk_package){
				...
				$this->Az->redirect('admin/member/editMember/'.$post['id'], 'system_message_error',lang('PACKAGE_ERROR'));	
			...
			if($chk_user_mobile){
				...
				$this->Az->redirect('admin/member/editMember/'.$post['id'], 'system_message_error',lang('MOBILE_ERROR'));	
	...
	public function saveInstantLoanQr(){
		...
		$account_id = $this->User->get_domain_account();
 	 	...
 	 	if($memberID == 0)
	 	{
	 		...
	 		$this->Az->redirect('admin/member/instantLoan', 'system_message_error',$msg);
	 	}
 	 	if($memberID > 0)
	 	{
	 	...
	 	$chkData = $this->db->get_where('account_instant_loan',array('account_id'=>$account_id,'member_id'=>$memberID))->num_rows();
		 	...
		 	$this->Az->redirect('admin/member/instantLoan', 'system_message_error',lang('CONTENT_SAVED'));
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] ='<a title="edit" class="btn btn-primary btn-sm" href="'.base_url('admin/member/viewBeneficiary').'/'.$list['id'].'"> View Beneficiary</a>';
	...
	public function viewBeneficiary($id = ''){
        ...
        {
        	$this->Az->redirect('admin/member/memberBeneficiaryList', 'system_message_error',lang('MEMBER_ERROR'));
        }
	...
	{	
		...
		$this->Az->redirect('admin/member/memberBeneficiaryList', 'system_message_error',lang('DELETE_SUCCESS'));
	}


	public function activeNsdlAeps($id)
	{	
		$account_id = $this->User->get_domain_account();
		$loggedUser = $this->User->getAdminLoggedUser(ADMIN_SESSION_ID);

		$chk_user = $this->db->get_where('users',array('account_id'=>$account_id,'id'=>$id))->num_rows();
		...
		$this->Az->redirect('admin/member/memberList', 'system_message_error',lang('AEPS_ACTIVE_SUCCESS'));
	}



	public function deactiveMemberList(){
		...
		$data = array(
            ...
            'system_info' => $this->Az->getsystemMessageInfo(),
            'system_warning' => $this->Az->getSystemMessageWarning(),
            'content_block' => 'member/deactiveMemberList'
        );
        $this->parser->parse('admin/layout/column-1' , $data);
	...
	public function getDeactiveMemberList()
	{	
		$account_id = $this->User->get_domain_account();
		$requestData= $this->input->get();
		$extra_search = $requestData['extra_search'];	
		...
		// datatable column index  => database column name
			...
			// getting total number records without any search
			$sql = "SELECT a.*,b.package_name FROM tbl_users as a LEFT JOIN tbl_package as b ON b.id = a.package_id where a.role_id IN (3,4,5,6,8) AND a.account_id = '$account_id' AND a.is_active = 0";
	...
	public function memberRequestList(){
		...
		$data = array(
            ...
            'system_info' => $this->Az->getsystemMessageInfo(),
            'system_warning' => $this->Az->getSystemMessageWarning(),
            'content_block' => 'member/memberRequestList'
        );
        $this->parser->parse('admin/layout/column-1' , $data);
	...
	public function getMemberRequestList()
	{	
		$account_id = $this->User->get_domain_account();
		$requestData= $this->input->get();
		$extra_search = $requestData['extra_search'];	
		...
		// datatable column index  => database column name
			...
			// getting total number records without any search
			$sql = "SELECT a.* FROM tbl_user_register_request as a WHERE a.account_id = '$account_id' ";
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] = $list['partner_type'];

				if($list['product_intrest'])
				{
					$service = explode(',', $list['product_intrest']);
					$nestedData[] = $service;
				}
				...
				$nestedData[] = '<a href="'.base_url('admin/member/rejectMember').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to reject this  request\')" class="btn btn-sm btn-danger"><i class="fa fa-close" aria-hidden="true"></i></a> <a href="#" onclick="approveMemberRequest('.$list['id'].'); return false;" class="btn btn-sm btn-success"><i class="fa fa-check" aria-hidden="true"></i></a>';
				}
				else
				{
				...
				if($list['status'] == 0)
				{
					$nestedData[] = '<font color="orange">Not Update</font>';
				}
		...
		public function getMemberRequestData($recharge_id = 0)
	{
		//get logged user info
        $account_id = $this->User->get_domain_account();
        $loggedUser = $this->User->getAdminLoggedUser(ADMIN_SESSION_ID);
        ...
        $chk_txn_id = $this->db->get_where('user_register_request',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
        if($chk_txn_id)
        {

			// check recharge status
			$get_recharge_data = $this->db->get_where('user_register_request',array('id'=>$recharge_id))->row_array();


			$str =  '<select class="form-control" onchange="updateMemberRequest('.$recharge_id.',this.value)" name="member_type">';
					$str.='<option value="">Select</option>';

					$role_list = $this->db->where_in('id',array(3,4,5,6))->get_where('user_roles',array('status'=>1))->result_array();
	...
	public function saveRequestMember()
	{
		$account_id = $this->User->get_domain_account();
        $loggedUser = $this->User->getAdminLoggedUser(ADMIN_SESSION_ID);
 	 	$response =array();
		...
		if($post['role_id'] == 1 || $post['role_id'] == 2  || $post['role_id'] == 7){
			...
			else
			{
				...
				$get_user_data = $this->db->get_where('user_register_request',array('account_id'=>$account_id,'id'=>$memberID))->row_array();




				$get_user_mobile = isset($get_user_data['mobile']) ? $get_user_data['mobile'] : '';

				$get_package_id = $this->db->select('id')->get_where('package',array('status'=>1,'is_default'=>1))->row_array();

				$package_id = isset($get_package_id['id']) ? $get_package_id['id'] : '';
				$post['package_id'] = $package_id;
				$post['is_active'] = 1;
				$post['min_wallet_balance'] = '0.00';
				$post['min_upi_wallet_balance'] = '0.00';
				$post['upi_lean_amount'] = '0.00';
				$post['chargeback_charge'] =  0;

				// check mobile already exits or not
			$chk_user_mobile = $this->db->get_where('users',array('account_id'=>$account_id,'mobile'=>$get_user_mobile))->num_rows();
			if($chk_user_mobile){
...

			{
				...
				'upi_lean_amount' =>$post['upi_lean_amount'],
				'chargeback_charge' =>$post['chargeback_charge'],
				'min_upi_wallet_balance' => $post['min_upi_wallet_balance']
				
			);



			$status = $this->Member_model->saveMember($member_data);
			...
			if($status == true)
			{
					...
					$this->db->update('user_register_request',array('status'=>2,'updated'=>date('Y-m-d H:i:s')));
    ...
    {   
        ...
        // datatable column index  => database column name
            ...
            // getting total number records without any search
            $sql = "SELECT a.*, b.user_code as user_code, b.name as name FROM tbl_portal_kyc as a INNER JOIN tbl_users as b ON b.id = a.user_id  where   a.account_id = '$account_id'";
        ...
        if($get_filter_data){
            ...
            foreach($get_filter_data as $list){
                ...
                if($list['state'])
				{
					$get_state_name = $stateList = $this->db->get_where('states',array('id'=>$list['state']))->row_array();

					$state_name= $get_state_name['name'];
				}

                $nestedData[] = 'State -'.$state_name.'<br>'.'City -'.$list['city'].'<br>'.'Address -'.$list['address'].'<br>'.'Pincode -'.$list['pincode'];
                $nestedData[] = 'Aadhar Number -'.$list['aadhar_number'].'<br>'.'Pancard Number -'.$list['pancard_number'];
                $nestedData[] = 'Aadhar Front Image  - '."<a href=".base_url($list['aadhar_front_image']).">".''.'Download'."</a>".'<br>'.'Aadhar Back Image  - '."<a href=".base_url($list['aadhar_back_image']).">".''.'Download'."</a>".'<br>'.'Pancard Front Image  - '."<a href=".base_url($list['pancard_front_image']).">".''.'Download'."</a>";
                $nestedData[]='Bank Account Number - '.$list['bank_account_number'].'<br>'.'Acount Holder Name - '.$list['account_holder_name'].'<br>'.'IFSC Code - '.$list['ifsc_code'].'<br>'.'Bank Name - '.$list['bank_name'].'<br>'.'GST Number - '.$list['gst_number'];
                ...
                if($list['status'] == 1){
                    ...
                    $nestedData[] = '<a href="'.base_url('admin/member/approveRequest/'.$list['id']).'" onclick="return confirm(\'Are you sure you want to approve request?\')" title="Approve Request" class="btn btn-success btn-sm"><i class="fa fa-check" aria-hidden="true"></i></a>
                    <a href="'.base_url('admin/member/rejectRequest/'.$list['id']).'" title="Reject Request" onclick="return confirm(\'Are you sure you want to reject request?\')" class="btn btn-danger btn-sm"><i class="fa fa-times" aria-hidden="true"></i></a>';
                }   
                else{
    ...
    public function approveRequest($id = 0){

        $loggedUser = $this->User->getAdminLoggedUser(ADMIN_SESSION_ID);
        $account_id = $this->User->get_domain_account();
        $chk_request = $this->db->get_where('portal_kyc',array('account_id'=>$account_id,'id'=>$id))->row_array();
        

        if(!$chk_request){

            $this->Az->redirect('admin/member/portalKyc', 'system_message_error',lang('MEMBER_ERROR'));
        	...
        	$this->db->where('account_id',$account_id);
            $this->db->where('id',$id);
            $this->db->update('portal_kyc',array('status'=>2));

            $this->Az->redirect('admin/member/portalKyc', 'system_message_error',lang('REQUEST_APPROVE_SUCCESS'));

        }


        public function rejectRequest($id = 0){
        ...
        if(!$chk_request){
        	...
        	$this->db->where('account_id',$account_id);
            $this->db->where('id',$id);
            $this->db->update('portal_kyc',array('status'=>3));

            $this->Az->redirect('admin/member/portalKyc', 'system_message_error',lang('REQUEST_REJECT_SUCCESS'));

        }


        public function rejectMember($recharge_id = 0)
	{
		$account_id = $this->User->get_domain_account();
		// check member
		$chkMember = $this->db->get_where('user_register_request',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		{
			$this->Az->redirect('admin/member/memberRequestList', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}

		// check member
		$chkMember = $this->db->where_in('status',array(0))->get_where('user_register_request',array('id'=>$recharge_id,'account_id'=>$account_id))->num_rows();
		if(!$chkMember)
		{
			$this->Az->redirect('admin/member/memberRequestList', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! Member Already Reject.</div>');
		}

		// check recharge status
		$get_recharge_data = $this->db->get_where('user_register_request',array('id'=>$recharge_id,'account_id'=>$account_id))->row_array();
		

		$this->db->where('account_id',$account_id);
		$this->db->where('id',$recharge_id);
		$this->db->update('user_register_request',array('status'=>4,'updated'=>date('Y-m-d H:i:s')));

		$this->Az->redirect('admin/member/memberRequestList', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Member Rejected successfully.</div>');
	}
