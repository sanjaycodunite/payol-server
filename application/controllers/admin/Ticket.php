...
class Ticket extends CI_Controller {    
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] = '<a href="'.base_url('admin/ticket/viewTicket/'.$list['id']).'">'.$list['ticket_id'].'</a>';
	...
	public function viewTicket($ticket_id = 0){
		...
		if(!$chkMember)
		{
			$this->Az->redirect('admin/ticket/ticketList', 'system_message_error',lang('MEMBER_ERROR'));
		}
	...
	public function ticketResponseAuth()
	{
		...
		{	
			...
			if($_FILES['attachment']['name'])
			{
				...
				if($uploadError){
					$this->Az->redirect('admin/ticket/viewTicket/'.$ticket_id, 'system_message_error',$uploadError);
				}
				else
				{
			...
			$this->Az->redirect('admin/ticket/viewTicket/'.$ticket_id, 'system_message_error',lang('TICKET_REPLY_SAVED'));
