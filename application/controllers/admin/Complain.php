...
class Complain extends CI_Controller {    
    ...
    public function __construct() 
    {
        ...
        $this->lang->load('admin/ticket', 'english');
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				if($list['status'] == 1) {
					...
					$nestedData[] = '<a href="'.base_url('admin/complain/resolveComplain').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to resolve this complain?\')" class="btn btn-sm btn-success">Resolve</a> <a href="'.base_url('admin/complain/closeComplain').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to close this complain?\')" class="btn btn-sm btn-danger">Close</a>';
				}
	...
	public function resolveComplain($record_id = 0)
	{
		...
		if(!$chkMember)
		{
			...
			$this->Az->redirect('admin/complain', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized to access this page.</div>');
		}
		...
		$this->Az->redirect('admin/complain', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Complain resolved successfully.</div>');
	}
	...
	public function closeComplain($record_id = 0)
	{
		...
		// save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - Account - '.$account_id.' - IP - '.$user_ip_address.' - User('.$loggedUser['user_code'].') - Open Admin Close Complain Page - Complain ID - '.$record_id.' closed redirect back to complain page.]'.PHP_EOL;
        $this->User->generateAccountActivityLog($log_msg);

		$this->Az->redirect('admin/complain', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Complain closed successfully.</div>');
	}
