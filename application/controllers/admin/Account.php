...
class Account extends CI_Controller {   
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if($_FILES['profile']['name'])
			{
				...
				if($uploadError){
					...
					$this->Az->redirect('admin/account/addAccount', 'system_message_error',$uploadError);
				}
				else
				{
			...
			$this->Az->redirect('admin/account/accountList', 'system_message_error',lang('ACCOUNT_SAVE_SUCCESS'));
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] ='<a title="Invoice" class="btn btn-primary btn-sm" href="'.base_url('admin/account/invoiceSummery').'/'.$list['id'].'">Invoice</a>';
	...
	public function generateDynamicInvoiceAuth()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->generateDynamicInvoice();
				...
				{
					$this->Az->redirect('admin/account/dynamicInvoice', 'system_message_error',lang('ACCOUNT_INVOICE_FAILED'));
				}
...

				{
                    ...
                    $getTotalOpenMoneyPayoutAmount = $this->db->select('SUM(transfer_amount) as totalAmount,SUM(transfer_charge_amount) as totalChargeAmount')->get_where('open_money_payout',array('account_id'=>$domain_account_id,'user_id'=>$userID,'status'=>3,'MONTH(created) ='=>$month,'YEAR(created)'=>$year))->row_array();
                    $totalOpenMoneyPayoutAmount = isset($getTotalOpenMoneyPayoutAmount['totalAmount']) ? $getTotalOpenMoneyPayoutAmount['totalAmount'] : 0 ;
                    $totalOpenMoneyPayoutChargeAmount = isset($getTotalOpenMoneyPayoutAmount['totalChargeAmount']) ? $getTotalOpenMoneyPayoutAmount['totalChargeAmount'] : 0 ;


                    //open settlement

                     $getTotalOpenMoneySettlementAmount = $this->db->select('SUM(transfer_amount) as totalAmount,SUM(transfer_charge_amount) as totalChargeAmount')->get_where('settlement_open_money_payout',array('account_id'=>$domain_account_id,'user_id'=>$userID,'status'=>3,'MONTH(created) ='=>$month,'YEAR(created)'=>$year))->row_array();
                    $totalOpenMoneySettlementAmount = isset($getTotalOpenMoneySettlementAmount['totalAmount']) ? $getTotalOpenMoneySettlementAmount['totalAmount'] : 0 ;

                    $totalOpenMoneySettlementChargeAmount = isset($getTotalOpenMoneySettlementAmount['totalChargeAmount']) ? $getTotalOpenMoneySettlementAmount['totalChargeAmount'] : 0 ;
                    ...
                    $final_payout_charge_amount = $totalPayoutChargeAmount + $totalOpenPayoutChargeAmount + $totalOtherPayoutChargeAmount + $aadharPayCharge + $totalOpenMoneyPayoutChargeAmount + $totalOpenMoneySettlementChargeAmount;
                    ...
                    //upi collection
                     ...
                     $getTotalUpiAmount = $this->db->select('SUM(amount) as totalAmount,SUM(charge_amount) as totalChargeAmount')->get_where('upi_transaction',array('account_id'=>$domain_account_id,'member_id'=>$userID,'status'=>2,'MONTH(created) ='=>$month,'YEAR(created)'=>$year))->row_array();
                    $totalUpiAmount = isset($getTotalUpiAmount['totalAmount']) ? $getTotalUpiAmount['totalAmount'] : 0 ;

                    $totalUpiChargeAmount = isset($getTotalUpiAmount['totalChargeAmount']) ? $getTotalUpiAmount['totalChargeAmount'] : 0 ;
                    ...
                    $user_invoice = 'MPINV'.rand(1111,9999);
						...
						'upi_charge_amount'=>$totalUpiChargeAmount,
						'created' => date('Y-m-d H:i:s')
					);
				...
				$this->Az->redirect('admin/account/dynamicInvoice', 'system_message_error',lang('ACCOUNT_INVOICE_SUCCESS'));
	...
	public function invoiceSummery($invoice_id = '')
	{
		...
		{
			$this->Az->redirect('admin/account/dynamicInvoice', 'system_message_error',lang('INVOICE_ERROR'));
		}
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] = date('d-M-Y H:i:s',strtotime($list['created']));
				$nestedData[] ='<a title="Invoice" class="btn btn-primary btn-sm" href="'.base_url('admin/account/memberInvoice').'/'.$list['id'].'">Invoice</a>';
	...
	public function memberInvoice($id = '')
	{
		...
		$upi_commission = isset($invoice_data['upi_charge_amount']) ? $invoice_data['upi_charge_amount'] : 0 ;
		//$get_recharge_tax_amount = $recharge_amount - $recharge_commission;
		$upi_taxable_amount =$upi_commission/1.18;
		$upi_tax_amount = $upi_commission - $upi_taxable_amount;
		...
		$total_taxable_amount = $recharge_taxable_amount + $bbps_taxable_amount + $payout_taxable_amount + $aeps_taxable_amount + $upi_taxable_amount;

		$total_tax_amount = $recharge_tax_amount + $bbps_tax_amount + $payout_tax_amount + $aeps_tax_amount + $upi_tax_amount;
		...
		$data = array(
			...
			'upi_commission'=>$upi_commission,
			'upi_taxable_amount' =>$upi_taxable_amount,
			'upi_tax_amount' =>$upi_tax_amount,
			'system_message' => $this->Az->getSystemMessageError(),
			'system_info' => $this->Az->getsystemMessageInfo(),
			'system_warning' => $this->Az->getSystemMessageWarning(),
	...
	public function generateTdsInvoiceAuth()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->generateTdsInvoice();
				...
				{
					$this->Az->redirect('admin/account/tdsInvoice', 'system_message_error',lang('ACCOUNT_INVOICE_FAILED'));
				}
				...
				$this->Az->redirect('admin/account/tdsInvoice', 'system_message_error',lang('ACCOUNT_INVOICE_SUCCESS'));
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] = date('d-M-Y H:i:s',strtotime($list['created']));
				$nestedData[] ='<a title="Invoice" class="btn btn-primary btn-sm" href="'.base_url('admin/account/tdsInvoiceSummery').'/'.$list['id'].'">Invoice</a>';
	...
	public function tdsInvoiceSummery($invoice_id = '')
	{
		...
		{
			$this->Az->redirect('admin/account/tdsInvoice', 'system_message_error',lang('INVOICE_ERROR'));
		}
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] = $list['user_code'].'<br>'.$list['name'];						
				$nestedData[] = date('d-M-Y H:i:s',strtotime($list['created']));
				$nestedData[] ='<a title="Invoice" class="btn btn-primary btn-sm" href="'.base_url('admin/account/memberTdsInvoice').'/'.$list['id'].'">Invoice</a>';
		...
		public function downloadTds()
	{
        ...
        header("Refresh:0; url=".$siteUrl."admin/account/tdsInvoice");
	...
	public function generateManualInvoiceAuth()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->generateManualInvoice();
				...
				$this->Az->redirect('admin/account/manualInvoiceList', 'system_message_error',lang('ACCOUNT_INVOICE_SUCCESS'));
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] = date('d-M-Y H:i:s',strtotime($list['created']));
				$nestedData[] ='<a title="Invoice" class="btn btn-primary btn-sm" href="'.base_url('admin/account/manualInvoiceSummery').'/'.$list['id'].'">Invoice</a>';
	...
	public function manualInvoiceSummery($invoice_id = '')
	{
		...
		{
			$this->Az->redirect('admin/account/manualInvoiceList', 'system_message_error',lang('INVOICE_ERROR'));
		}
