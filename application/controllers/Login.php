...
class Login extends CI_Controller{
	...
	public function otpAuth(){
        ...
        else
        {
			...
			$chk_otp = $this->db->get_where('users_otp',array('account_id'=>$account_id,'otp_code'=>$otp_code,'status'=>0,'encrypt_otp_code'=>$encrypt_otp_code))->row_array();
				...
				{
					 $chk_otp = $this->db->get_where('users_otp',array('account_id'=>$account_id,'status'=>0,'encrypt_otp_code'=>$encrypt_otp_code))->row_array();
    ...
    public function resendOtpAuth($encode_opt_code = ''){
		...
		if(!$get_otp_data)
		{
			$this->Az->redirect('login', 'system_message_error', '<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! something went wrong.</div>');
		}
			...
			$this->Az->redirect('login/otp/'.$encode_opt_code, 'system_message_error', '<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Otp sent to your registered mobile number. Please Verify.</div>');
		// }
		// else{
