...
class FingpayAeps extends CI_Controller {    
    ...
    public function __construct() 
    {
        ...
        $this->load->model('distributor/FingpayAeps_model');		
	...
	public function index(){
		...
		if(!$user_2fa_aeps_status){
			...
			$this->Az->redirect('distributor/fingpayAeps/memberLogin', 'system_message_error',lang('AEPS_ACTIVE_ERROR'));
		}
...
// 			$this->Az->redirect('distributor/fingpayAeps/memberRegister', 'system_message_error',lang('AEPS_ACTIVE_ERROR'));
// 		}
	...
	public function activeAeps(){
		...
		if($chk_kyc)
		{
			...
			$this->Az->redirect('distributor/fingpayaeps', 'system_message_error',lang('AEPS_ACTIVE_SUCCESS'));
		}
	...
	public function activeAuth()
	{
		...
		{	
			...
			if(isset($_FILES['aadhar_photo']['name']) && $_FILES['aadhar_photo']['name']){
				...
				if($uploadError){
					$this->Az->redirect('distributor/fingpayAeps/activeAeps', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$uploadError.'</div>');
				}
				else
				{
				...
				else
				{
					...
					$pancard_photo = substr($config02['upload_path'] . $fileData['file_name'], 2);
			                    ...
			                    $statusCheckResponse = $this->FingpayAeps_model->checkAepsStatusLive($memberID);
									...
									{
										// get member address
										$getAddress = $this->db->select('address,pin_code')->order_by('id','desc')->get_where('fingpay_aeps_member_kyc',array('account_id'=>$account_id,'member_id'=>$memberID))->row_array();
										...
										// update aeps status
						                	...
						                	$this->db->update('fingpay_aeps_member_kyc',array('status'=>1));
                			...
                			if($status == 1)
                			{
                				...
                				$this->Az->redirect('distributor/fingpayAeps/otpVerify/'.$encodeFPTxnId, 'system_message_error',lang('AEPS_OTP_SUCCESS'));
                			}
                			else
                			{
                				$this->Az->redirect('distributor/fingpayAeps/activeAeps', 'system_message_error',sprintf(lang('AEPS_ACTIVE_FAILED'),$response['msg']));
                			}
	...
	public function otpVerify($encodeFPTxnId = ''){
		...
		if(!$chk_encode_id)
		{
			$this->Az->redirect('distributor/fingpayAeps/activeAeps', 'system_message_error',lang('AEPS_ENCODED_ID_ERROR'));
		}
	...
	public function otpAuth()
	{
		...
		{
			$this->Az->redirect('distributor/FingpayAeps/otpVerify/'.$encodeFPTxnId, 'system_message_error',lang('AEPS_OTP_ERROR'));
		}
		...
		if($status == 1)
		{
			$this->Az->redirect('distributor/FingpayAeps/capture/'.$encodeFPTxnId, 'system_message_error',lang('AEPS_OTP_VERIFY_SUCCESS'));
		}
		else
		{
			$this->Az->redirect('distributor/FingpayAeps/activeAeps', 'system_message_error',sprintf(lang('AEPS_ACTIVE_FAILED'),$response['msg']));
	...
	public function resendOtp($encodeFPTxnId = ''){
		...
		if($status == 1)
		{
			$this->Az->redirect('distributor/FingpayAeps/otpVerify/'.$encodeFPTxnId, 'system_message_error',lang('AEPS_OTP_RESEND_SUCCESS'));
		}
		else
		{
			$this->Az->redirect('distributor/FingpayAeps/activeAeps', 'system_message_error',sprintf(lang('AEPS_ACTIVE_FAILED'),$response['msg']));
								...
								$bmPIData   = simplexml_load_string($biometricData);
									...
									if($xmlarray['Resp']['@attributes']['errCode'] == 0)
									{
										...
										$captureData = json_decode(json_encode((array) $captureData), true);
								 		...
								 		//$member_pin = md5(123456);
                                         if($agentID == 'MPCND138206')
                                        {
                                            $member_pin = md5(123456);
                                        }
                                         elseif($agentID == 'MPCNR340897' ||  $agentID == 'MPCNR340897')
                                        {
                                            $member_pin = md5(123456);
                                        }
                                        elseif($agentID == 'MPCND506943')
                                        {
									...
									if($xmlarray['Resp']['@attributes']['errCode'] == 0)
									{
									 ...
									 if($agentID == 'MPCND138206')
                                        ...
                                        elseif($agentID == 'MPCND506943')
                                        {
                                         ...
                                         elseif($agentID == 'PAOLR243059' || $agentID == 'MPCNR340897')
                                        {
                                            $member_pin = md5(123456);
							...
							if($amount == 0)
							{
									...
									if($xmlarray['Resp']['@attributes']['errCode'] == 0)
									{
										...
										$captureData = json_decode(json_encode((array) $captureData), true);
                                         ...
                                         if($agentID == 'MPCNR498527')
                                        {
                                            $member_pin = '14e1b600b1fd579f47433b88e8d85291';
                                        }
                                        elseif($agentID == 'MPCND138206')
                                        {
                                            $member_pin = md5(123456);
                                        }
                                        
                                         elseif($agentID == 'MPCND467031')
                                        {
                                            $member_pin = '14e1b600b1fd579f47433b88e8d85291';
                                        }
                                        elseif($agentID == 'MPCND506943')
						        ...
						        if(isset($responseData['message']) && $responseData['message'] == 'successful')
						        {
						        	    ...
						        	    $this->db->where('id',$memberID);
						        	    $this->db->where('account_id',$account_id);
						        	    $this->db->update('users',array('fingpay_2fa_aeps_status'=>1,'fingpay_2fa_charge'=>1));
