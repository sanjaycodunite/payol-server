...
class Dmt extends CI_Controller {    
    ...
    public function __construct() 
    {
        ...
        $this->load->model('distributor/Dmt_model');		
        $this->lang->load('distributor/dmt', 'english');
	...
	public function registerAuth()
	{
		...
		{	
			...
			if($memberData)
			{
				...
				$this->Az->redirect('distributor/dmt/transferNow', 'system_message_error',lang('DMT_CUSTOMER_ALREADY_REGISTER_ERROR'));
			}
			...
			{
				$this->Az->redirect('distributor/dmt/activeOtp/'.$response['token'], 'system_message_error',lang('DMT_CUSTOMER_OTP_SENT'));
			}
			elseif($response['status'] == 2)
			{
				$this->Az->redirect('distributor/dmt/transferBen/'.$response['mobile'], 'system_message_error',lang('DMT_CUSTOMER_VERIFY_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('distributor/dmt/transferNow', 'system_message_error',sprintf(lang('DMT_CUSTOMER_REGISTER_ERROR'),$response['message']));
			}
		}
	}
	...
	public function activeOtp($token = '')
    {
		...
		if(!$chk_token)
		{
			$this->Az->redirect('distributor/dmt/transferNow', 'system_message_error',lang('DMT_TOKEN_ERROR'));
		}
    ...
    public function activeOtpAuth()
	{
		...
		{	
			...
			{
				$this->Az->redirect('distributor/dmt/activeOtp/'.$token, 'system_message_error',sprintf(lang('DMT_OTP_ERROR'),$response['message']));
			}
		}
	}
	...
	public function loginAuth()
	{
		...
		{	
			...
			if(!$memberData)
			{
				...
				if($response['status'] == 1)
				{
					$this->Az->redirect('distributor/dmt/transferBen/'.$post['mobile'], 'system_message_error','');
				}
				else
				{
					$this->Az->redirect('distributor/dmt/transferNow', 'system_message_error',lang('DMT_CUSTOMER_NOT_REGISTER_ERROR'));
				}
			}
	...
	public function transferBen($mobile = '')
    {
		...
		{
			$this->Az->redirect('distributor/dmt/transferNow', 'system_message_error',lang('WALLET_ERROR'));
		}
		...
		{
			$this->Az->redirect('distributor/dmt/updateDetail/'.$mobile, 'system_message_error',lang('UPDATE_DETAIL_ERROR'));
		}
    ...
    public function updatePersonalDetailAuth()
	{
		...
		{	
			...
			$this->Az->redirect('distributor/dmt/transferBen/'.$post['accountMobile'], 'system_message_error',lang('DMT_CUS_UPDATE_SUCCESS'));
    ...
    public function beneficiaryAuth()
	{
		...
		{	
			...
			if(isset($post['verfiy']))
			{
				...
				if(!$surcharge_amount)
				{
		            ...
		            if($admin_wallet_balance < $admin_surcharge_amount){
		              ...
		              $this->Az->redirect('distributor/dmt/transferBen/'.$mobile, 'system_message_error',lang('ADMIN_WALLET_BALANCE_ERROR'));
	            ...
	            if($before_balance < $final_amount){
	                ...
	                $this->Az->redirect('distributor/dmt/transferBen/'.$mobile, 'system_message_error',lang('WALLET_BALANCE_ERROR'));
	            }
			}
			...
			if($response['status'] == 1)
			{
				$this->Az->redirect('distributor/dmt/transferBen/'.$mobile, 'system_message_error',lang('DMT_BEN_VERIFY_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('distributor/dmt/transferBen/'.$mobile, 'system_message_error',sprintf(lang('DMT_BEN_REGISTER_ERROR'),$response['message']));
			}
		}
	}
	...
	public function deleteBen($benId = '',$mobile = '')
    {
		...
		if(!$chk_ben_id)
		{
			$this->Az->redirect('distributor/dmt/transferBen/'.$mobile, 'system_message_error',lang('DMT_BEN_ID_ERROR'));
		}

		$response = $this->Dmt_model->deleteBen($benId,$mobile);
		if($response['status'] == 1)
		{
			$this->Az->redirect('distributor/dmt/transferBen/'.$mobile, 'system_message_error',lang('DMT_BEN_DELETE_SUCCESS'));
		}
		else
		{
			$this->Az->redirect('distributor/dmt/transferBen/'.$mobile, 'system_message_error',sprintf(lang('DMT_BEN_DELETE_ERROR'),$response['message']));
		}
    ...
    public function moneyTransfer($benId = '')
    {
		...
		if($benId != '')
		{
			...
			if(!$chk_ben_id)
			{
				$this->Az->redirect('distributor/dmt/transferNow', 'system_message_error',lang('DMT_BEN_ID_ERROR'));
			}
	...
	public function transferAuth()
	{
		...
		{	
			...
			if(!$active_api_id)
			{
				$this->Az->redirect('distributor/dmt/moneyTransfer/'.$post['benId'], 'system_message_error',lang('DMT_API_ERROR'));
			}
	        ...
	        for($startLoop = 1; $startLoop <= $loopTime; $startLoop++)
	        {
				...
				if(!$surcharge_amount)
				{
		            ...
		            if($admin_wallet_balance < $admin_surcharge_amount){
		              ...
		              $this->Az->redirect('distributor/dmt/moneyTransfer/'.$post['benId'], 'system_message_error',lang('ADMIN_WALLET_BALANCE_ERROR'));
	            ...
	            if($before_balance < $final_amount){
	                ...
	                $this->Az->redirect('distributor/dmt/moneyTransfer/'.$post['benId'], 'system_message_error',lang('WALLET_BALANCE_ERROR'));
	            }
				...
				{
					$this->Az->redirect('distributor/dmt', 'system_message_error',sprintf(lang('MANUAL_TRANSFER_FAILED'),$response['message']));
				}
			...
			$this->Az->redirect('distributor/dmt', 'system_message_error',lang('MANUAL_TRANSFER_SUCCESS'));
