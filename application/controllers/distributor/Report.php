...
class Report extends CI_Controller {    
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] = "<a href=".base_url('distributor/report/bbpsLiveInvoice/').$list['recharge_display_id']." style='text-decoration:none;' target='_blank'>Invoice</a>";	
    ...
    public function complainAuth()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Az->redirect('distributor/report/bbpsHistory', 'system_message_error',lang('FORM_ERROR'));
        }
        else
        {   
            ...
            if(!$chk_recharge)
            {
                ...
                $this->Az->redirect('distributor/report/bbpsHistory', 'system_message_error',lang('AUTHORIZE_ERROR'));  
            ...
            if($status == true)
            {
                $this->Az->redirect('distributor/report/bbpsHistory', 'system_message_error',lang('COMPLAIN_SAVED'));
            }
            else
            {
                $this->Az->redirect('distributor/report/bbpsHistory', 'system_message_error',lang('COMMON_ERROR'));
            }
       ...
       $loggedUser = $this->User->getAdminLoggedUser(DISTRIBUTOR_SESSION_ID);
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] ='<a title="edit" class="btn btn-primary btn-sm" href="'.base_url('distributor/report/checkNsdlPanStatus').'/'.$list['id'].'">Check PAN Status</a>';
				$nestedData[] ='<a title="edit" class="btn btn-primary btn-sm" href="'.base_url('distributor/report/checkNsdlPanTranscationStatus').'/'.$list['id'].'">Check Transcation Status</a>';
	...
	public function checkNsdlPanStatus($ref_id = '') {
		...
		if(!$chk_ref_id){
			...
			$this->Az->redirect('distributor/report/nsdlPanReport', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! something went wrong.</div>');
		}
		...
		if(isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true){
			...
			$this->Az->redirect('distributor/report/nsdlPanReport', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$responseData['message'].'</div>');

		}
		else{

			
			$this->Az->redirect('distributor/report/nsdlPanReport', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$responseData['message'].'</div>');
		}
    ...
    public function checkNsdlPanTranscationStatus($ref_id = '') {
		...
		if(isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true){
			...
			$this->Az->redirect('distributor/report/nsdlPanReport', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transcation Status Check Succesfully.</div>');
