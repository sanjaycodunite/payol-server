...
class Newaeps extends CI_Controller {    
	...
	public function __construct() 
	{
		...
		$this->load->model('distributor/Newaeps_model');		
	...
	public function index(){
		...
		if(!$user_2fa_aeps_status){
			$this->Az->redirect('distributor/newaeps/memberRegistration', 'system_message_error',lang('AEPS_ACTIVE_ERROR'));
		}
		
		$user_2fa_aeps_loginn_status = $this->User->get_member_2fa_aeps_login_status($loggedUser['id']);

		if(!$user_2fa_aeps_loginn_status){
			$this->Az->redirect('distributor/newaeps/memberLogin', 'system_message_error',lang('AEPS_ACTIVE_ERROR'));
		}
	...
	public function activeAeps(){
...

		{
			...
			//call onboard status pipe wise
	            ...
	            elseif (isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true && $responseData['is_approved'] == 'Pending') {
	            	 ...
	            	 $this->Az->redirect('distributor/dashboard', 'system_message_error','<div class="alert alert-warning alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Onboarding complete,Please wait 6hr for AEPS Activation on FINO Payment Bank activation.</div>');
	            ...
	            {
	            	$this->Az->redirect('distributor/dashboard', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Onboarding Rejected by bank.</div>');
	            }
	            elseif (isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == false && $responseData['is_approved'] == 'Pending') {
	            	$this->Az->redirect('distributor/dashboard', 'system_message_error','<div class="alert alert-warning alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Onboarding Not Completed.</div>');
	            }
		...
		if($chk_kyc)
		{
			...
			$this->Az->redirect('distributor/newaeps', 'system_message_error',lang('AEPS_ACTIVE_SUCCESS'));
		}
	...
	public function activeAuth()
	{
		...
		{	
			...
			if(isset($_FILES['aadhar_photo']['name']) && $_FILES['aadhar_photo']['name']){
				...
				if($uploadError){
					$this->Az->redirect('distributor/newaeps/activeAeps', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$uploadError.'</div>');
				}
				else
				{
			...
			{	
				...
				if(!$chk_already){
					...
					$chk_wallet_balance = $this->db->get_where('users',array('account_id'=>$account_id,'id'=>$memberID))->row_array();
			...
			else
			{
				...
				$this->Az->redirect('distributor/newaeps/activeAeps', 'system_message_error',sprintf(lang('AEPS_ACTIVE_FAILED'),$response['msg']));
			}
	...
	public function benificaryAuth(){
		...
		{   
			...
			if(isset($responseData) && $responseData['response_code'] == 2 && $responseData['status'] == true){
				...
				$this->Az->redirect('distributor/newaeps/uploadDocument/'.$bene_id, 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Account Detailed saved successfully. Please upload Supportive Document to activate</div>');
			}
			elseif(isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true){
				...
				$this->Az->redirect('distributor/newaeps/payout', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Account Detailed saved successfully</div>');
			}
			else{

				$error = isset($responseData['message']) ? $responseData['message'] : 'Sorry!! something went wrong. Please try again.';

				$this->Az->redirect('distributor/newaeps/addBeneficiary', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$error.'</div>');
			}
		...
		{   
			...
			if(isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true){
				...
				if(isset($_FILES['passbook']['name']) && $_FILES['passbook']['name']){
					...
					$uploadError = $this->upload->display_errors();
					if($uploadError){
						$this->Az->redirect('distributor/newaeps/payout', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$uploadError.'</div>');
					}
					else
					{
						$fileData = $this->upload->data();
				...
				$this->Az->redirect('distributor/newaeps/payout', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Account Documet upload successfully and verified</div>');
			}
			else{

				$error = isset($responseData['message']) ? $responseData['message'] : 'Sorry!! something went wrong. Please try again.';

				$this->Az->redirect('distributor/newaeps/uploadDocument/'.$post['bene_id'], 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$error.'</div>');
			}
	...
	public function fundTransfer($bene_id = 0){
		...
		if(!$benificaryData){
			$this->Az->redirect('distributor/new/payout', 'system_message_error',lang('DB_ERROR'));
		}
...
*/
	public function fundTransferAuth() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if(!$benificaryData){
				$this->Az->redirect('distributor/newaeps/payout', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! beneficiary not valid.</div>');
			}
			...
			if($accountDetail['transaction_password'] != do_hash($post['txn_pass'])){
				...
				$this->Az->redirect('distributor/newaeps/fundTransfer/'.$post['bene_id'], 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! transaction password id wrong.</div>');		
			...
			if($wallet_balance < $total_wallet_deduct)
			{
				...
				$this->Az->redirect('distributor/newaeps/fundTransfer/'.$post['bene_id'], 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! Insufficient balance in your wallet.</div>');
			}
            ...
            curl_close ($curl);
             ...
             $this->Az->redirect('distributor/newaeps/transferOTP/'.$decode_otp_code, 'system_message_error',lang('PAYOUT_OTP_SENT'));	
			...
			if(isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true){
				...
				$this->Az->redirect('distributor/newaeps/transferReport', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Aeps payout successfully.</div>');
				...
				$this->Az->redirect('distributor/newaeps/fundTransfer/'.$post['bene_id'], 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$error.'</div>');
			}
...
// 			$this->Az->redirect('distributor/newaeps/transferReport', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! something went wrong.</div>');
// 		}
...
// 			$this->Az->redirect('distributor/newaeps/transferReport', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Status checked successfully.</div>');
...
// 			$this->Az->redirect('distributor/newaeps/transferReport', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$error.'</div>');
// 		}
...
// 	}
	...
	public function transferOTP($decode_otp_code = ''){
        ...
        if(!$chk_otp)
        {
            $this->Az->redirect('distributor/newaeps/fundTransfer', 'system_message_error',lang('DB_ERROR'));
        }
     ...
     public function updateTransferOTPAuth() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(!$chk_otp)
            {
                $this->Az->redirect('distributor/newaeps/transferOTP/'.$decode_otp_code, 'system_message_error',lang('OTP_ERROR'));
            }
			...
			if(isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true){
				...
				$this->Az->redirect('distributor/newaeps/fundTransfer/'.$get_beneficiaryData['id'], 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$error.'</div>');
			}
     ...
     public function checkAccountStatus($ref_id = '') {
	  ...
	  $loggedUser = $this->User->getAdminLoggedUser(DISTRIBUTOR_SESSION_ID);
		...
		if(isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true && $responseData['accountstatus'] == 1 ){
			...
			$this->db->update('new_payout_beneficiary',array('is_verified'=>1,'updated'=>date('Y-m-d H:i:s')));

			$this->Az->redirect('distributor/newaeps/payout', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Status checked successfully.</div>');
