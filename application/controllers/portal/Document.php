...
class Document extends CI_Controller {    
	...
	public function openCallbackAuth()
	{
		$post = $this->input->post();
		$call_back_url = isset($post['open_payout_call_back_url']) ? $post['open_payout_call_back_url'] : '';
		if($call_back_url)
		{
			$account_id = $this->User->get_domain_account();
	    	$loggedUser = $this->User->getAdminLoggedUser(API_MEMBER_SESSION_ID);
	    	...
	    	$this->db->where('id',$loggedAccountID);
	    	$this->db->where('account_id',$account_id);
	    	$this->db->update('users',array('open_payout_call_back_url'=>$call_back_url));
		}
		$this->Az->redirect('portal/document/OpenMoneyPayout', 'system_message_error',lang('CALLBACK_SAVE_SUCCESS'));
	}
	...
	public function pennyDrop(){
        ...
        $data = array(
            ...
            'meta_keywords' => lang('SITE_NAME'),
            'meta_description' => lang('SITE_NAME'),
            'content_block' => 'document/pennyDrop',           
            'manager_description' => lang('SITE_NAME'),
          	'system_message' => $this->Az->getSystemMessageError(),
            'system_info' => $this->Az->getSystemMessageInfo(),
            'system_warning' => $this->Az->getSystemMessageWarning() 
	...
	public function OpenMoneyPayout(){
		...
		// get call back url
        $get_call_back = $this->db->get_where('users',array('id'=>$loggedAccountID,'account_id'=>$account_id))->row_array();
        $open_payout_call_back_url = isset($get_call_back['open_payout_call_back_url']) ? $get_call_back['open_payout_call_back_url'] : '';
        ...
        $data = array(
            ...
            'meta_keywords' => lang('SITE_NAME'),
            'meta_description' => lang('SITE_NAME'),
            'content_block' => 'document/open-money-payout',  
            'open_payout_call_back_url'=>$open_payout_call_back_url,         
            'manager_description' => lang('SITE_NAME'),
          	'system_message' => $this->Az->getSystemMessageError(),
            'system_info' => $this->Az->getSystemMessageInfo(),
            'system_warning' => $this->Az->getSystemMessageWarning() 
