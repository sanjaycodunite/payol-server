...
class Ticket extends CI_Controller {    
    ...
    public function __construct() 
    {
        ...
        $this->load->model('portal/Ticket_model');		
        $this->lang->load('portal/ticket', 'english');
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData=array(); 
				$nestedData[] = $i;
				$nestedData[] = '<a href="'.base_url('portal/ticket/viewTicket/'.$list['id']).'">'.$list['ticket_id'].'</a>';
	...
	public function saveTicketAuth()
	{
		...
		{	
			...
			if($_FILES['attachment']['name'])
			{
				...
				if($uploadError){
					$this->Az->redirect('portal/ticket/create', 'system_message_error',$uploadError);
				}
				else
				{
			...
			$this->Az->redirect('portal/ticket/ticketList', 'system_message_error',lang('TICKET_SAVED'));
	...
	public function viewTicket($ticket_id = 0){
		...
		if(!$chkMember)
		{
			$this->Az->redirect('portal/ticket/ticketList', 'system_message_error',lang('MEMBER_ERROR'));
		}
	...
	public function ticketResponseAuth()
	{
		...
		{	
			...
			if($_FILES['attachment']['name'])
			{
				...
				$uploadError = $this->upload->display_errors();
				if($uploadError){
					$this->Az->redirect('portal/ticket/viewTicket/'.$ticket_id, 'system_message_error',$uploadError);
				}
				else
				{
					$fileData = $this->upload->data();
			...
			$this->Az->redirect('portal/ticket/viewTicket/'.$ticket_id, 'system_message_error',lang('TICKET_REPLY_SAVED'));
