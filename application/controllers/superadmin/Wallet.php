...
class Wallet extends CI_Controller {    
    ...
    public function __construct() 
    {
        ...
        $this->load->model('superadmin/Wallet_model');		
        $this->lang->load('superadmin/wallet', 'english');
	...
	{	
		...
		// datatable column index  => database column name
			...
			$sql = "SELECT a.*, b.user_code as user_code, b.name as name FROM tbl_member_wallet as a INNER JOIN tbl_users as b ON b.id = a.member_id  where b.role_id = 2 AND  a.wallet_type = 1";
	...
	{	
		...
		// datatable column index  => database column name
			...
			// getting total number records without any search
			$sql = "SELECT a.*, b.user_code as user_code, b.name as name FROM tbl_member_wallet as a INNER JOIN tbl_users as b ON b.id = a.member_id  where a.type = 1 AND b.role_id = 2 AND  a.wallet_type = 1";
	...
	{	
		...
		// datatable column index  => database column name
			...
			// getting total number records without any search
			$sql = "SELECT a.*, b.user_code as user_code, b.name as name FROM tbl_member_wallet as a INNER JOIN tbl_users as b ON b.id = a.member_id  where a.type = 2 AND b.role_id = 2 AND  a.wallet_type = 1";
	...
	public function saveWallet()
	{
		...
		{	
			...
			if(!$chk_member)
			{
				$this->Az->redirect('superadmin/wallet/walletList', 'system_message_error',lang('WALLET_ERROR'));
			}
			...
			if($status == true)
			{
				$this->Az->redirect('superadmin/wallet/walletList', 'system_message_error',lang('WALLET_SAVED'));
			}
			else
			{
				$this->Az->redirect('superadmin/wallet/walletList', 'system_message_error',lang('WALLET_ERROR'));
	...
	public function getMemberWalletBalance($memberID = 0)
	{
		echo json_encode(array(
			'status' => 1,
			'balance' => $this->User->getMemberWalletBalanceSP($memberID, $walletType = 1)
		));
	}
	...
	{	
		...
		// datatable column index  => database column name
			...
			// getting total number records without any search
			$sql = "SELECT a.* FROM tbl_member_fund_request as a INNER JOIN tbl_users as b ON b.id = a.member_id WHERE b.role_id = 2 AND a.request_wallet_type = 1";
			
			$totalData = $this->db->query($sql)->num_rows();
			$totalFiltered = $totalData;  // when there is no search parameter then total number rows = total number filtered rows.
		
		
			$sql = "SELECT a.*,b.name as member_name, b.user_code as member_code FROM tbl_member_fund_request as a INNER JOIN tbl_users as b ON b.id = a.member_id WHERE b.role_id = 2 AND a.request_wallet_type = 1";	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				if($list['status'] == 1)
				{
					...
					$nestedData[] ='<a title="Approve" class="btn btn-success btn-sm" href="'.base_url('superadmin/wallet/updateRequestAuth').'/'.$list['id'].'/1" onclick="return confirm(\'Are you sure you want to approve this request?\')"><i class="fa fa-check" aria-hidden="true"></i></a> <a title="Reject" class="btn btn-danger btn-sm" href="'.base_url('superadmin/wallet/updateRequestAuth').'/'.$list['id'].'/2" onclick="return confirm(\'Are you sure you want to reject this request?\')"><i class="fa fa-times" aria-hidden="true"></i></a>';
				}
	...
	public function updateRequestAuth($requestID = 0, $status = 0)
	{
		...
		if(!$chk_request_id)
		{
			$this->Az->redirect('superadmin/wallet/requestList', 'system_message_error',lang('WALLET_ERROR'));
		}
		
		$this->Wallet_model->updateRequestAuth($requestID,$status);
		if($status == 1){
			$this->Az->redirect('superadmin/wallet/requestList', 'system_message_error',lang('REQUEST_APPROVE_SUCCESS'));
		}
		else
		{
			$this->Az->redirect('superadmin/wallet/requestList', 'system_message_error',lang('REQUEST_REJECT_SUCCESS'));
		}
	}

	public function getBbspWalletBalance()
	{
		$bbps_api_before_balance = $this->User->get_bbps_api_balance(1);
		$user_wallet = array(
          'bbps_balance'=>$bbps_api_before_balance,        
      	);    
      	$this->db->where('id',1);
      	$this->db->update('users',$user_wallet);
      	echo json_encode(array('status'=>1,'balance'=>number_format($bbps_api_before_balance)));
	}
