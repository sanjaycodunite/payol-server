...
class Wallet extends CI_Controller {    
    ...
    public function requestAuth()
	{
		...
		else
		{
			...
			if($status == true)
			{
				$this->Az->redirect('retailer/wallet/myRequestList', 'system_message_error',lang('REQUEST_GENERATE_SUCCESS'));
			}
			else
			{
				$this->Az->redirect('retailer/wallet/myRequestList', 'system_message_error',lang('WALLET_ERROR'));
			}
	...
	public function paymentResponse()
	{
		...
		if (empty($_POST['razorpay_payment_id']) === false)
		{
			...
			catch(SignatureVerificationError $e)
			{
				...
				$this->Az->redirect('retailer/wallet/topup', 'system_message_error',$e->getMessage());
			}
		}
		...
		{	
            ...
            $this->Az->redirect('retailer/wallet/topup', 'system_message_error',lang('PAYMENT_SUCCESS'));
		...
		{	
			...
			$this->Az->redirect('retailer/wallet/topup', 'system_message_error',lang('PAYMENT_FAILED'));
		}
	...
	{	
		...
		{	
			...
			else
			{
				...
				$this->Az->redirect('retailer/wallet/dynamicQr', 'system_message_error',$error);
			}
	...
	public function activeQr()
    {
		...
		{
			$error = '<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$responseData['message'].'</div>';
			$this->Az->redirect('retailer/wallet/addFund', 'system_message_error',$error);
		}
    }
	...
	public function mapQr()
    {
		...
		// check qr is active or not
	 	...
	 	if($chk_qr_status)
		{
			$this->Az->redirect('retailer/wallet/addFund', 'system_message_error',lang('QR_ALREDY_ACTIVE_ERROR'));
		}
	...
	{	
		...
		{	
			...
			if(!$chk_txn_id)
			{
				$this->Az->redirect('retailer/wallet/mapQr', 'system_message_error',lang('MAP_TXN_ERROR'));
			}
			...
			if($chk_txn_id)
			{
				$this->Az->redirect('retailer/wallet/mapQr', 'system_message_error',lang('MAP_ALREADY_ERROR'));
			}
			...
			$this->Az->redirect('retailer/wallet/addFund', 'system_message_error',lang('MAP_SUCCESS'));
	...
	{	
		...
		{	
			...
			{
				$error = '<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$responseData['message'].'</div>';
				$this->Az->redirect('retailer/wallet/dynamicCashQr', 'system_message_error',$error);
			}
	...
	public function activeCashQr()
    {
		...
		{
			$error = '<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$responseData['message'].'</div>';
			$this->Az->redirect('retailer/wallet/addCashFund', 'system_message_error',$error);
		}
    }
	...
	public function mapCashQr()
    {
		...
		// check qr is active or not
	 	...
	 	if($chk_qr_status)
		{
			$this->Az->redirect('retailer/wallet/addCashFund', 'system_message_error',lang('QR_ALREDY_ACTIVE_ERROR'));
		}
	...
	{	
		...
		{	
			...
			if(!$chk_txn_id)
			{
				$this->Az->redirect('retailer/wallet/mapCashQr', 'system_message_error',lang('MAP_TXN_ERROR'));
			}
			...
			if($chk_txn_id)
			{
				$this->Az->redirect('retailer/wallet/mapCashQr', 'system_message_error',lang('MAP_ALREADY_ERROR'));
			}
			...
			$this->Az->redirect('retailer/wallet/addCashFund', 'system_message_error',lang('MAP_SUCCESS'));
    ...
    public function payolWalletAuth()
	{
		...
		{	
			if($post['amount'] < 1)
			{
				$this->Az->redirect('retailer/wallet/payolTransfer', 'system_message_error',lang('AMOUNT_VALID_ERROR'));
			}
			...
			if(!$chk_member)
			{
				$this->Az->redirect('retailer/wallet/payolTransfer', 'system_message_error',lang('MEMBER_VALID_ERROR'));
			}
			...
			{
				$this->Az->redirect('retailer/wallet/payolTransfer', 'system_message_error',lang('WALLET_BALANCE_ERROR'));
			}
			...
			{
				$this->Az->redirect('retailer/wallet/payolTransfer', 'system_message_error',lang('SELF_TRANSFER_ERROR'));
			}
			...
			$this->Az->redirect('retailer/wallet/payolTransferVerify/'.$encode_txnid, 'system_message_error','');
	...
	public function payolTransferVerify($encode_txnid = '')
    {
		...
		if(!$chk_member)
		{
			$this->Az->redirect('retailer/wallet/payolTransfer', 'system_message_error',lang('WALLET_ERROR'));
		}
    ...
    public function payolWalletPinAuth()
	{
		...
		{	
			...
			if(!$chk_member)
			{
				$this->Az->redirect('retailer/wallet/payolTransferVerify/'.$encode_txnid, 'system_message_error',lang('TXN_PIN_ERROR'));
			}
			...
			$this->Az->redirect('retailer/wallet/payolTransfer', 'system_message_error',lang('PAYOL_TRANSFER_SUCCESS'));
    ...
    {    
		...
		if(!$chk_va_status)
		{
		    $this->Az->redirect('retailer/wallet/activeVirtualAccount', 'system_message_error');
		}
    ...
    public function virtualAccountAuth()
	{
        ...
        $virtual_account_no = $van_prefix.$loggedUser['mobile'];
		 $data = array(
		  ...
		  $this->Az->redirect('retailer/wallet/virtualAccount', 'system_message_error',$msg);
