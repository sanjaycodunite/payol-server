...
class Settlement extends CI_Controller
{
        ...
        if ($get_filter_data) {
            ...
            foreach ($get_filter_data as $list) {
                ...
                if ($list['transaction_id']) {
                    $nestedData[] = '<a href="' . base_url('retailer/settlement/transferInvoice/' . $list['id'] . '') . '" target="_blank">' . $list['transaction_id'] . '</a>';
                } else {
                    $nestedData[] = 'Not Available';
                }
    ...
    public function fundTransfer($bene_id = 0, $sender_id = 0)
    {
        ...
        if ($bene_id && !$chk_beneficiary) {
            $this->Az->redirect('retailer/settlement/beneficiaryList', 'system_message_error', lang('DB_ERROR'));
        }
    ...
    public function fundTransferAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($responseData['error'])) {
                ...
                $this->Az->redirect('retailer/settlement/beneficiaryList', 'system_message_error', lang('MANUAL_TRANSFER_FAILED', $api_msg));
            } else {
                $this->Az->redirect('retailer/settlement/beneficiaryList', 'system_message_error', sprintf(lang('MANUAL_TRANSFER_SUCCESS')));
            }
        }
    }
    ...
    public function newPayoutTransferAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($before_balance < $final_amount) {
                ...
                $log_msg = '[' . date('d-m-Y H:i:s') . ' - MD(' . $loggedUser['user_code'] . ') - Insufficient Wallet Error]' . PHP_EOL;
                $this->User->generateLog($log_msg);
                $this->Az->redirect('retailer/settlement', 'system_message_error', lang('WALLET_BALANCE_ERROR'));
            }
            ...
            if ($before_balance < $final_deduct_wallet_balance) {
                ...
                $this->Az->redirect('retailer/settlement', 'system_message_error', lang('MIN_WALLET_ERROR'));
            }
            ...
            if (isset($responseData['error'])) {
                ...
                $this->Az->redirect('retailer/settlement', 'system_message_error', lang('MANUAL_TRANSFER_FAILED', $api_msg));
            } else {
                $this->Az->redirect('retailer/settlement', 'system_message_error', lang('MANUAL_TRANSFER_SUCCESS'));
            }
        }
    }
    ...
    public function upiPayoutFundTransfer($bene_id = 0)
    {
        ...
        if (!$chk_beneficiary) {
            $this->Az->redirect('retailer/settlement/upiPayoutBeneficiaryList', 'system_message_error', lang('DB_ERROR'));
        }
    ...
    public function upiOpenPayoutBeneficiaryList()
    {
        ...
        $benificaryList = $this->db->get_where('settlement_user_vpa_benificary', ['account_id' => $account_id, 'user_id' => $loggedAccountID,'is_delete'=>0])->result_array();
        // get bank list
        $bankList = $this->db->get('aeps_bank_list')->result_array();
    ...
    public function upiOpenPayoutBenificaryAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            //$chkBeneficiary = $this->db->get_where('settlement_user_vpa_benificary',array('user_id'=>$loggedAccountID,'account_id'=>$account_id))->num_rows();

            if ($chkTxnId) {
                $this->Az->redirect('retailer/settlement/upiOpenPayoutBeneficiaryList', 'system_message_error', lang('TXN_ALREADY_ERROR'));
            }
            //   elseif($chkBeneficiary)
            //   {
            //   	$this->Az->redirect('retailer/settlement/upiOpenPayoutBeneficiaryList', 'system_message_error',lang('BENEFICIARY_ALREADY_ERROR'));
            //   }
            else {
                $api_url = OPEN_MONEY_CREATE_BENEFICIARY_URL;
                // save system log
                ...
                if (isset($responseData) && $responseData['status'] == "active") {
                    ...
                    $this->Az->redirect('retailer/settlement/upiOpenPayoutBeneficiaryList', 'system_message_error', lang('BENEFICIARY_SAVE_SUCCESS'));
                } else {
                    // save system log
                    $log_msg = '[' . date('d-m-Y H:i:s') . ' - Add Beneficiary Api Error.]' . PHP_EOL;
                    $this->User->generateLog($log_msg);

                    $this->Az->redirect('retailer/settlement/upiOpenPayoutBeneficiaryList', 'system_message_error', lang('WALLET_ERROR'));
                }
            }
    ...
    public function upiOpenPayoutFundTransfer($bene_id = 0)
    {
        ...
        if (!$chk_beneficiary) {
            $this->Az->redirect('retailer/settlement/upiOpenPayoutBeneficiaryList', 'system_message_error', lang('DB_ERROR'));
        }
    ...
    public function upiOpenPayoutTransferAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($before_balance < $final_amount) {
                ...
                $log_msg = '[' . date('d-m-Y H:i:s') . ' - MD(' . $loggedUser['user_code'] . ') - Insufficient Wallet Error]' . PHP_EOL;
                $this->User->generateLog($log_msg);
                $this->Az->redirect('retailer/settlement/upiOpenPayoutBeneficiaryList', 'system_message_error', lang('WALLET_BALANCE_ERROR'));
            }
            ...
            if ($before_balance < $final_deduct_wallet_balance) {
                ...
                $this->Az->redirect('retailer/settlement/upiOpenPayoutBeneficiaryList', 'system_message_error', lang('MIN_WALLET_ERROR'));
            }
            ...
            if (isset($responseData['error'])) {
                ...
                $this->Az->redirect('retailer/settlement/upiOpenPayoutBeneficiaryList', 'system_message_error', lang('MANUAL_TRANSFER_SUCCESS'));
            }
        }
    }
            ...
            if ($beneficiary_data) {
                ...
                foreach ($beneficiary_data as $key => $list) {
                    ...
                    $str .=
                        ...
                        base_url() .
                        'retailer/settlement/newPayoutFundTransfer/' .
                        $list['ben_id'] .
                        '"> Transfer</a>&nbsp;&nbsp;<a href="' .
                        base_url('retailer/settlement/deleteBeneficiaryAccount') .
                        '/' .
                        $list['id'] .
            ...
            if ($beneficiary_data) {
                ...
                foreach ($beneficiary_data as $key => $list) {
                    ...
                    $str .=
                        '<td><a title="Fund Transfer" class="btn btn-primary btn-sm" href="' .
                        base_url() .
                        'retailer/settlement/upiPayoutFundTransfer/' .
                        $list['ben_id'] .
                        '"> Transfer</a>&nbsp;&nbsp;<a href="' .
                        base_url('retailer/settlement/deleteBeneficiaryUpiAccount') .
                        '/' .
                        $list['id'] .
                        '" onclick="return confirm(\'Are you sure you want to delete this Account?\')" class="btn btn-sm btn-danger">Delete</a></td>';
    ...
    public function saveBenificaryBankAccount()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($post['type'] == 1) {
                ...
                $chkBeneficiary = $this->db->get_where('open_money_payout_user_benificary', ['user_id' => $loggedAccountID, 'account_id' => $account_id, 'is_active' => 1, 'is_delete' => 0])->num_rows();

                if ($chkTxnId) {
                    $this->Az->redirect('retailer/settlement/addBankAccount', 'system_message_error', lang('TXN_ALREADY_ERROR'));
                } elseif ($chkBeneficiary == 5) {
                    $this->Az->redirect('retailer/settlement/addBankAccount', 'system_message_error', lang('BENEFICIARY_ALREADY_ERROR'));
                } else {
                    $api_url = OPEN_MONEY_CREATE_BENEFICIARY_URL;
                    // save system log
                    $log_msg = '[' . date('d-m-Y H:i:s') . ' - RT(' . $loggedUser['user_code'] . ') - Add Beneficiary API URL - ' . $api_url . ']' . PHP_EOL;
                    ...
                    if (isset($responseData) && $responseData['status'] == "active") {
                        ...
                        $this->Az->redirect('retailer/settlement/addBankAccount', 'system_message_error', lang('BENEFICIARY_SAVE_SUCCESS'));
                    } else {
                        // save system log
                        $log_msg = '[' . date('d-m-Y H:i:s') . ' - Add Beneficiary Api Error.]' . PHP_EOL;
                        $this->User->generateLog($log_msg);

                        $this->Az->redirect('retailer/settlement/addBankAccount', 'system_message_error', lang('WALLET_ERROR'));
                    }
                }
            } else {
                //vpa beneficiary api
