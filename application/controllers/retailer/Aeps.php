...
class Aeps extends CI_Controller {    
    ...
    public function __construct() 
    {
        ...
        $this->load->model('retailer/Aeps_model');		
	...
	public function activeAeps(){
		...
		if($chk_kyc)
		{
			...
			$this->Az->redirect('retailer/aeps', 'system_message_error',lang('AEPS_ACTIVE_SUCCESS'));
		}
	...
	public function activeAuth()
	{
		...
		{	
			...
			if(isset($_FILES['aadhar_photo']['name']) && $_FILES['aadhar_photo']['name']){
				...
				if($uploadError){
					$this->Az->redirect('retailer/aeps/activeAeps', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$uploadError.'</div>');
				}
				else
				{
			...
			if($status == 1)
			{
				...
				$this->Az->redirect('retailer/aeps/otpVerify/'.$encodeFPTxnId, 'system_message_error',lang('AEPS_OTP_SUCCESS'));
			}
			else
			{
	...
	public function otpAuth()
	{
		...
		{
			$this->Az->redirect('retailer/aeps/otpVerify/'.$encodeFPTxnId, 'system_message_error',lang('AEPS_OTP_ERROR'));
		}
		...
		if($status == 1)
		{
			$this->Az->redirect('retailer/aeps/capture/'.$encodeFPTxnId, 'system_message_error',lang('AEPS_OTP_VERIFY_SUCCESS'));
		}
		else
		{
			$this->Az->redirect('retailer/aeps/activeAeps', 'system_message_error',sprintf(lang('AEPS_ACTIVE_FAILED'),$response['msg']));
