...
class Report extends CI_Controller {    
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				if($list['invoice_no']){
				 ...
				 $nestedData[] = '<a href="'.base_url('retailer/report/transferInvoice/'.$list['id'].'').'" target="_blank">'.$list['invoice_no'].'</a>';
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] = "<a href=".base_url('retailer/report/bbpsLiveInvoice/').$list['recharge_display_id']." style='text-decoration:none;' target='_blank'>Invoice</a>";	
    ...
    public function complainAuth()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Az->redirect('retailer/report/bbpsHistory', 'system_message_error',lang('FORM_ERROR'));
        }
        else
        {   
            ...
            if(!$chk_recharge)
            {
                ...
                $this->Az->redirect('retailer/report/bbpsHistory', 'system_message_error',lang('AUTHORIZE_ERROR'));  
            ...
            if($status == true)
            {
                $this->Az->redirect('retailer/report/bbpsHistory', 'system_message_error',lang('COMPLAIN_SAVED'));
            }
            else
            {
                $this->Az->redirect('retailer/report/bbpsHistory', 'system_message_error',lang('COMMON_ERROR'));
            }
	...
	{	
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				$nestedData[] ='<a title="edit" class="btn btn-primary btn-sm" href="'.base_url('retailer/report/checkNsdlPanStatus').'/'.$list['id'].'">Check PAN Status</a>';
				$nestedData[] ='<a title="edit" class="btn btn-primary btn-sm" href="'.base_url('retailer/report/checkNsdlPanTranscationStatus').'/'.$list['id'].'">Check Transcation Status</a>';
	...
	public function checkNsdlPanStatus($ref_id = '') {
		...
		if(!$chk_ref_id){
			...
			$this->Az->redirect('retailer/report/nsdlPanReport', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! something went wrong.</div>');
		}
		...
		if(isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true){
			...
			$this->Az->redirect('retailer/report/nsdlPanReport', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$responseData['message'].'</div>');

		}
		else{

			
			$this->Az->redirect('retailer/report/nsdlPanReport', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$responseData['message'].'</div>');
		}
    ...
    public function checkNsdlPanTranscationStatus($ref_id = '') {
		...
		if(isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true){
			...
			$utr_no = $requestData['data']['utr_no'];
			$ack_no = $requestData['data']['ack_no'];
			$status = '';
			$added_date = $requestData['data']['addeddate'];
...

			{
				...
				$this->db->update('member_nsdl_transcation',array('status'=>$status,'ackno'=>$ack_no,'creation_date'=>$added_date,'updated'=>date('Y-m-d H:i:s')));

			$this->Az->redirect('retailer/report/nsdlPanReport', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transcation Status Check Succesfully.</div>');
	...
	public function nsdlPanCardReport(){
		...
		$data = array(
            ...
            'system_info' => $this->Az->getsystemMessageInfo(),
            'system_warning' => $this->Az->getSystemMessageWarning(),
            'content_block' => 'report/nsdl-pan-card-report'
        );
        $this->parser->parse('retailer/layout/column-1' , $data);
	...
	{	
		...
		// datatable column index  => database column name
			...
			// getting total number records without any search
			$sql = "SELECT a.*, b.user_code as user_code, b.name as name FROM tbl_morningpay_pancard_history as a INNER JOIN tbl_users as b ON b.id = a.user_id where a.user_id = '$loggedAccountID' AND a.account_id = '$account_id'";
		...
		if($get_filter_data){
			...
			foreach($get_filter_data as $list){
				...
				// $nestedData[] ='<a title="Check Status" class="btn btn-primary btn-sm" href="'.base_url('retailer/report/checkNsdlPanCardStatus').'/'.$list['txn_id'].'">Check Transcation Status</a>';
				// $nestedData[] ='<a title="Check Status" class="btn btn-primary btn-sm" href="'.base_url('retailer/report/checkPanCardStatus').'/'.$list['txn_id'].'">Check Pan Status</a>';
	...
	//morningpay nsdl pan status

	public function checkNsdlPanCardStatus($ref_id = '') {
		...
		$chk_ref_id = $this->db->get_where('morningpay_pancard_history',array('account_id'=>$account_id,'user_id'=>$memberID,'txn_id'=>$ref_id))->row_array();


		$txn_id = isset($chk_ref_id['txn_id']) ? $chk_ref_id['txn_id'] : 0 ;


		if(!$chk_ref_id){

			$this->Az->redirect('retailer/report/nsdlPanCardReport', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! something went wrong.</div>');
		}
		
		$httpUrl = MORNINGPAY_PAN_TRANSCATION_STATUS;

		log_message('debug', 'Morningpay NSDL PAN  check status api called.'.'Api Url - '.$api_url);

		$data =array(

    		'Token' =>MORNINGPAY_NSDL_API_TOKEN,
    		'PancardrefId' =>$txn_id

    	);


		log_message('debug', ' Morningpay NSDL PAN  check status  api post request data - '.json_encode($data));
		...
		log_message('debug', 'Morningpay PAN Transction STATUS check  status api final response - '.$output);
		...
		$this->db->insert('morningpay_nsdl_api_response',$api_data);

		
		if(isset($responseData) && $responseData['StatusCode'] == 1 && $responseData['Message'] == 'Success'){

			$pan_status = $responseData['Transactions']['Status'];
			$mobile = $responseData['Transactions']['Number'];
        	$amount = $responseData['Transactions']['Amount'];
        	$ackno = $responseData['Transactions']['AckNo'];
        	$order_id = $responseData['Transactions']['OrderID'];
        	$txn_date = $responseData['Transactions']['TxnDate'];        	
        	$type = $responseData['Transactions']['Type'];
			...
			//update record in pan transcation

			$this->db->where('account_id',$account_id);
			$this->db->where('user_id',$memberID);
			$this->db->where('txn_id',$txn_id);
			$this->db->update('morningpay_pancard_history',array('pan_status'=>$status,'ackno'=>$ackno,'updated'=>date('Y-m-d H:i:s')));

			$this->Az->redirect('retailer/report/nsdlPanCardReport', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Transcation Status Check Succesfully.</div>');
	...
	public function checkPanCardStatus($ref_id = '') {
		...
		$httpUrl = MORNINGPAY_PAN_STATUS;

		log_message('debug', 'Morningpay NSDL PAN Status check status api called.'.'Api Url - '.$api_url);
		...
		log_message('debug', ' Morningpay NSDL PAN  Status check status  api post request data - '.json_encode($data));
		...
		log_message('debug', 'Morningpay PAN  STATUS check  status api final response - '.$output);
		...
		if(isset($responseData) && $responseData['StatusCode'] == 1 && $responseData['Message'] == 'Success'){
			...
			$this->db->where('user_id',$memberID);
			$this->db->where('txn_id',$txn_id);
			$this->db->update('morningpay_pancard_history',array('pan_no'=>$pan_number,'ackno'=>$ackno,'updated'=>date('Y-m-d H:i:s')));

			$this->Az->redirect('retailer/report/nsdlPanCardReport', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Pan Status Check Succesfully.</div>');
	...
	{	
		...
		// datatable column index  => database column name
			...
			// getting total number records without any search
			$sql = "SELECT a.*, b.user_code as user_code, b.name as name FROM tbl_upi_dynamic_qr as a LEFT JOIN tbl_users as b ON b.id = a.member_id  where a.id > 0 AND a.account_id = '$account_id' AND a.member_id = '$loggedAccountID'";
			if($fromDate && $toDate)
            {
                $sql.=" AND DATE(a.created) >= '".date('Y-m-d',strtotime($fromDate))."' AND DATE(a.created) <= '".date('Y-m-d',strtotime($toDate))."'";
            }
			...
			$sql_summery = "SELECT SUM(COALESCE(CASE WHEN a.status = 1 THEN a.amount END,0)) totalSuccessAmount,count( case when a.status=1 then 1 else NULL end) totalSuccessRecord FROM tbl_upi_dynamic_qr as a INNER JOIN tbl_users as b ON b.id = a.member_id WHERE a.account_id = '$account_id' AND a.member_id = '$loggedAccountID'";
			if($fromDate && $toDate)
            {
                $sql_summery.=" AND DATE(a.created) >= '".date('Y-m-d',strtotime($fromDate))."' AND DATE(a.created) <= '".date('Y-m-d',strtotime($toDate))."'";
            }
