...
class Recharge extends CI_Controller{
    ...
    public function __construct() {
        ...
        $this->load->model('retailer/Master_model');      
    ...
    public function mobilePrepaid(){
        ...
        $recent_recharge_list = $this->db->order_by('created','DESC')->limit(10)->get_where('recharge_history',array('recharge_type'=>1))->result_array();



         $member_package_id = $this->User->getMemberPackageID($loggedUser['id']);
        ...
        $data = array(
            ...
            'circle'         => $circle,
            'recent_recharge_list'=>$recent_recharge_list,
    ...
    {   
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Recharge Post Data - '.json_encode($post).']'.PHP_EOL;
        ...
        $this->form_validation->set_rules('txn_pass', 'Transaction Pin', 'required');
        ...
        {   
            ...
            // save system log
            $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Wallet Balance - '.$chk_wallet_balance['wallet_balance'].']'.PHP_EOL;
            $this->User->generateLog($log_msg);

            
            $user_before_balance = $this->User->getMemberWalletBalanceSP($loggedAccountID);
            $check_txn_pass = $this->User->CheckTranscationPin($loggedAccountID,$post['txn_pass']);

            if($check_txn_pass != 1){
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Transction Password  Error]'.PHP_EOL;
                $this->User->generateLog($log_msg);
                
                $this->Az->redirect('retailer/recharge/mobilePrepaid', 'system_message_error',lang('TXN_PASS_ERROR'));
            ...
            elseif($user_before_balance < $post['amount']){
                ...
                $this->Az->redirect('retailer/recharge/mobilePrepaid', 'system_message_error',lang('WALLET_ERROR'));
           ...
           /* $is_cogent_instantpay_api = $this->User->get_admin_instant_cogent_api($account_id);
            if($is_cogent_instantpay_api)
            {
                ...
                if($admin_wallet_balance < $post['amount']){
                    // save system log
                    $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Insufficient Virtual Wallet Error]'.PHP_EOL;
                    $this->User->generateLog($log_msg);
                    
                    $this->Az->redirect('retailer/recharge/mobilePrepaid', 'system_message_error',lang('ADMIN_WALLET_BALANCE_ERROR'));
            ...
            if($user_before_balance < $final_deduct_wallet_balance){
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Minimum Wallet Error]'.PHP_EOL;
                $this->User->generateLog($log_msg);
                
                $this->Az->redirect('retailer/recharge/mobilePrepaid', 'system_message_error',lang('MIN_WALLET_ERROR'));
            ...
            // save system log
            $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Rechage API Generate Response - '.json_encode($response).']'.PHP_EOL;
            $this->User->generateLog($log_msg);
            if($response['status'] && $response['api_id'])
            {
                ...
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Rechage API Operator Code - '.$opt_code.']'.PHP_EOL;
                $this->User->generateLog($log_msg);
                ...
                $circle_code = 19;
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Rechage API Circle Code - '.$circle_code.']'.PHP_EOL;
                $this->User->generateLog($log_msg);

                // generate recharge unique id
                $recharge_unique_id = rand(1111,9999).time();
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Rechage Unique ID - '.$recharge_unique_id.']'.PHP_EOL;
                $this->User->generateLog($log_msg);
                ...
                if($response['is_instantpay_api'])
                {
                    ...
                    {
                        $this->Az->redirect('retailer/recharge/mobilePrepaid', 'system_message_error',sprintf(lang('RECHARGE_FAILED'),'Sorry ! Your eKyc not approved yet, please submit your eKyc.'));
                    }
                }
                ...
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Save Rechage Data System Recharge ID - '.$recharge_id.']'.PHP_EOL;
                $this->User->generateLog($log_msg);
                ...
                if($response['is_instantpay_api'])
                {
                    ...
                    {
                        // call recharge API
                        $api_response = $this->User->instantpay_rechage_api($opt_code,$loggedUser['id'],$recharge_unique_id,$post['mobile'],$post['amount'],$response['api_id'],$loggedUser['user_code'],'R');
                    }
                    else
                    {
                        $api_response = array(
                ...
                else
                {
                    ...
                    // save system log
                    $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Final API URL - '.$api_url.' - Post Data - '.json_encode($api_post_data).']'.PHP_EOL;
                    $this->User->generateLog($log_msg);

                    // call recharge API
                    $api_response = $this->User->prepaid_rechage_api($api_url,$api_post_data,$loggedUser['id'],$recharge_unique_id,$response['api_id'],$response['response_type'],$response['responsePara'],$response['seperator'],$response['header_data'],$loggedUser['user_code'],'R');
                }

                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - API Final Response - '.json_encode($api_response).']'.PHP_EOL;
                $this->User->generateLog($log_msg);
                ...
                if(isset($api_response['status']) && ($api_response['status'] == 1 || $api_response['status'] == 2))
                {
                    ...
                    if($api_response['status'] == 1){
                        ...
                        $this->Az->redirect('retailer/recharge/mobilePrepaid', 'system_message_error',lang('RECHARGE_PENDING'));
                    ...
                    elseif($api_response['status'] == 2)
                    {
                        ...
                        // save system log
                        $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Distribute Commision Start]'.PHP_EOL;
                        $this->User->generateLog($log_msg);
                        // distribute commision
                        $this->User->distribute_recharge_commision($recharge_id,$recharge_unique_id,$post['amount'],$loggedUser['id']);
                        // save system log
                        $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Distribute Commision End]'.PHP_EOL;
                        $this->User->generateLog($log_msg);

                        
                        
                        $this->Az->redirect('retailer/recharge/mobilePrepaid', 'system_message_error',lang('RECHARGE_SUCCESS'));
                ...
                {
                    // save system log
                    $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Recharge Failed]'.PHP_EOL;
                    $this->User->generateLog($log_msg);
                    // update recharge status
                    $this->db->where('id',$recharge_id);
                    $this->db->where('recharge_display_id',$recharge_unique_id);
                    ...
                    $this->Az->redirect('retailer/recharge/mobilePrepaid', 'system_message_error',sprintf(lang('RECHARGE_FAILED'),$api_response['opt_msg']));
            ...
            else
            {
                ...
                $this->Az->redirect('retailer/recharge/mobilePrepaid', 'system_message_error',lang('API_ERROR'));
    ...
    {   
        ...
        $post = $this->input->post();
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - DTH Recharge Post Data - '.json_encode($post).']'.PHP_EOL;
        $this->User->generateLog($log_msg);
        $this->load->library('form_validation');
        $this->form_validation->set_rules('operator', 'Operator', 'required');
        ...
        {   
            ...
            $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Wallet Balance - '.$user_before_balance.']'.PHP_EOL;
            $this->User->generateLog($log_msg);
            ...
            if($check_txn_pass != 1){
                ...
                $this->Az->redirect('retailer/recharge/dth', 'system_message_error',lang('TXN_PASS_ERROR'));
            ...
            elseif($user_before_balance < $post['amount']){
                ...
                $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Insufficient Wallet Error]'.PHP_EOL;
                $this->User->generateLog($log_msg);
                $this->Az->redirect('retailer/recharge/dth', 'system_message_error',lang('WALLET_ERROR'));   
            ...
            if($user_before_balance < $final_deduct_wallet_balance){
                ...
                $this->Az->redirect('retailer/recharge/dth', 'system_message_error',lang('MIN_WALLET_ERROR'));
            ...
            if($is_cogent_instantpay_api)
            {
                ...
                if($admin_wallet_balance < $post['amount']){
                    ...
                    $this->Az->redirect('retailer/recharge/dth', 'system_message_error',lang('ADMIN_WALLET_BALANCE_ERROR'));
            ...
            if($response['status'] && $response['api_id'])
            {
                ...
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Recharge Wallet Amount Deducted]'.PHP_EOL;
                $this->User->generateLog($log_msg);
                ...
                if($response['is_instantpay_api'])
                {
                    ...
                    {
                        // call recharge API
                        $api_response = $this->User->instantpay_rechage_api($opt_code,$loggedUser['id'],$recharge_unique_id,$post['cardNumber'],$post['amount'],$response['api_id'],$loggedUser['user_code'],'R');
                    }
                    else
                    {
                        $api_response = array(
                ...
                if(isset($api_response['status']) && ($api_response['status'] == 1 || $api_response['status'] == 2))
                {
                    ...
                    if($api_response['status'] == 1){
                        ...
                        $this->Az->redirect('retailer/recharge/dth', 'system_message_error',lang('RECHARGE_PENDING'));   
                    ...
                    elseif($api_response['status'] == 2)
                    {
                        ...
                        $this->Az->redirect('retailer/recharge/dth', 'system_message_error',lang('RECHARGE_SUCCESS'));   
                ...
                else
                {
                    ...
                    $this->Az->redirect('retailer/recharge/dth', 'system_message_error',sprintf(lang('RECHARGE_FAILED'),$api_response['opt_msg']));
                         
                }
            }
            else
            {
                $this->Az->redirect('retailer/recharge/dth', 'system_message_error',lang('API_ERROR'));
            }
    ...
    {   
        ...
        $post = $this->input->post();
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Electicity Post Data - '.json_encode($post).']'.PHP_EOL;
        $this->User->generateLog($log_msg);
        $this->load->library('form_validation');
        $this->form_validation->set_rules('operator', 'Operator', 'required');
        ...
        else
        {
            ...
            {
                $this->Az->redirect('retailer/recharge/electricity', 'system_message_error',lang('OPERATOR_VALID_ERROR'));   
            }
            ...
            if($admin_wallet_balance < $post['amount']){
                // save system log
                $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Insufficient Wallet Error in Admin Account.]'.PHP_EOL;
                $this->User->generateLog($log_msg);
                $this->Az->redirect('retailer/recharge/electricity', 'system_message_error',lang('ADMIN_WALLET_BALANCE_ERROR'));
            } */
            ...
            if($chk_wallet_balance['wallet_balance'] < $post['amount']){
                ...
                $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Insufficient wallet error.]'.PHP_EOL;
                $this->User->generateLog($log_msg);
                $this->Az->redirect('retailer/recharge/electricity', 'system_message_error',lang('WALLET_ERROR'));   
            ...
            if($user_before_balance < $final_deduct_wallet_balance){
                ...
                $this->Az->redirect('retailer/recharge/electricity', 'system_message_error',lang('MIN_WALLET_ERROR'));
            ...
            if(isset($api_response['status']) && ($api_response['status'] == 1 || $api_response['status'] == 2))
            {
                ...
                if($api_response['status'] == 1){
                    ...
                    $this->db->where('recharge_display_id',$recharge_unique_id);
                    $this->db->update('recharge_history',array('txid'=>$api_response['txid'],'operator_ref'=>$api_response['operator_ref'],'api_response_id'=>$api_response['api_response_id']));
                    $this->Az->redirect('retailer/recharge/electricity', 'system_message_error',lang('RECHARGE_PENDING'));
                }
                else
                {
                    // update recharge status
                    ...
                    $this->User->generateLog($log_msg);
                    $this->Az->redirect('retailer/recharge/electricity', 'system_message_error',lang('RECHARGE_SUCCESS'));
                }
            }
            else
            {
                ...
                $this->db->where('recharge_display_id',$recharge_unique_id);
                $this->db->update('recharge_history',array('status'=>3));
                $this->Az->redirect('retailer/recharge/electricity', 'system_message_error',lang('RECHARGE_FAILED'));
            }
   ...
   public function getOperatorPlanList()
                ...
                if(isset($plan['Error']) && $plan['Error'] == 'False')
                {
                    ...
                    if($records)
                    {
                        ...
                        foreach($records as $tabKey=>$tabData)
                        {
                            ...
                            $str2.='<div class="col-sm-12 table-responsive">';
                            $str2.='<br />';
        ...
        elseif($post['cardNumber'] == ''){
            ...
            if($isInstantPayApiAllow)
            {
                ...
                if($isMemberKyc)
                {
                    ...
                    $paramsArray = array(
                        'param1' => $post['cardNumber']
                    );

                    $apiresponse = $this->User->generate_api_url($member_id,$post['operator'],0);
                    // get operator code
                    $get_operator_code = $this->db->get_where('api_operator',array('account_id'=>$account_id,'api_id'=>$apiresponse['api_id'],'opt_id'=>$post['operator']))->row_array();
                    ...
                    if(isset($decodeResponse['statuscode']) && $decodeResponse['statuscode'] == 'TXN')
                    {
                        $response = array(
                            ...
                            'status' => 1,
                            'msg' => 'success',
                            'customerName' => isset($decodeResponse['data']['customername']) ? $decodeResponse['data']['customername'] : '',
                            'monthlyRechargeAmount' => isset($decodeResponse['data']['billamount']) ? $decodeResponse['data']['billamount'] : 0,
                            'balance' =>  isset($decodeResponse['data']['billamount']) ? $decodeResponse['data']['billamount'] : 0
                    ...
                    else
                    {
                        $response = array(
                            'status' => 0,
                            'msg' => isset($decodeResponse['status']) ? $decodeResponse['status'] : '',
                        );
                    }
                }
                else
                {
                    $response = array(
                        'status' => 0,
                        'msg' => 'Sorry ! Your eKyc not approved yet, please submit your eKyc.'
                    );
                }
            }
            else
    ...
    public function complainAuth()
    {
        ...
        {   
            ...
            if(!$chk_recharge)
            {
                ...
                $this->Az->redirect('retailer/report/recharge', 'system_message_error',lang('AUTHORIZE_ERROR'));  
            ...
            if($status == true)
            {
                $this->Az->redirect('retailer/report/recharge', 'system_message_error',lang('COMPLAIN_SAVED'));
            }
            else
            {
                $this->Az->redirect('retailer/report/recharge', 'system_message_error',lang('COMMON_ERROR'));
            }
    ...
    public function ekycAuth()
    {
        ...
        {   
            ...
            if($response['status'] == 1)
            {
                ...
                $this->Az->redirect('retailer/recharge/ekycOtp/'.$response['otpReferenceID'], 'system_message_error',$msg);
            }
            else
            {
                ...
                $this->Az->redirect('retailer/recharge/ekyc', 'system_message_error',$msg);
            }
    ...
    public function ekycOtpAuth()
    {
        ...
        {   
            ...
            {
                $msg = '<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$response['msg'].'</div>';
                $this->Az->redirect('retailer/recharge/ekycOtp/'.$otpReferenceID, 'system_message_error',$msg);
            }
