...
class Transfer extends CI_Controller
{
    ...
    public function fundTransfer($bene_id = 0, $sender_id = 0)
    {
        ...
        if ($bene_id && !$chk_beneficiary) {
            $this->Az->redirect('retailer/transfer/beneficiaryList', 'system_message_error', lang('DB_ERROR'));
        }
    ...
    public function fundTransferAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($before_balance < $final_deduct_wallet_balance) {
                ...
                $this->User->generateLog($log_msg);
                $this->Az->redirect('retailer/transfer/beneficiaryList', 'system_message_error', lang('WALLET_BALANCE_ERROR'));
            }
            ...
            if (isset($responseData['statuscode']) && $responseData['statuscode'] == 'TXN' && $responseData['status'] == 'Transaction Successful') {
                ...
                $this->User->distribute_money_transfer_commision($recordID, $transaction_id, $amount, $loggedAccountID, $surcharge_amount, 'RT', $loggedUser['user_code'], $txnType);

                // save system log
                $log_msg = '[' . date('d-m-Y H:i:s') . ' - RT(' . $loggedUser['user_code'] . ') - AEPS Payout Api - Distribute Commision/Surcharge End]' . PHP_EOL;
                $this->User->generateLog($log_msg);

                $this->Az->redirect('retailer/transfer/beneficiaryList', 'system_message_error', lang('MANUAL_TRANSFER_SUCCESS'));
            } elseif (
                ...
                $this->Az->redirect('retailer/transfer/beneficiaryList', 'system_message_error', sprintf(lang('MANUAL_TRANSFER_SUCCESS')));
            }
        }
    }
    ...
    public function payoutBenificaryAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->Az->redirect('retailer/transfer/payoutBeneficiaryList', 'system_message_error', lang('BENEFICIARY_SAVE_SUCCESS'));
        }
    }
    ...
    public function payoutFundTransfer($bene_id = 0)
    {
        ...
        if (!$chk_beneficiary) {
            $this->Az->redirect('retailer/transfer/payoutBeneficiaryList', 'system_message_error', lang('DB_ERROR'));
        }
    ...
    /* public function payoutTransferAuth()
	{
		...
		else
		{
            ...
            if($chk_wallet_balance['wallet_balance'] < $final_deduct_wallet_balance){
                ...
                $this->User->generateLog($log_msg);
                $this->Az->redirect('retailer/transfer/payoutBeneficiaryList', 'system_message_error',lang('WALLET_BALANCE_ERROR'));
            }
			...
			{
				$this->Az->redirect('retailer/transfer/payoutBeneficiaryList', 'system_message_error',lang('MANUAL_TRANSFER_SUCCESS'));
			...
			elseif(isset($responseData['status']) && $responseData['status'] == 3)
			{
				...
				$this->Az->redirect('retailer/transfer/payoutBeneficiaryList', 'system_message_error',sprintf(lang('MANUAL_TRANSFER_FAILED'),$apiMsg));
			}
    ...
    public function payoutTransferAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($before_balance < $final_deduct_wallet_balance) {
                ...
                $this->Az->redirect('retailer/transfer/payoutBeneficiaryList', 'system_message_error', lang('MIN_WALLET_ERROR'));
            }
            ...
            if (isset($responseData['Error']) && $responseData['Error'] == 'False') {
                ...
                $this->User->generateLog($log_msg);
                $this->Az->redirect('retailer/transfer/payoutBeneficiaryList', 'system_message_error', sprintf(lang('MANUAL_TRANSFER_ERROR'), $responseData['Message']));
            }
        }
    }
    ...
    public function benificaryAccountAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->Az->redirect('retailer/transfer/benificaryAccountList', 'system_message_error', lang('BENEFICIARY_SAVE_SUCCESS'));
        }
    }
    ...
    public function newPayoutBenificaryAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->Az->redirect('retailer/transfer/newPayoutBeneficiaryList', 'system_message_error', lang('BENEFICIARY_SAVE_SUCCESS'));
        }
    }
    ...
    public function newPayoutFundTransfer($bene_id = 0)
    {
        ...
        if (!$chk_beneficiary) {
            $this->Az->redirect('retailer/transfer/newPayoutBeneficiaryList', 'system_message_error', lang('DB_ERROR'));
        }
    ...
    public function newPayoutTransferAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $mobile = $post['mobile'];
            $account_holder_name = ucwords($chk_beneficiary['account_holder_name']);
            $account_no = $chk_beneficiary['account_no'];
            $ifsc = $chk_beneficiary['ifsc'];
            $amount = $post['amount'];
            $mode = $post['mode'];
            ...
            if ($before_balance < $final_amount) {
                ...
                $log_msg = '[' . date('d-m-Y H:i:s') . ' - MD(' . $loggedUser['user_code'] . ') - Insufficient Wallet Error]' . PHP_EOL;
                $this->User->generateLog($log_msg);
                $this->Az->redirect('retailer/transfer/newPayoutBeneficiaryList', 'system_message_error', lang('WALLET_BALANCE_ERROR'));
            }
            ...
            if ($before_balance < $final_deduct_wallet_balance) {
                ...
                $this->Az->redirect('retailer/transfer/newPayoutBeneficiaryList', 'system_message_error', lang('MIN_WALLET_ERROR'));
            }
            ...
            if (isset($responseData['statuscode']) && $responseData['statuscode'] == 'TXN' && $responseData['status'] == 'Transaction Successful') {
                ...
                $this->Az->redirect('retailer/transfer/newPayoutBeneficiaryList', 'system_message_error', lang('MANUAL_TRANSFER_SUCCESS'));
            } elseif (
                isset($responseData['statuscode']) &&
                ($responseData['statuscode'] == 'SPD' ||
                    $responseData['statuscode'] == 'IAN' ||
                ...
                $this->Az->redirect('retailer/transfer/newPayoutBeneficiaryList', 'system_message_error', lang('MANUAL_TRANSFER_FAILED', $api_msg));
            } else {
                $this->Az->redirect('retailer/transfer/newPayoutBeneficiaryList', 'system_message_error', lang('MANUAL_TRANSFER_SUCCESS'));
            }
        }
    ...
    public function iciciBenificaryAccountAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->Az->redirect('retailer/transfer/iciciBenificaryAccountList', 'system_message_error', lang('BENEFICIARY_SAVE_SUCCESS'));
        }
    }
    ...
    public function upiPayoutBenificaryAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->Az->redirect('retailer/transfer/upiPayoutBeneficiaryList', 'system_message_error', lang('BENEFICIARY_SAVE_SUCCESS'));
        }
    }
    ...
    public function upiPayoutFundTransfer($bene_id = 0)
    {
        ...
        if (!$chk_beneficiary) {
            $this->Az->redirect('retailer/transfer/upiPayoutBeneficiaryList', 'system_message_error', lang('DB_ERROR'));
        }
    ...
    public function upiPayoutTransferAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($amount < 0) {
                $this->Az->redirect(
                    'retailer/transfer/upiPayoutBeneficiaryList',
            ...
            if ($before_balance < $final_amount) {
                ...
                $log_msg = '[' . date('d-m-Y H:i:s') . ' - MD(' . $loggedUser['user_code'] . ') - Insufficient Wallet Error]' . PHP_EOL;
                $this->User->generateLog($log_msg);
                $this->Az->redirect('retailer/transfer/upiPayoutBeneficiaryList', 'system_message_error', lang('WALLET_BALANCE_ERROR'));
            }
            ...
            if ($before_balance < $final_deduct_wallet_balance) {
                ...
                $this->Az->redirect('retailer/transfer/upiPayoutBeneficiaryList', 'system_message_error', lang('MIN_WALLET_ERROR'));
            }
            ...
            if (isset($response_data[5]) && $response_data[5] == 'Transaction success') {
                ...
                $this->Az->redirect('retailer/transfer/upiPayoutBeneficiaryList', 'system_message_error', lang('MANUAL_TRANSFER_SUCCESS'));
                ...
                $this->db->where('account_id', $account_id);
                $this->db->where('user_id', $loggedAccountID);
                $this->db->update('user_new_fund_transfer', ['api_response' => json_encode($response), 'status' => 4, 'updated' => date('Y-m-d H:i:s')]);
    ...
    public function upiOpenPayoutBeneficiaryList()
    {
        ...
        $benificaryList = $this->db->get_where('instantpay_upi_open_payout_user_benificary', ['account_id' => $account_id, 'user_id' => $loggedAccountID, 'is_delete' => 0])->result_array();
        // get bank list
        $bankList = $this->db->get('aeps_bank_list')->result_array();
    ...
    public function upiOpenPayoutBenificaryAuth()
    {
        ...
        $this->load->library('form_validation');
        $this->form_validation->set_rules('account_holder_name', 'Account Holder Name', 'required|trim|xss_clean|regex_match[/^[a-zA-Z]+( [a-zA-Z]+)*$/]');
        $this->form_validation->set_rules('ben_upi_id_account_number', 'Account Number', 'required|trim|xss_clean');
        $this->form_validation->set_rules('mobile_no', 'Mobile Number', 'required|trim|xss_clean|numeric|min_length[10]|max_length[10]');
    ...
    public function upiOpenPayoutFundTransfer($bene_id = 0)
    {
        ...
        if (!$chk_beneficiary) {
            $this->Az->redirect('retailer/transfer/upiOpenPayoutBeneficiaryList', 'system_message_error', lang('DB_ERROR'));
        }
    ...
    public function upiOpenPayoutTransferAuth()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($before_balance < $final_amount) {
                ...
                $log_msg = '[' . date('d-m-Y H:i:s') . ' - MD(' . $loggedUser['user_code'] . ') - Insufficient Wallet Error]' . PHP_EOL;
                $this->User->generateLog($log_msg);
                $this->Az->redirect('retailer/transfer/upiOpenPayoutBeneficiaryList', 'system_message_error', lang('WALLET_BALANCE_ERROR'));
            }
            ...
            if ($before_balance < $final_deduct_wallet_balance) {
                ...
                $this->Az->redirect('retailer/transfer/upiOpenPayoutBeneficiaryList', 'system_message_error', lang('MIN_WALLET_ERROR'));
            }
            ...
            if (isset($response_data[5]) && $response_data[5] == 'Transaction success') {
                ...
                $this->Az->redirect('retailer/transfer/upiOpenPayoutBeneficiaryList', 'system_message_error', lang('MANUAL_TRANSFER_SUCCESS'));
            } elseif (isset($response_data[5]) && $response_data[5] == 'Transaction failed') {
                $api_msg = 'Transcation Failed From Bank Side.';
                ...
                $this->Az->redirect('retailer/transfer/upiOpenPayoutBeneficiaryList', 'system_message_error', lang('MANUAL_TRANSFER_FAILED', $api_msg));
            } else {
                $this->Az->redirect('retailer/transfer/upiOpenPayoutBeneficiaryList', 'system_message_error', lang('MANUAL_TRANSFER_SUCCESS'));
            }
        }
    ...
    public function deleteBeneficiaryAccount($id)
    {
        ...
        if ($chk_user < 1) {
            $this->Az->redirect('retailer/transfer/newPayoutBeneficiaryList', 'system_message_error', lang('MEMBER_ERROR'));
        }
        ...
        $this->Az->redirect('retailer/transfer/newPayoutBeneficiaryList', 'system_message_error', lang('DELETE_SUCCESS'));
    }
            ...
            if ($beneficiary_data) {
                ...
                foreach ($beneficiary_data as $key => $list) {
                    ...
                    $str .=
                        ...
                        base_url() .
                        'retailer/transfer/newPayoutFundTransfer/' .
                        $list['id'] .
                        '"> Transfer</a>&nbsp;<a title="Fund Transfer" class="btn btn-danger btn-sm" href="' .
                        base_url() .
                        'retailer/transfer/deleteBeneficiaryAccount/' .
                        $list['id'] .
        ...
        if ($benificaryList) {
            ...
            if ($beneficiary_data) {
                ...
                foreach ($beneficiary_data as $key => $list) {
                    ...
                    $str .= '<td>' . $list['account_no'] . '</td>';
                    $str .= '<td>' . date('d-m-Y', strtotime($list['created'])) . '</td>';
                    $str .= '<td><a title="Fund Transfer" class="btn btn-primary btn-sm" href="' . base_url() . 'retailer/transfer/upiPayoutFundTransfer/' . $list['id'] . '"> Transfer</a></td>';
    ...
    public function saveBenificaryBankAccount()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($post['type'] == 1) {
                ...
                if ($chkBeneficiary == 5) {
                    $this->Az->redirect('retailer/trasnfer/addBankAccount', 'system_message_error', lang('BENEFICIARY_ALREADY_ERROR'));
                } else {
                    $bene_data = [
                        'account_id' => $account_id,
                        'user_id' => $loggedAccountID,
    ...
    /** Delete Money Transfer UPI ID Beneficiary*/
    public function deleteM1UpiBeneficiary($id)
    {
        $account_id = $this->User->get_domain_account();
        $loggedUser = $this->User->getAdminLoggedUser(RETAILER_SESSION_ID);
        $loggedAccountID = $loggedUser['id'];
        ...
        $this->db->where('user_id', $loggedAccountID);
        $this->db->where('id', $id);
        $success = $this->db->update('instantpay_upi_open_payout_user_benificary', ['is_delete' => 1]);
        if ($success) {
            if ($this->db->affected_rows() > 0) {
                $response = [
                    'error' => false,
            ...
            log_message('error', 'Money Transfer UPI,Failed to delete beneficiary: ' . $this->db->last_query());
            $response = [
                'error' => true,
                'dataval' => 'Money Transfer UPI,Failed to delete beneficiary. Please try again.',
            ];
        }
        echo json_encode($response);
    }
