...
class Newaeps extends CI_Controller {    
	...
	public function __construct() 
	{
		...
		$this->load->model('retailer/Newaeps_model');		
	...
	public function index(){
		...
		if(!$user_2fa_aeps_status){
			$this->Az->redirect('retailer/newaeps/memberRegistration', 'system_message_error',lang('AEPS_ACTIVE_ERROR'));
		}
		
		$user_2fa_aeps_loginn_status = $this->User->get_member_2fa_aeps_login_status($loggedUser['id']);

		if(!$user_2fa_aeps_loginn_status){
			$this->Az->redirect('retailer/newaeps/memberLogin', 'system_message_error',lang('AEPS_ACTIVE_ERROR'));
		}
	...
	public function activeAeps(){
...

		{
			...
			//call onboard status pipe wise
	            ...
	            elseif (isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true && $responseData['is_approved'] == 'Pending') {
	            	 ...
	            	 $this->Az->redirect('retailer/dashboard', 'system_message_error','<div class="alert alert-warning alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Onboarding complete,Please wait 6hr for AEPS Activation on FINO Payment Bank activation.</div>');
	            ...
	            {
	            	$this->Az->redirect('retailer/dashboard', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Onboarding Rejected by bank.</div>');
	            }
	            elseif (isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == false && $responseData['is_approved'] == 'Pending') {
	            	$this->Az->redirect('retailer/dashboard', 'system_message_error','<div class="alert alert-warning alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Onboarding Not Completed.</div>');
	            }
		...
		if($chk_kyc)
		{
			...
			$this->Az->redirect('retailer/newaeps', 'system_message_error',lang('AEPS_ACTIVE_SUCCESS'));
		}
	...
	public function activeAuth()
	{
		...
		{	
			...
			if(isset($_FILES['aadhar_photo']['name']) && $_FILES['aadhar_photo']['name']){
				...
				if($uploadError){
					$this->Az->redirect('retailer/newaeps/activeAeps', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$uploadError.'</div>');
				}
				else
				{
			...
			else
			{
				...
				$this->Az->redirect('retailer/newaeps/activeAeps', 'system_message_error',sprintf(lang('AEPS_ACTIVE_FAILED'),$response['msg']));
			}
		...
		$totalData = $this->db->query($sql)->num_rows();
			...
			if($get_filter_data){
				...
				foreach($get_filter_data as $list){
					...
					elseif($list['service'] == 'Cash Deposite')
					{
	...
	public function benificaryAuth(){
		...
		{   
			...
			if(isset($responseData) && $responseData['response_code'] == 2 && $responseData['status'] == true){
				...
				$this->Az->redirect('retailer/newaeps/uploadDocument/'.$bene_id, 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Account Detailed saved successfully. Please upload Supportive Document to activate</div>');
			}
			elseif(isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true){
				...
				$this->Az->redirect('retailer/newaeps/payout', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Account Detailed saved successfully</div>');
			}
			else{

				$error = isset($responseData['message']) ? $responseData['message'] : 'Sorry!! something went wrong. Please try again.';

				$this->Az->redirect('retailer/newaeps/addBeneficiary', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$error.'</div>');
			}
	...
	public function uploadDocument($bene_id = ''){
		...
		if(!$chk_bene){
			...
			$this->Az->redirect('retailer/newaeps/payout', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! Something went wrong.</div>');
		}
		...
		{   
			...
			if(isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true){
				...
				if(isset($_FILES['passbook']['name']) && $_FILES['passbook']['name']){
					...
					$uploadError = $this->upload->display_errors();
					if($uploadError){
						$this->Az->redirect('retailer/newaeps/payout', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$uploadError.'</div>');
					}
					else
					{
						$fileData = $this->upload->data();
				...
				$this->Az->redirect('retailer/newaeps/payout', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Account Documet upload successfully and verified</div>');
			}
			else{

				$error = isset($responseData['message']) ? $responseData['message'] : 'Sorry!! something went wrong. Please try again.';

				$this->Az->redirect('retailer/newaeps/uploadDocument/'.$post['bene_id'], 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$error.'</div>');
			}
	...
	public function fundTransfer($bene_id = 0){
		...
		if(!$benificaryData){
			$this->Az->redirect('retailer/newaeps/payout', 'system_message_error',lang('DB_ERROR'));
		}
...
*/
	public function fundTransferAuth() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if(!$benificaryData){
				$this->Az->redirect('retailer/newaeps/payout', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! beneficiary not valid.</div>');
			}
			...
			if($accountDetail['transaction_password'] != do_hash($post['txn_pass'])){
				...
				$this->Az->redirect('retailer/newaeps/fundTransfer/'.$post['bene_id'], 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! transaction password id wrong.</div>');		
			...
			if($wallet_balance < $total_wallet_deduct)
			{
				...
				$this->Az->redirect('retailer/newaeps/fundTransfer/'.$post['bene_id'], 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! Insufficient balance in your wallet.</div>');
			}
            ...
            curl_close ($curl);
             ...
             $this->Az->redirect('retailer/newaeps/transferOTP/'.$decode_otp_code, 'system_message_error',lang('PAYOUT_OTP_SENT'));	
			...
			if(isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true){
				...
				$this->Az->redirect('retailer/newaeps/transferReport', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Aeps payout successfully.</div>');
				...
				$this->Az->redirect('retailer/newaeps/fundTransfer/'.$post['bene_id'], 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$error.'</div>');
			}
...
// 			$this->Az->redirect('retailer/newaeps/transferReport', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry!! something went wrong.</div>');
// 		}
...
// 			$this->Az->redirect('retailer/newaeps/transferReport', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Status checked successfully.</div>');
...
// 			$this->Az->redirect('retailer/newaeps/transferReport', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$error.'</div>');
// 		}
...
// 	}
	...
	public function transferOTP($decode_otp_code = ''){
        ...
        if(!$chk_otp)
        {
            $this->Az->redirect('retailer/newaeps/fundTransfer', 'system_message_error',lang('DB_ERROR'));
        }
     ...
     public function updateTransferOTPAuth() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(!$chk_otp)
            {
                $this->Az->redirect('retailer/newaeps/transferOTP/'.$decode_otp_code, 'system_message_error',lang('OTP_ERROR'));
            }
			...
			if(isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true){
				...
				$this->Az->redirect('retailer/newaeps/fundTransfer/'.$get_beneficiaryData['id'], 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$error.'</div>');
			}
    ...
    public function checkAccountStatus($ref_id = '') {
		...
		if(isset($responseData) && $responseData['response_code'] == 1 && $responseData['status'] == true && $responseData['accountstatus'] == 1 ){
			...
			$this->Az->redirect('retailer/newaeps/payout', 'system_message_error','<div class="alert alert-success alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Status checked successfully.</div>');

		}
		else{

			
			$this->Az->redirect('retailer/newaeps/payout', 'system_message_error','<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'.$responseData['message'].'</div>');
		}
	...
	public function memberRegistration(){
		...
		$siteUrl = base_url();	
  		...
  		$get_member_data  = $this->db->order_by('id','DESC')->get_where('new_aeps_member_kyc',array('member_id'=>$loggedUser['id'],'account_id'=>$account_id))->row_array();
			...
			if($user_aeps_status)
			{
				...
				if($post)
				{
				    ...
				    if(isset($responseData['response_code']) && $responseData['response_code'] == 1 && $responseData['status'] == true)
					{
					    ...
					    $response = array(
            				'status' => 1,
            				'msg' => $responseData['message'].' Please Attach Customer Biometric',
	...
	public function cashDeposite(){
		...
		$data = array(
			...
			'system_info' => $this->Az->getsystemMessageInfo(),
			'system_warning' => $this->Az->getSystemMessageWarning(),
			'content_block' => 'newaeps/cash-deposite'
		);
		$this->parser->parse('retailer/layout/column-1' , $data);
	...
	public function depositeApiAuth()
	{
		$account_id = $this->User->get_domain_account();
		$accountData = $this->User->get_account_data($account_id);
		$loggedUser = $this->User->getAdminLoggedUser(RETAILER_SESSION_ID);
			...
			if($user_aeps_status)
			{
				...
				if($post)
				{
					...
					$user_before_balance = $this->User->getMemberWalletBalanceSP($memberID);
					 ...
					 if($user_before_balance < $amount){
		                // save system log
		                $log_msg = '['.date('d-m-Y H:i:s').' - R('.$loggedUser['user_code'].') - Insufficient Wallet Error]'.PHP_EOL;
		                $this->User->generateLog($log_msg);
		                
		                $response = array(
									'status' => 0,
									'msg' => 'Sorry ! Insufficient Wallet Balance.'
								);


		            }
					elseif($aadharNumber && $mobile && $biometricData && $iin)
					{	
						log_message('debug', ' Cash Deposite AEPS api API Call');

						
						if($serviceType == 'Cash Deposite')
						{	
							log_message('debug', 'Cash Deposite New AEPS api API Call');

							$txnID = uniqid('CSDD' . rand(11,99));
							$txnType = 'CD';
							$remarks = 'Withdrawal';
							$api_url = PAYSPRINT_CASH_DEPOSITE_API_URL;
							$is_withdrawal = 1;
							$is_bal_info = 0;
							$Servicestype = 'AccountWithdrowal';
							

							log_message('debug', 'Cash Deposite AEPS api API Url - '.$api_url);
							...
							if($amount >= 50 && $amount <= 10000)
							{
								...
								log_message('debug', 'Cash Deposite AEPS api post data - '.json_encode($datapost));
								...
								log_message('debug', 'Cash Deposite AEPS api header data - '.json_encode($header));
								...
								log_message('debug', 'Cash Deposite AEPS api response - '.json_encode($output));
								...
								{   
									...
									$recordID = $this->Newaeps_model->saveAepsTxn($txnID,'Cash Deposite',$aadharNumber,$mobile,$amount,$iin,$api_url,$output,$responseData['message'],2,$statementList,$balanceAmount,$bankRRN,$transactionAmount);
									...
									$this->Newaeps_model->debitBalance($txnID,$aadharNumber,$iin,$amount,$recordID,$serviceType,2);
									$str = '';
									$str = '<div class="table-responsive">';
									$str.='<table class="table table-bordered table-striped" width="100%" cellspacing="0">';
									$str.='<tr>';
								    ...
								    elseif($responseData['txnstatus'] == 4)
								    {
								        
								           $balanceAmount = 'Not Available';
									$bankRRN = 'Not Available';

									$transactionAmount = 'Not Available';
									$statementList = array();
									
									$recordID = $this->Newaeps_model->saveAepsTxn($txnID,'Cash Deposite',$aadharNumber,$mobile,$amount,$iin,$api_url,$output,$responseData['message'],4,$statementList,$balanceAmount,$bankRRN,$transactionAmount);
									...
									$this->Newaeps_model->debitBalance($txnID,$aadharNumber,$iin,$amount,$recordID,$serviceType,4);
									$str = '';
									$str = '<div class="table-responsive">';
									$str.='<table class="table table-bordered table-striped" width="100%" cellspacing="0">';
									$str.='<tr>';
									$str.='<td>Txn Status</td><td><font color="orange">Hold</font></td>';
									$str.='</tr>';
									...
									$response = array(
										'status' => 1,
										'msg' => 'Transcation Hold From Bank Side',
										'balanceAmount' => '',
										'bankRRN' => '',
										'is_bal_info' => $is_bal_info,
										'is_withdrawal' => $is_withdrawal,
