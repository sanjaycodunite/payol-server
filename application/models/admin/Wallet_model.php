...
class Wallet_model extends CI_Model {
    ...
    {       
            ...
            'description'         => 'By Admin '.$post['description'],
            'is_manual' => 1            
            );
    ...
    {       
            ...
            'created'             => date('Y-m-d H:i:s'),      
            'credited_by'         => $loggedUser['id'],
            'description'         => 'By Admin '.$post['description'].' | '.$txnid.' | '.$ref_no.' | '.$post['utr_no'].' | Amount :'.$request_amount.' | Charge : '.number_format($charge_amount,2),
            'is_manual' => 1,
     ...
     //upi wallet
    public function saveUpiWallet($post)
    {       
            $account_id = $this->User->get_domain_account();
            $accountData = $this->User->get_account_data($account_id);
            $loggedUser = $this->User->getAdminLoggedUser(ADMIN_SESSION_ID);
            ...
            if($accountData['account_type'] == 2){
                if($type == 1)
                {
                    $account_wallet_balance = $this->User->getMemberUpiWalletBalanceSP($loggedAccountID);
