...
class User extends CI_Model
{
    public function checkPermission($mode = SUPERADMIN_SESSION_ID, $is_front = false)
    {
        ...
        $memberDetail = $this->db->get_where('users', ['id' => $loggedUser['id'], 'is_active' => 1, 'password' => $loggedUser['password']])->num_rows();
        if (!$memberDetail) {
    ...
    public function checkEmployePermission($mode = ADMIN_EMPLOYE_SESSION_ID, $is_front = false)
    {
        ...
        $memberDetail = $this->db->get_where('users', ['id' => $loggedUser['id'], 'is_active' => 1])->num_rows();
        if (!$memberDetail) {
    ...
    public function getLoggedUser($sessionID = '')
    {
        ...
        if ($user) {
            $user = $this->db->get_where('users', ['id' => $user['id']])->row_array();
            return $user;
        }
    }
    ...
    public function getAdminLoggedUser($sessionID = '')
    {
        ...
        if ($user) {
            $user = $this->db
                ->select('id,account_id,role_id,user_code,name,is_active,mobile,instantpay_outlet_id')
                ->get_where('users', ['id' => $user['id']])
                ->row_array();
            return $user;
        }
    }
    ...
    public function generate_unique_member_id($role_id = 0)
    {
        ...
        $chk_member_id = $this->db->get_where('users', ['account_id' => $account_id, 'user_code' => $user_display_id])->num_rows();
        if ($chk_member_id) {
    ...
    public function generate_unique_admin_id($account_code = '')
    {
        ...
        // check member id already registered or not
        $chk_member_id = $this->db->get_where('users', ['user_code' => $user_display_id])->num_rows();
        if ($chk_member_id) {
    ...
    public function get_domain_account()
    {
        ...
        // get domain id
        $check_domain = $this->db->get_where('account', ['domain_url' => $domain_name, 'status' => 1])->num_rows();
        if (!$check_domain) {
            redirect('errorpage');
        }

        // get domain id
        $get_domain_id = $this->db->get_where('account', ['domain_url' => $domain_name, 'status' => 1])->row_array();
    ...
    {
        // get domain id
        $get_domain_id = $this->db->get_where('account', ['id' => $account_id])->row_array();
    ...
    {
        // get domain id
        $get_domain_id = $this->db->get_where('account_custom_api_permission', ['account_id' => $account_id, 'api_id' => 1, 'status' => 1])->num_rows();
        return $get_domain_id;
    }
    ...
    {
        // get domain id
        $get_domain_id = $this->db->get_where('users', ['id' => $account_id, 'is_nsdl_active' => 1])->num_rows();
        return $get_domain_id;
    }
    ...
    {
        // get domain id
        $get_domain_id = $this->db->get_where('account_custom_api_permission', ['account_id' => $account_id, 'api_id' => 2, 'status' => 1])->num_rows();
        return $get_domain_id;
    }
    ...
    {
        // get domain id
        $get_domain_id = $this->db
            ->select('package_id')
            ->get_where('account', ['id' => $account_id])
            ->row_array();
    ...
    public function getMemberWalletBalanceSP($account_id = 0, $walletType = 1)
    {
        ...
        $procResponse = $this->db
            ->query("SELECT ROUND(SUM((CASE WHEN type = 1 THEN amount ELSE CONCAT('-',amount) END)),2) as actualBalance FROM tbl_member_wallet WHERE member_id = " . $account_id . " and wallet_type = " . $walletType)
            ->row_array();
    ...
    public function getMemberUpiWalletBalanceSP($account_id = 0, $walletType = 1)
    {
        ...
        $queryData = ['member_id' => $account_id, 'walletType' => $walletType];
        $procQuery = $this->db->query($callStoreProc, $queryData);
        $procResponse = $procQuery->row_array();
        //add this two line
        $procQuery->next_result();
    ...
    public function getAccountWalletBalanceSP($account_id = 0, $walletType = 1, $roleID = 0)
    {
        ...
        $queryData = ['member_id' => $account_id, 'walletType' => $walletType, 'roleID' => $roleID];
        $procQuery = $this->db->query($callStoreProc, $queryData);
        $procResponse = $procQuery->row_array();
        //add this two line
        $procQuery->next_result();
    ...
    public function getMemberCollectionWalletBalance($account_id = 0)
    {
        // get domain id
        $get_domain_id = $this->db
            ->select('collection_wallet_balance')
            ->get_where('users', ['id' => $account_id])
            ->row_array();
    ...
    public function getAdminAepsCommisionBlance($domain_account_id = 0)
    {
        ...
        // get domain id
        $get_domain_id = $this->db
            ->select('SUM(wallet_settle_amount) as totalBalance')
            ->get_where('member_aeps_comm', ['account_id' => $domain_account_id, 'is_paid' => 0])
            ->row_array();
    ...
    public function getAccountWiseAepsCommisionBlance($domain_account_id = 0)
    {
        ...
        $get_domain_id = $this->db
            ->select('SUM(wallet_settle_amount) as totalBalance')
            ->get_where('member_aeps_comm', ['member_id' => $domain_account_id, 'is_paid' => 0])
            ->row_array();
        return isset($get_domain_id['totalBalance']) ? $get_domain_id['totalBalance'] : 0;
    }
    ...
    public function getSuperadminAepsCommisionBlance()
    {
        ...
        $get_domain_id = $this->db
            ->select('SUM(wallet_settle_amount) as totalBalance')
            ->get_where('member_aeps_comm', ['is_paid' => 0])
            ->row_array();
        return isset($get_domain_id['totalBalance']) ? $get_domain_id['totalBalance'] : 0;
    }
    ...
    public function getMemberAepsCommisionBlance($account_id = 0)
    {
        ...
        $get_domain_id = $this->db
            ->select('SUM(wallet_settle_amount) as totalBalance')
            ->get_where('member_aeps_comm', ['account_id' => $domain_account_id, 'member_id' => $account_id, 'is_paid' => 0])
            ->row_array();
        return isset($get_domain_id['totalBalance']) ? $get_domain_id['totalBalance'] : 0;
    }
    ...
    public function getSuperadminCollectionWalletBalance($account_id = 0)
    {
        // get domain id
        $get_domain_id = $this->db
            ->select('SUM(collection_wallet_balance) as totalBalance')
    ...
    public function get_distributor_id($user_id = 0, $domain_account_id = 0)
    {
        ...
        // get account role id
        $get_role_id = $this->db
            ->select('role_id,created_by')
            ->get_where('users', ['id' => $user_id, 'account_id' => $domain_account_id])
            ->row_array();
        ...
        $get_return_id_role = $this->db
            ->select('role_id')
            ->get_where('users', ['id' => $md_id, 'account_id' => $domain_account_id])
            ->row_array();
    ...
    public function get_member_downline_id($user_id = 0)
    {
        ...
        // get account role id
        $get_role_id = $this->db
            ->select('GROUP_CONCAT(id) as member_id_str')
            ->get_where('users', ['created_by' => $user_id, 'account_id' => $domain_account_id])
            ->row_array();
        $md_id = isset($get_role_id['member_id_str']) ? explode(',', $get_role_id['member_id_str']) : [];
        $md_idd = [];
        $md_iddd = [];
        if ($md_id) {
            // get account role id
            $get_role_idd = $this->db
                ->select('GROUP_CONCAT(id) as member_id_str')
                ->where_in('created_by', $md_id)
                ->get_where('users', ['account_id' => $domain_account_id])
                ->row_array();
            $md_idd = isset($get_role_idd['member_id_str']) ? explode(',', $get_role_idd['member_id_str']) : [];
            if ($md_idd) {
                // get account role id
                $get_role_iddd = $this->db
                    ->select('GROUP_CONCAT(id) as member_id_str')
                    ->where_in('created_by', $md_idd)
                    ->get_where('users', ['account_id' => $domain_account_id])
                    ->row_array();
                $md_iddd = isset($get_role_iddd['member_id_str']) ? explode(',', $get_role_iddd['member_id_str']) : [];
            }
        }
    ...
    public function get_member_sponser_data($sponser_id = 0)
    {
        ...
        // get account role id
        $get_role_id = $this->db
            ->select('id,role_id,user_code,name')
            ->get_where('users', ['id' => $sponser_id, 'account_id' => $domain_account_id])
            ->row_array();
        return $get_role_id;
    }
    ...
    public function get_master_distributor_id($user_id = 0, $domain_account_id = 0)
    {
        ...
        if ($user_role_id == 4) {
            ...
            $get_id = $this->db
                ->select('role_id,created_by')
                ->get_where('users', ['id' => $created_by, 'account_id' => $domain_account_id])
                ->row_array();
    ...
    public function get_admin_id($domain_account_id = 0)
    {
        ...
        $get_role_id = $this->db
            ->select('id')
            ->get_where('users', ['role_id' => 2, 'account_id' => $domain_account_id])
            ->row_array();
    ...
    public function get_admin_wallet_balance($admin_id = 0, $domain_account_id = 0)
    {
        ...
        $get_role_id = $this->db
            ->select('wallet_balance')
            ->get_where('users', ['id' => $admin_id, 'account_id' => $domain_account_id])
            ->row_array();
    ...
    {
        $domain_account_id = $this->User->get_domain_account();
        $activeService = $this->db
            ->select('account_user_services.*')
            ->get_where('account_user_services', ['account_id' => $domain_account_id, 'member_id' => $account_id, 'status' => 1])
            ->result_array();
    ...
    public function admin_active_service($account_id = 0)
    {
        $domain_account_id = $this->User->get_domain_account();
        $activeService = $this->db
            ->select('account_services.*')
            ->get_where('account_services', ['account_id' => $domain_account_id, 'status' => 1])
            ->result_array();
        $activeServiceID = [];
        if ($activeService) {
            foreach ($activeService as $key => $list) {
    ...
    public function account_active_gateway()
    {
        $domain_account_id = $this->User->get_domain_account();
        $activeService = $this->db
            ->select('account_payment_gateway.*')
            ->get_where('account_payment_gateway', ['account_id' => $domain_account_id, 'status' => 1])
            ->result_array();
    ...
    {
        $domain_account_id = $this->User->get_domain_account();
        $chk_member = $this->db->get_where('users', ['id' => $member_id, 'account_id' => $domain_account_id, 'aeps_status' => 1])->num_rows();
        return $chk_member;
    }

    public function get_member_fingpay_aeps_status($member_id = 0)
    {
        $domain_account_id = $this->User->get_domain_account();
        $chk_member = $this->db->get_where('users', ['id' => $member_id, 'account_id' => $domain_account_id, 'fingpay_aeps_status' => 1])->num_rows();
        return $chk_member;
    }

    public function get_member_fingpay_2fa_aeps_status($member_id = 0)
    {
        $domain_account_id = $this->User->get_domain_account();
        $chk_member = $this->db->get_where('users', ['id' => $member_id, 'account_id' => $domain_account_id, 'fingpay_2fa_aeps_status' => 1])->num_rows();
        return $chk_member;
    }

    public function get_member_fingpay_2fa_ap_status($member_id = 0)
    {
        $domain_account_id = $this->User->get_domain_account();
        $chk_member = $this->db->get_where('users', ['id' => $member_id, 'account_id' => $domain_account_id, 'fingpay_2fa_ap_status' => 1])->num_rows();
        return $chk_member;
    }
    ...
    {
        $domain_account_id = $this->User->get_domain_account();
        $chk_member = $this->db->get_where('users', ['id' => $member_id, 'account_id' => $domain_account_id, 'new_aeps_status' => 1])->num_rows();
        return $chk_member;
    }
    ...
    public function get_member_2fa_aeps_status($member_id = 0)
    {
        ...
        $chk_member = $this->db
            ->limit(1)
            ->get_where('aeps_member_registration', ['member_id' => $member_id, 'account_id' => $domain_account_id, 'status' => 1])
            ->num_rows();
        return $chk_member;
    }
    ...
    public function get_member_2fa_aeps_login_status($member_id = 0)
    {
        ...
        $chk_member = $this->db
            ->limit(1)
            ->get_where('aeps_member_login_status', ['member_id' => $member_id, 'account_id' => $domain_account_id, 'status' => 1])
            ->num_rows();
        return $chk_member;
    }

    public function get_member_2fa_instantpay_aeps_status($member_id = 0)
    {
        $domain_account_id = $this->User->get_domain_account();
        $accountData = $this->User->get_account_data($domain_account_id);

        $get_outlet_id = $this->db
            ->select('instantpay_outlet_id')
            ->get_where('users', ['account_id' => $domain_account_id, 'id' => $member_id])
            ->row_array();
        $outlet_id = isset($get_outlet_id['instantpay_outlet_id']) ? $get_outlet_id['instantpay_outlet_id'] : 0;

        $api_url = INSTANTPAY_2FA_URL;
        $header = [
            'X-Ipay-Auth-Code: 1',
        ...
        log_message('debug', '2FA Register AEPS API header  data - ' . json_encode($header));
        ...
        if (isset($responseData['statuscode']) && $responseData['statuscode'] == 'TXN' && $responseData['status'] == 'Transaction Successful') {
            ...
            $this->db->update('users', ['is_2fa_register' => 1]);

            //save member for 2f auth
            ...
            $this->db->insert('instantpay_aeps_member_login_status', $loginData);
    ...
    public function get_member_2fa_instantpay_aeps_login_status($member_id = 0)
    {
        $domain_account_id = $this->User->get_domain_account();
        $chk_member = $this->db->get_where('instantpay_aeps_member_login_status', ['member_id' => $member_id, 'account_id' => $domain_account_id, 'status' => 1])->num_rows();
        return $chk_member;
    }
    ...
    {
        $domain_account_id = $this->User->get_domain_account();
        $chk_member = $this->db->get_where('users', ['id' => $member_id, 'account_id' => $domain_account_id, 'instantpay_aeps_status' => 1])->num_rows();
        return $chk_member;
    }
    ...
    public function account_razorpay_key()
    {
        ...
        $activeService = $this->db
            ->select('account_payment_gateway.*')
            ->get_where('account_payment_gateway', ['account_id' => $domain_account_id, 'gateway_id' => 1, 'status' => 1])
            ->row_array();
    ...
    public function saveUserLoginLog($account_id = 0, $ip_address = '', $sessionData = [], $postData = [])
    {
        $loginData = [
    ...
    public function prepaid_rechage_api($api_url, $api_post_data, $account_id, $recharge_unique_id, $api_id, $response_type = 0, $responsePara = [], $seperator = '', $api_header_data = [], $user_code = '', $user_type = '')
    {
        $domain_account_id = $this->User->get_domain_account();
    ...
    public function instantpay_rechage_api($opt_code, $account_id, $recharge_unique_id, $mobile, $amount, $api_id, $user_code = '', $user_type = '')
    {
        ...
        $userData = $this->db
            ->select('instantpay_outlet_id,mobile')
            ->get_where('users', ['id' => $account_id, 'account_id' => $domain_account_id])
            ->row_array();
        $outlet_id = $userData['instantpay_outlet_id'];
        $memberMobile = $userData['mobile'];

        $getAadharData = $this->db
            ->select('mobile,aadhar_data')
            ->get_where('instantpay_ekyc', ['account_id' => $domain_account_id, 'member_id' => $account_id, 'status' => 2])
            ->row_array();
        $agentMobile = $getAadharData['mobile'];
        $aadhar_data = isset($getAadharData['aadhar_data']) ? json_decode($getAadharData['aadhar_data'], true) : [];
    ...
    public function getElectricityOperatorBillerDetail($operator_code = '', $account_number = '', $account_id = 0)
    {
        if ($account_id) {
            ...
            $url =
                ELECTRICITY_RECHARGE_FETCH_CUSTOMER_API_URL .
                'memberid=' .
                RECHARGE_MEMBERID .
                '&pin=' .
                RECHARGE_API_PIN .
    ...
    public function distribute_recharge_commision($recharge_id, $recharge_unique_id, $amount, $account_id, $domain_account_id = 0)
    {
        ...
        if ($user_role_id == 3) {
            // get operator id
            $get_recharge_data = $this->db
                ->select('recharge_history.api_id,recharge_history.operator_code')
                ->get_where('recharge_history', ['recharge_history.id' => $recharge_id, 'recharge_history.account_id' => $domain_account_id])
                ->row_array();
            ...
            if ($api_id && $operator_code) {
                if ($accountData['account_type'] == 2) {
                    // get operator id
                    $get_op_id = $this->db
                        ->select('api_operator.opt_id')
                        ->get_where('api_operator', ['api_operator.account_id' => SUPERADMIN_ACCOUNT_ID, 'api_operator.api_id' => $api_id, 'api_operator.opt_code' => $operator_code])
                        ->row_array();
                } else {
                    // get operator id
                    $get_op_id = $this->db
                        ->select('api_operator.opt_id')
                        ->get_where('api_operator', ['api_operator.account_id' => $domain_account_id, 'api_operator.api_id' => $api_id, 'api_operator.opt_code' => $operator_code])
                        ->row_array();
                }
            ...
            if ($op_id) {
                ...
                $get_comm = $this->db->get_where('recharge_commision', ['account_id' => $domain_account_id, 'package_id' => $member_package_id, 'op_id' => $op_id])->row_array();
                // save system log
            ...
            if ($is_cogent_instantpay_api) {
                ...
                if ($op_id) {
                    // get recharge commision
                    $get_comm = $this->db->get_where('recharge_commision', ['account_id' => SUPERADMIN_ACCOUNT_ID, 'package_id' => $admin_package_id, 'op_id' => $op_id])->row_array();
                    // save system log
            ...
            if ($op_id) {
                ...
                if ($md_id) {
                    ...
                    $get_comm = $this->db->get_where('recharge_commision', ['account_id' => $domain_account_id, 'package_id' => $md_package_id, 'op_id' => $op_id])->row_array();
                    // save system log
            ...
            if ($op_id) {
                ...
                if ($distributor_id) {
                    ...
                    // get master distribution recharge commision
                    $get_comm = $this->db->get_where('recharge_commision', ['account_id' => $domain_account_id, 'package_id' => $distributor_package_id, 'op_id' => $op_id])->row_array();
                    // save system log
    ...
    public function distribute_electricity_commision($recharge_id, $recharge_unique_id, $amount, $account_id, $domain_account_id = 0)
    {
        ...
        if ($user_role_id == 3 || $user_role_id == 6) {
            // get operator id
            $get_recharge_data = $this->db
                ->select('recharge_history.operator_code')
                ->get_where('recharge_history', ['recharge_history.id' => $recharge_id, 'recharge_history.account_id' => $domain_account_id])
                ->row_array();
            $operator_code = isset($get_recharge_data['operator_code']) ? $get_recharge_data['operator_code'] : '';
            ...
            if ($operator_code) {
                ...
                $get_op_id = $this->db
                    ->select('operator.id')
                    ->get_where('operator', ['operator.operator_code' => $operator_code])
                    ->row_array();
    ...
    public function distribute_bbps_commision($recharge_id, $recharge_unique_id, $amount, $account_id)
    {
        ...
        if ($user_role_id == 3) {
            // get operator id
            $get_recharge_data = $this->db
                ->select('bbps_history.service_id')
                ->get_where('bbps_history', ['bbps_history.id' => $recharge_id, 'bbps_history.account_id' => $domain_account_id])
                ->row_array();
            ...
            if ($service_id) {
                // get recharge commision
                $get_comm = $this->db->get_where('bbps_commision', ['account_id' => $domain_account_id, 'package_id' => $member_package_id, 'service_id' => $service_id])->row_array();
                // save system log
                $log_msg = '[' . date('d-m-Y H:i:s') . ' - Commision Data - ' . json_encode($get_comm) . ']' . PHP_EOL;
                $this->User->generateLog($log_msg);
                if ($get_comm) {
                ...
                if ($accountData['account_type'] == 2) {
                    ...
                    $this->User->generateLog($log_msg);
                    // get master distribution recharge commision
                    $get_comm = $this->db->get_where('bbps_commision', ['package_id' => $admin_package_id, 'service_id' => $service_id])->row_array();
                    // save system log
                    $log_msg = '[' . date('d-m-Y H:i:s') . ' - Admin Commision Data - ' . json_encode($get_comm) . ']' . PHP_EOL;
                    $this->User->generateLog($log_msg);
                    if ($get_comm) {
            ...
            if ($service_id) {
                ...
                if ($md_id) {
                    ...
                    $this->User->generateLog($log_msg);
                    // get master distribution recharge commision
                    $get_comm = $this->db->get_where('bbps_commision', ['account_id' => $domain_account_id, 'package_id' => $md_package_id, 'service_id' => $service_id])->row_array();
                    // save system log
                    $log_msg = '[' . date('d-m-Y H:i:s') . ' - MD Commision Data - ' . json_encode($get_comm) . ']' . PHP_EOL;
                    $this->User->generateLog($log_msg);
                    if ($get_comm) {
            ...
            if ($service_id) {
                ...
                if ($distributor_id) {
                    ...
                    // get master distribution recharge commision
                    $get_comm = $this->db->get_where('bbps_commision', ['account_id' => $domain_account_id, 'package_id' => $distributor_package_id, 'service_id' => $service_id])->row_array();
                    // save system log
                    $log_msg = '[' . date('d-m-Y H:i:s') . ' - Distributor Commision Data - ' . json_encode($get_comm) . ']' . PHP_EOL;
                    $this->User->generateLog($log_msg);
    ...
    public function get_admin_dmr_surcharge($amount = 0, $loggedAccountID = 0)
    {
        ...
        $domain_account_id = $this->User->get_domain_account();
        // get surcharge
        $getSurcharge = $this->db->get_where('dmr_commision', ['account_id' => $domain_account_id, 'member_id' => $loggedAccountID, 'start_range <=' => $amount, 'end_range >=' => $amount])->row_array();
        if ($getSurcharge) {
    ...
    public function get_dmr_surcharge($amount = 0, $loggedAccountID = 0)
    {
        ...
        $domain_account_id = $this->User->get_domain_account();
        // get surcharge
        $getSurcharge = $this->db->get_where('dmr_commision', ['account_id' => $domain_account_id, 'package_id' => $member_package_id, 'start_range <=' => $amount, 'end_range >=' => $amount])->row_array();
    ...
    public function get_xpress_payout_surcharge($amount = 0, $loggedAccountID = 0)
    {
        ...
        $domain_account_id = $this->User->get_domain_account();
        // get surcharge
        $getSurcharge = $this->db->get_where('xpress_payout_charge', ['account_id' => $domain_account_id, 'package_id' => $member_package_id, 'start_range <=' => $amount, 'end_range >=' => $amount])->row_array();
    ...
    public function get_member_dmt_surcharge($amount = 0, $loggedAccountID = 0)
    {
        ...
        $domain_account_id = $this->User->get_domain_account();
        // get surcharge
        $getSurcharge = $this->db->get_where('dmt_commision', ['account_id' => $domain_account_id, 'package_id' => $member_package_id, 'start_range <=' => $amount, 'end_range >=' => $amount])->row_array();
    ...
    public function get_member_nsdl_surcharge($domain_account_id = 0, $loggedAccountID = 0)
    {
        ...
        $surcarge_amount = 0;
        // get surcharge
        $getSurcharge = $this->db->get_where('nsdl_pancard_charge', ['account_id' => $domain_account_id, 'package_id' => $member_package_id])->row_array();
    ...
    public function get_admin_nsdl_surcharge($domain_account_id = 0, $loggedAccountID = 0)
    {
        ...
        $surcarge_amount = 0;
        // get surcharge
        $getSurcharge = $this->db->get_where('nsdl_pancard_charge', ['account_id' => 10000, 'package_id' => $admin_package_id])->row_array();
    ...
    public function get_money_transfer_surcharge($amount = 0, $loggedAccountID = 0, $mode = '')
    {
        $member_package_id = $this->User->getMemberPackageID($loggedAccountID);
        $member_role_id = $this->User->getMemberRoleID($loggedAccountID);
        $surcarge_amount = 0;
        $domain_account_id = $this->User->get_domain_account();
        // get surcharge
        $getSurcharge = $this->db
            ->get_where('money_transfer_commision', ['account_id' => $domain_account_id, 'package_id' => $member_package_id, 'start_range <=' => $amount, 'end_range >=' => $amount, 'com_type' => strtoupper($mode)])
            ->row_array();
    ...
    public function get_aeps_commission($amount = 0, $loggedAccountID = 0, $com_type = 0, $domain_account_id = 0)
    {
        ...
        $commission_amount = 0;
        // get surcharge
        $getSurcharge = $this->db->get_where('aeps_commision', ['account_id' => $domain_account_id, 'package_id' => $member_package_id, 'start_range <=' => $amount, 'end_range >=' => $amount, 'com_type' => $com_type])->row_array();
        if ($getSurcharge) {
            $is_flat = $getSurcharge['is_flat'];
        ...
        return ['commission_amount' => $commission_amount, 'is_surcharge' => $is_surcharge];
    }
    ...
    public function get_gateway_charge($amount = 0, $loggedAccountID = 0)
    {
        ...
        $commission_amount = 0;
        // get surcharge
        $getSurcharge = $this->db->get_where('gateway_charge', ['account_id' => $domain_account_id, 'package_id' => $member_package_id, 'start_range <=' => $amount, 'end_range >=' => $amount])->row_array();
        if ($getSurcharge) {
            $is_flat = $getSurcharge['is_flat'];
            $is_surcharge = $getSurcharge['is_surcharge'];
            if ($member_role_id == 3) {
    ...
    public function get_upi_commission($loggedAccountID = 0, $amount = 0)
    {
        ...
        $commission_amount = 0;
        // get surcharge
        $getSurcharge = $this->db->get_where('upi_commision', ['account_id' => $domain_account_id, 'package_id' => $member_package_id])->row_array();
        if ($getSurcharge) {
            $is_flat = $getSurcharge['is_flat'];
            $is_surcharge = $getSurcharge['is_surcharge'];
            if ($member_role_id == 3) {
    ...
    public function get_upi_cash_commission($loggedAccountID = 0, $amount = 0)
    {
        ...
        $commission_amount = 0;
        // get surcharge
        $getSurcharge = $this->db->get_where('upi_cash_commision', ['account_id' => $domain_account_id, 'package_id' => $member_package_id])->row_array();
        if ($getSurcharge) {
            $is_flat = $getSurcharge['is_flat'];
            $is_surcharge = $getSurcharge['is_surcharge'];
            if ($member_role_id == 3) {
    ...
    public function get_account_verify_surcharge($loggedAccountID = 0)
    {
        ...
        // get surcharge
        $getSurcharge = $this->db->get_where('dmr_account_verify_charge', ['account_id' => $domain_account_id, 'package_id' => $member_package_id])->row_array();
    ...
    public function get_admin_account_verify_surcharge($loggedAccountID = 0)
    {
        ...
        // get surcharge
        $getSurcharge = $this->db->get_where('dmr_account_verify_charge', ['package_id' => $account_package_id])->row_array();
        if ($getSurcharge) {
            $surcarge_amount = $getSurcharge['surcharge'];
        }
        return $surcarge_amount;
    ...
    public function distribute_payout_commision($dmt_id, $transaction_id, $amount, $account_id, $surcharge_amount, $user_type = '', $user_code = '', $txnType = '')
    {
        ...
        $log_msg =
            ...
            '(' .
            $user_code .
            ') - AEPS Payout Api - Distribute Commision/Surcharge - User Role - ' .
            $user_role_id .
            ' transaction_id - ' .
            $transaction_id .
            ' - Txn Amount - ' .
        ...
        if ($user_role_id == 4) {
            ...
            if ($md_id) {
                ...
                $log_msg =
                    ...
                    '(' .
                    $user_code .
                    ') - AEPS Payout Api - Distribute Commision/Surcharge- MD ID - ' .
                    $md_id .
                    ' - MD Charge - ' .
                    $md_surcharge_amount .
                    ' transaction_id - ' .
                ...
                if ($surcharge_amount > $md_surcharge_amount) {
                    ...
                    // save system log
                    $log_msg =
                        '[' . date('d-m-Y H:i:s') . ' - ' . $user_type . '(' . $user_code . ') - AEPS Payout Api - Distribute Commision/Surcharge - MD Commision - ' . $commision . ' transaction_id - ' . $transaction_id . ']' . PHP_EOL;
                    $this->User->generateLog($log_msg);
                    if ($commision) {
                        // credit wallet
            ...
            if ($distributor_id) {
                ...
                // save system log
                $log_msg =
                    '[' . date('d-m-Y H:i:s') . ' - ' . $user_type . '(' . $user_code . ') - AEPS Payout Api - Distribute Commision/Surcharge - DT Charge - ' . $dist_surcharge_amount . ' transaction_id - ' . $transaction_id . ']' . PHP_EOL;
                $this->User->generateLog($log_msg);
                ...
                if ($surcharge_amount > $dist_surcharge_amount) {
                    ...
                    // save system log
                    $log_msg =
                        '[' . date('d-m-Y H:i:s') . ' - ' . $user_type . '(' . $user_code . ') - AEPS Payout Api - Distribute Commision/Surcharge - DT Commision - ' . $commision . ' transaction_id - ' . $transaction_id . ']' . PHP_EOL;
                    $this->User->generateLog($log_msg);
                    if ($commision) {
                        // credit wallet
                ...
                // save system log
                $log_msg =
                    '[' . date('d-m-Y H:i:s') . ' - ' . $user_type . '(' . $user_code . ') - AEPS Payout Api - Distribute Commision/Surcharge - MD Charge - ' . $md_surcharge_amount . ' transaction_id - ' . $transaction_id . ']' . PHP_EOL;
                $this->User->generateLog($log_msg);
    ...
    public function distribute_money_transfer_commision($dmt_id, $transaction_id, $amount, $account_id, $surcharge_amount, $user_type = '', $user_code = '', $txnType = '')
    {
        $domain_account_id = $this->User->get_domain_account();
        // get account role id
        $get_role_id = $this->db
        ...
        if ($user_role_id == 4) {
            ...
            if ($md_id) {
                // get dmr surcharge
                $md_surcharge_amount = $this->User->get_money_transfer_surcharge($amount, $md_id, $txnType);
                // save system log
            ...
            if ($distributor_id) {
                // get dmr surcharge
                $dist_surcharge_amount = $this->User->get_money_transfer_surcharge($amount, $distributor_id, $txnType);
                // save system log
    ...
    public function distribute_aeps_commision($dmt_id, $transaction_id, $account_id, $amount, $com_amount, $is_surcharge, $com_type, $user_type = '', $user_code = '', $domain_account_id = 0, $txnType = 'AEPS')
    {
        ...
        $log_msg =
            ...
            ') - ' .
            $txnType .
            ' - Distribute Commision/Surcharge - User Role - ' .
            $user_role_id .
            ' - Commision Amount - ' .
            $com_amount .
            ' - Is Surcharge - ' .
            ...
            if ($md_id) {
                ...
                if ($is_surcharge) {
                    if ($com_amount > $md_amount) {
                        ...
                        if ($commision) {
                            // credit wallet
                            $accountBalanceData = $this->db->get_where('users', ['id' => $md_id, 'account_id' => $domain_account_id])->row_array();
            ...
            if ($distributor_id) {
                ...
                if ($is_surcharge) {
                    if ($com_amount > $dt_amount) {
                        ...
                        if ($commision) {
                            // credit wallet
                            $accountBalanceData = $this->db->get_where('users', ['id' => $distributor_id, 'account_id' => $domain_account_id])->row_array();
                            $before_balance = $accountBalanceData['wallet_balance'];
                            $after_balance = $before_balance + $commision;
    ...
    public function generate_api_url($member_id = 0, $opt_id = 0, $amount = 0, $loggedAccountID = 0)
    {
        ...
        if ($accountData['account_type'] == 2) {
            $account_id = SUPERADMIN_ACCOUNT_ID;
            // get active api id
            $get_api_id = $this->db->get_where('member_active_api', ['account_id' => $account_id, 'package_id' => $account_package_id, 'op_id' => $opt_id])->row_array();
            $api_id = isset($get_api_id['api_id']) ? $get_api_id['api_id'] : 0;
        } else {
            if ($amount == 0) {
                // get active api id
                $get_api_id = $this->db->get_where('member_active_api', ['account_id' => $account_id, 'package_id' => $upper_package_id, 'op_id' => $opt_id])->row_array();
                $api_id = isset($get_api_id['api_id']) ? $get_api_id['api_id'] : 0;
                if (!$api_id) {
                    // get active api id
                    $get_api_id = $this->db->get_where('member_active_api', ['account_id' => $account_id, 'member_id' => $member_id, 'op_id' => $opt_id])->row_array();
                    $api_id = isset($get_api_id['api_id']) ? $get_api_id['api_id'] : 0;
                }
            } else {
                // get active api id
                $get_amount_api_id = $this->db->get_where('amount_active_api', ['account_id' => $account_id, 'start_range <=' => $amount, 'end_range >=' => $amount, 'op_id' => $opt_id])->row_array();
                $api_id = isset($get_amount_api_id['api_id']) ? $get_amount_api_id['api_id'] : 0;
                if (!$api_id) {
                    // get active api id
                    $get_api_id = $this->db->get_where('member_active_api', ['account_id' => $account_id, 'package_id' => $member_package_id, 'op_id' => $opt_id])->row_array();
                    $api_id = isset($get_api_id['api_id']) ? $get_api_id['api_id'] : 0;
                    if (!$api_id) {
                        // get active api id
                        $get_api_id = $this->db->get_where('member_active_api', ['account_id' => $account_id, 'member_id' => $loggedAccountID, 'op_id' => $opt_id])->row_array();
                        $api_id = isset($get_api_id['api_id']) ? $get_api_id['api_id'] : 0;
                        if (!$api_id) {
                            // get active api id
                            $get_api_id = $this->db->get_where('member_active_api', ['account_id' => $account_id, 'package_id' => $upper_package_id, 'op_id' => $opt_id])->row_array();
        ...
        if ($api_id) {
            // get api data
            $apiData = $this->db->get_where('api', ['id' => $api_id, 'account_id' => $account_id])->row_array();

            // get method data
            $getMethodData = $this->db->get_where('api_parameter', ['account_id' => $account_id, 'api_id' => $api_id, 'type' => 1])->row_array();

            // get header data
            $getHeaderData = $this->db->get_where('api_header_parameter', ['account_id' => $account_id, 'api_id' => $api_id, 'type' => 1])->row_array();

            // get method all parameters
            $getParaList = $this->db->get_where('api_get_parameter', ['account_id' => $account_id, 'api_id' => $api_id])->result_array();

            // post method data
            $postMethodData = $this->db->get_where('api_parameter', ['account_id' => $account_id, 'api_id' => $api_id, 'type' => 2])->row_array();

            // get method all parameters
            $postParaList = $this->db->get_where('api_post_parameter', ['account_id' => $account_id, 'api_id' => $api_id])->result_array();

            $responseParaList = [];
            if ($apiData['response_type'] == 1) {
                $responseParaList = $this->db->get_where('api_str_response', ['account_id' => $account_id, 'api_id' => $api_id])->result_array();
            } elseif ($apiData['response_type'] == 2) {
                $responseParaList = $this->db->get_where('api_xml_response', ['account_id' => $account_id, 'api_id' => $api_id])->result_array();
            } elseif ($apiData['response_type'] == 3) {
                $responseParaList = $this->db->get_where('api_json_response', ['account_id' => $account_id, 'api_id' => $api_id])->result_array();
            }
                ...
                if ($getParaList) {
                    foreach ($getParaList as $getList) {
                        if ($getList['value'] == '#') {
                            ...
                            // get operator type
                            $get_opt_type = $this->db
                                ->select('type,operator_code')
                                ->get_where('operator', ['id' => $opt_id])
                                ->row_array();
            ...
            return [
                ...
                'responsePara' => $responsePara,
                'seperator' => $apiData['response_seperator'],
                'is_active_pathway_api' => $apiData['is_active_pathway_api'],
            ];
        } else {
            return ['status' => 0, 'msg' => 'API Not Found.'];
        }
    }
    ...
    public function generate_get_balance_api_url($api_id = 0, $account_id = 0)
    {
        ...
        if ($get_balance_base_url) {
            // get method data
            $getMethodData = $this->db->get_where('api_get_balance_parameter', ['account_id' => $account_id, 'api_id' => $api_id, 'type' => 1])->row_array();

            // get header data
            $getHeaderData = $this->db->get_where('api_get_balance_header_parameter', ['account_id' => $account_id, 'api_id' => $api_id, 'type' => 1])->row_array();

            // get method all parameters
            $getParaList = $this->db->get_where('api_get_balance_get_parameter', ['account_id' => $account_id, 'api_id' => $api_id])->result_array();

            // post method data
            $postMethodData = $this->db->get_where('api_get_balance_parameter', ['account_id' => $account_id, 'api_id' => $api_id, 'type' => 2])->row_array();

            // get method all parameters
            $postParaList = $this->db->get_where('api_get_balance_post_parameter', ['account_id' => $account_id, 'api_id' => $api_id])->result_array();

            $responseParaList = [];
            if ($apiData['get_balance_response_type'] == 1) {
                $responseParaList = $this->db->get_where('api_get_balance_str_response', ['account_id' => $account_id, 'api_id' => $api_id])->result_array();
            } elseif ($apiData['get_balance_response_type'] == 2) {
                $responseParaList = $this->db->get_where('api_get_balance_xml_response', ['account_id' => $account_id, 'api_id' => $api_id])->result_array();
            } elseif ($apiData['get_balance_response_type'] == 3) {
                $responseParaList = $this->db->get_where('api_get_balance_json_response', ['account_id' => $account_id, 'api_id' => $api_id])->result_array();
            }
            ...
            return [
                ...
                'post_data' => $post_data,
                'header_data' => $header_data,
                'response_type' => $apiData['get_balance_response_type'],
                'responsePara' => $responsePara,
                'seperator' => $apiData['get_balance_response_seperator'],
            ];
        } else {
            return ['status' => 0, 'msg' => 'No API Found.'];
        }
    }

    public function call_get_balance_api($api_url, $api_post_data, $api_id = 0, $response_type = 0, $responsePara = [], $seperator = '', $api_header_data = [])
    {
        $domain_account_id = $this->User->get_domain_account();
    ...
    {
        return $this->db->get_where('mobikwik_bbps_service_category', ['service_id' => $service_id, 'status' => 1])->result_array();
    }

    public function get_bbps_biller_id($biller_id = 0)
    {
        return $this->db->get_where('mobikwik_bbps_service_category', ['id' => $biller_id])->row_array();
    }

    public function get_bbps_pmr_service_id($biller_id = 0)
    {
        return $this->db->get_where('mobikwik_bbps_service', ['id' => $biller_id])->row_array();
    }
    ...
    public function get_bbps_biller_param($service_id = 0, $biller_id = 0)
    {
        ...
        return $this->db->get_where('mobikwik_bbps_category_params', ['service_id' => $service_id, 'cat_id' => $biller_id])->result_array();
    }
    ...
    public function generate_bbps_token($member_id = 0)
    {
        $account_id = $this->User->get_domain_account();
        // get active token
        $get_token = $this->db->get_where('bbps_token', ['status' => 1])->row_array();
        ...
        if ($access_token == '') {
            ...
            $api_post_data['scope'] =
                'read_biller_categories read_billers read_regions read_bills create_transactions read_transactions read_plans register_complain check_complain_status read_operators read_circles read_operator_circle read_packs read_agent_balance';
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $api_url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
    ...
    public function call_bbps_service_bill_fetch_api($member_id = 0, $biller_payu_id = '', $pmr_service_id = 0, $post_data = [])
    {
        $account_id = $this->User->get_domain_account();
        $accountData = $this->User->get_account_data($account_id);
        ...
        if ($isInstantPayApiAllow) {
            $isMemberKyc = $this->db->get_where('users', ['id' => $member_id, 'is_instantpay_ekyc' => 1])->num_rows();
            if ($isMemberKyc) {
                ...
                $getAadharData = $this->db
                    ->select('mobile,aadhar_data')
                    ->get_where('instantpay_ekyc', ['account_id' => $account_id, 'member_id' => $member_id, 'status' => 2])
                    ->row_array();
                $agentMobile = $getAadharData['mobile'];
                $aadhar_data = isset($getAadharData['aadhar_data']) ? json_decode($getAadharData['aadhar_data'], true) : [];
                $pincode = isset($aadhar_data['pincode']) ? $aadhar_data['pincode'] : '';
            ...
            if ($pmr_service_id == 32) {
                ...
                $headers = ['Token:' . $accountData['dmt_token'], 'ServiceID:' . $pmr_service_id, 'biller_id:' . $biller_payu_id, 'BillerNumber:' . $number, 'BillerNumber2:'];
            } else {
                $headers = ['Token:' . $accountData['dmt_token'], 'ServiceID:' . $pmr_service_id, 'biller_id:' . $biller_payu_id];
                $params = isset($post_data['params']) ? $post_data['params'] : [];
                if ($params) {
                    $i = 3;
                    $k = 1;
    ...
    public function call_bbps_service_bill_pay_api($member_id = 0, $biller_payu_id = '', $pmr_service_id = 0, $post_data = [], $recharge_unique_id = '', $billerName = '')
    {
        $account_id = $this->User->get_domain_account();
        $accountData = $this->User->get_account_data($account_id);
        $isInstantPayApiAllow = $this->User->get_account_instantpay_bbps_api_status($account_id);
        ...
        if ($isInstantPayApiAllow) {
            ...
            if ($pmr_service_id == 32) {
                ...
                $params = isset($post_data['params']) ? $post_data['params'] : [];
                $headers = ['Token:' . $accountData['dmt_token'], 'ServiceID:' . $pmr_service_id, 'biller_id:' . $biller_payu_id, 'BillerName:' . $billerName, 'BillAmount:' . $post_data['amount'], 'TransID:' . $recharge_unique_id];
    ...
    public function call_bbps_electricity_bill_fetch_api($member_id = 0, $biller_payu_id = '', $pmr_service_id = 0, $post_data = [])
    {
        $account_id = $this->User->get_domain_account();
        $accountData = $this->User->get_account_data($account_id);
        $isInstantPayApiAllow = $this->User->get_account_instantpay_bbps_api_status($account_id);
        ...
        $final_response = [];
        if ($isInstantPayApiAllow) {
            $isMemberKyc = $this->db->get_where('users', ['id' => $member_id, 'instantpay_aeps_status' => 1])->num_rows();
            ...
            if ($isMemberKyc) {
                ...
                $getAadharData = $this->db
                    ->select('mobile,aadhar_data')
                    ->get_where('instantpay_ekyc', ['account_id' => $account_id, 'member_id' => $member_id, 'status' => 1])
                    ->row_array();
    ...
    public function call_bbps_electricity_bill_pay_api($member_id = 0, $biller_payu_id = '', $pmr_service_id = 0, $post_data = [], $recharge_unique_id = '', $billerName = '')
    {
        $account_id = $this->User->get_domain_account();
        $accountData = $this->User->get_account_data($account_id);
        $isInstantPayApiAllow = $this->User->get_account_instantpay_bbps_api_status($account_id);
    ...
    public function instantpay_biller_detail($outlet_id, $biller_payu_id, $recharge_unique_id, $paramsArray, $agentMobile, $pincode, $member_id = 0)
    {
        ...
        return ['fetchiPayID' => $fetchiPayID, 'validateiPayID' => $validateiPayID];
    }
        ...
        if ($api_id && $operator_code) {
            if ($account_id) {
                $get_operator_name = $this->db
                    ...
                    ->join('operator', 'operator.id = api_operator.opt_id')
                    ->get_where('api_operator', ['api_operator.account_id' => $account_id, 'api_operator.api_id' => $api_id, 'api_operator.opt_code' => $operator_code])
                    ->row_array();
            } else {
                $get_operator_name = $this->db
                    ->select('operator.operator_name')
                    ->join('operator', 'operator.id = api_operator.opt_id')
                    ->get_where('api_operator', ['api_operator.api_id' => $api_id, 'api_operator.opt_code' => $operator_code])
                    ->row_array();
            }
    ...
    public function checkMasterUpiPermission()
    {
        ...
        if (!in_array(5, $activeService)) {
            $this->Az->redirect(
                ...
                '<div class="alert alert-danger alert-dismissable">  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Sorry ! You are not authorized for UPI service.</div>'
            );
        }
    }
    ...
    public function sendNotification($user_id, $title, $message)
    {
        ...
        if ($accountData['notification_server_key']) {
            ...
            if ($user_id == 0) {
                ...
                $userDetail = $this->db->get_where('users', ['id' => $user_id])->row_array();
    ...
    public function cibAutoSettlement($account_holder_name, $account_no, $ifsc, $amount, $transaction_id, $bankID, $txnType, $account_code, $account_name, $account_id, $loggedAccountID)
    {
        ...
        // Create Header
        $header = ['Content-type: application/json', 'key: ecCIpsMXYix7R0JDFl1DiNYRmJSdvcZiPkN', 'X-Requested-With: XMLHttpRequest', 'Authorization: ' . $tokenType . ' ' . $accessToken];
...
-----END RSA PRIVATE KEY-----';
    ...
    public function menu_permission($menu_id = 0, $type = 1)
    {
        ...
        // check role id
        $chk_role = $this->db->get_where('superadmin_role_permission', ['role_id' => $access_role_id, $column => $menu_id])->num_rows();
        return $chk_role;
    }
    ...
    public function admin_menu_permission($menu_id = 0, $type = 1)
    {
        ...
        // check role id
        $chk_role = $this->db->get_where('admin_role_permission', ['role_id' => $access_role_id, $column => $menu_id])->num_rows();
        return $chk_role;
    }
    ...
    public function utiKycStatus($mobile = '')
    {
        ...
        $headers = ['Token: ' . $accountData['dmt_token'], 'mobile: ' . $mobile, 'Content-Type:application/x-www-form-urlencoded'];
    ...
    {
        $account_id = $this->User->get_domain_account();
        $recordData = $this->db->get_where('account_instant_loan', ['account_id' => $account_id, 'member_id' => $member_id])->row_array();
    ...
    {
        $recordData = $this->db->get_where('account', ['id' => $account_id, 'is_cogent_instant_api' => 1])->num_rows();
        return $recordData;
    }
    ...
    public function cibStatusCheck($transaction_id = '', $loggedAccountID = 0)
    {
        ...
        $header = ['Content-type: text/plain', 'apikey: eASJqqNDQGnQsIb1FqMxYJAj4Dy9nZld'];
    ...
    public function get_pan_activation_charge($member_id = 0)
    {
        ...
        $domain_account_id = $this->User->get_domain_account();
        // get surcharge
        $getSurcharge = $this->db->get_where('pan_activation_charge', ['account_id' => $domain_account_id, 'package_id' => $member_package_id])->row_array();
    ...
    public function get_find_pan_charge($member_id = 0)
    {
        ...
        $domain_account_id = $this->User->get_domain_account();
        // get surcharge
        $getSurcharge = $this->db->get_where('find_pan_charge', ['account_id' => $domain_account_id, 'package_id' => $member_package_id])->row_array();
    ...
    public function get_pan_charge($member_id = 0)
    {
        ...
        $domain_account_id = $this->User->get_domain_account();
        // get surcharge
        $getSurcharge = $this->db->get_where('pan_charge', ['account_id' => $domain_account_id, 'package_id' => $member_package_id])->row_array();
    ...
    {
        $domain_account_id = $this->User->get_domain_account();
        $totalTicket = $this->db->get_where('ticket', ['is_read' => 0, 'account_id' => $domain_account_id])->num_rows();
        return $totalTicket;
    }
    ...
    public function checkUserDecryptToken($decryptToken = "", $userID = "", $password = "", $Deviceid = "")
    {
        ...
        if ($tokenUserID && $tokenPwd && $tokenIP) {
            ...
            if ($chk_token_user && $tokenUserID == $userID) {
                $chk_user = $this->db->get_where('users', ['id' => $userID, 'password' => $password, 'device_id' => $Deviceid, 'is_active' => 1])->row_array();
    ...
    public function saveMemberLiveStatus($member_id = 0, $status = 0, $club_id = 0)
    {
        $account_id = $this->User->get_domain_account();
        ...
        if ($status == 0) {
            ...
            $this->db->update('club_live_member', ['status' => 0, 'updated' => date('Y-m-d H:i:s')]);
        } else {
            // check record exits or not
            $chkRecord = $this->db->get_where('club_live_member', ['account_id' => $account_id, 'member_id' => $member_id, 'club_id' => $club_id])->num_rows();
            if ($chkRecord) {
                $this->db->where('account_id', $account_id);
                $this->db->where('member_id', $member_id);
                $this->db->where('club_id', $club_id);
                $this->db->update('club_live_member', ['status' => 1, 'updated' => date('Y-m-d H:i:s')]);
            } else {
                $data = [
                    'account_id' => $account_id,
                    'member_id' => $member_id,
                ...
                $this->db->insert('club_live_member', $data);
            }
        }
        return true;
    }

    public function getClubNotification($member_id = 0)
    {
        $memberRoleId = $this->getMemberRoleID($member_id);
        $getUserCreatedBy = $this->db
            ->select('created_by')
            ->get_where('users', ['id' => $member_id])
            ->row_array();
        $userCreatedBy = isset($getUserCreatedBy['created_by']) ? $getUserCreatedBy['created_by'] : 0;
        $recordList = $this->db
            ->where_in('to_member_id', [0, $member_id])
            ->get_where('club_notification', ['is_read' => 0])
            ->result_array();
        if ($recordList) {
            foreach ($recordList as $key => $list) {
                // check member already request submitted or not
                $chkRequest = $this->db->get_where('club_member_request', ['member_id' => $member_id, 'club_id' => $list['club_id']])->num_rows();
                if ($chkRequest) {
                    unset($recordList[$key]);
                }
                if ($list['to_member_id'] == 0) {
                    $getClubCreatedBy = $this->db
                        ->select('created_by')
                        ->get_where('club_list', ['id' => $list['club_id']])
                        ->row_array();
                    $clubCreatedBy = isset($getClubCreatedBy['created_by']) ? $getClubCreatedBy['created_by'] : 0;
                    if ($memberRoleId == 8 && $userCreatedBy != $clubCreatedBy) {
                        unset($recordList[$key]);
                    }
                }
            }
    ...
    public function sendOtp($user_id = '', $otp_code = '', $post)
    {
        ...
        if ($user_id && $otp_code) {
            $status = $this->db->get_where('users', ['id' => $user_id, 'account_id' => $account_id])->row_array();
    ...
    public function getAccountSMSPermission($account_id = 0)
    {
        return $this->db->get_where('account', ['id' => $account_id, 'is_otp_login' => 1])->num_rows();
    }

    //uti balance request

    public function get_uti_balance_charge($member_id = 0)
    {
        $member_package_id = $this->User->getMemberPackageID($member_id);
        $member_role_id = $this->User->getMemberRoleID($member_id);
        ...
        $domain_account_id = $this->User->get_domain_account();
        // get surcharge
        $getSurcharge = $this->db->get_where('uti_balance_charge', ['account_id' => $domain_account_id, 'package_id' => $member_package_id])->row_array();
    ...
    public function distribute_uti_balance_commision($dmt_id, $transaction_id, $account_id, $surcharge_amount)
    {
        $domain_account_id = $this->User->get_domain_account();
        $accountData = $this->User->get_account_data($domain_account_id);
        // get account role id
        ...
        if ($user_role_id == 4) {
            ...
            if ($md_id) {
                // get dmr surcharge
                $md_surcharge_amount = $this->User->get_uti_balance_charge($md_id);
                ...
                if ($surcharge_amount > $md_surcharge_amount) {
                    ...
                    if ($commision) {
                        ...
                        $wallet_data = [
                            ...
                            'created' => date('Y-m-d H:i:s'),
                            'credited_by' => $account_id,
                            'description' => 'UTI BALANCE #' . $transaction_id . ' Commision Credited.',
                        ];
                        ...
                        if ($accountData['is_tds_amount'] == 1) {
                            ...
                            $wallet_data = [
                                ...
                                'created' => date('Y-m-d H:i:s'),
                                'credited_by' => $account_id,
                                'description' => 'UTI BALANCE #' . $transaction_id . '  Commision tds amount deducted',
                            ];
                    ...
                    if ($commision) {
                        ...
                        $wallet_data = [
                            ...
                            'created' => date('Y-m-d H:i:s'),
                            'credited_by' => $account_id,
                            'description' => 'UTI BALANCE #' . $transaction_id . ' Charge Debited.',
                        ];
                    }
                }
            }
            ...
            if ($distributor_id) {
                // get dmr surcharge
                $dist_surcharge_amount = $this->User->get_uti_balance_charge($distributor_id);
    ...
    {
        $domain_account_id = $this->User->get_domain_account();
        $totalTicket = $this->db->get_where('web_notification', ['is_read' => 0, 'account_id' => $domain_account_id])->num_rows();
        return $totalTicket;
    }

    public function call_mobikwik_bbps_electricity_bill_fetch_api($member_id = 0, $biller_id = '', $service_id = 0, $post_data = [])
    {
        ...
        $header = ['Content-Type:application/json', 'X-MClient:14', 'Checksum:+8aUoqoan3uf97WELlmPPRQPipapE1wKrLUyYK8bNIQ='];

        //$canumber = isset($post_data['canumber']) ? $post_data['canumber'] : '';
        if ($post_data['params']) {
            $canumber = $post_data['params'][0];
        } else {
            $canumber = isset($post_data['canumber']) ? $post_data['canumber'] : '';
        }
        ...
        $additionalParamsStr = "";
        if ($service_id == 12) {
            if ($post_data['billerID'] == 137) {
                $additionalParamsStr = '"bankName":"11"';
            } elseif ($post_data['billerID'] == 138) {
                $additionalParamsStr = '"bankName":"12"';
            } elseif ($post_data['billerID'] == 139) {
                $additionalParamsStr = '"bankName":"13"';
            } elseif ($post_data['billerID'] == 140) {
                $additionalParamsStr = '"bankName":"14"';
            } elseif ($post_data['billerID'] == 141) {
                $additionalParamsStr = '"bankName":"15"';
            } elseif ($post_data['billerID'] == 142) {
                $additionalParamsStr = '"bankName":"30"';
            } elseif ($post_data['billerID'] == 143) {
                $additionalParamsStr = '"bankName":"16"';
            } elseif ($post_data['billerID'] == 144) {
                $additionalParamsStr = '"bankName":"17"';
            } elseif ($post_data['billerID'] == 145) {
                $additionalParamsStr = '"bankName":"18"';
            } elseif ($post_data['billerID'] == 146) {
                $additionalParamsStr = '"bankName":"19"';
            } elseif ($post_data['billerID'] == 147) {
                $additionalParamsStr = '"bankName":"20"';
            } elseif ($post_data['billerID'] == 148) {
                $additionalParamsStr = '"bankName":"21"';
            } elseif ($post_data['billerID'] == 149) {
                $additionalParamsStr = '"bankName":"22"';
            } elseif ($post_data['billerID'] == 150) {
                $additionalParamsStr = '"bankName":"23"';
            } elseif ($post_data['billerID'] == 151) {
                $additionalParamsStr = '"bankName":"24"';
            } elseif ($post_data['billerID'] == 152) {
                $additionalParamsStr = '"bankName":"25"';
            } elseif ($post_data['billerID'] == 153) {
                $additionalParamsStr = '"bankName":"26"';
            } elseif ($post_data['billerID'] == 154) {
                $additionalParamsStr = '"bankName":"27"';
            } elseif ($post_data['billerID'] == 155) {
                $additionalParamsStr = '"bankName":"31"';
            } elseif ($post_data['billerID'] == 423) {
                $additionalParamsStr = '"bankName":"28"';
            } elseif ($post_data['billerID'] == 441) {
                $additionalParamsStr = '"bankName":"29"';
            } elseif ($post_data['billerID'] == 540) {
                $additionalParamsStr = '"bankName":"34"';
            } elseif ($post_data['billerID'] == 541) {
                $additionalParamsStr = '"bankName":"32"';
            } elseif ($post_data['billerID'] == 542) {
                $additionalParamsStr = '"bankName":"33"';
            } elseif ($post_data['billerID'] == 554) {
                $additionalParamsStr = '"bankName":"35"';
            }
        }

        if ($post_data['params']) {
            //$additionalParamsStr =array();
            $i = 1;
            for ($i = 1; $i < count($post_data['params']); $i++) {
                if ($post_data['billerID'] == 821 || $post_data['biller_id'] == 821 || $biller_id == 821) {
                    $biller_id = 1160;
                    $additionalParamsStr = '"emailId":"' . $post_data['params'][1] . '",';
                    $additionalParamsStr .= '"dateofBirth":"' . $post_data['params'][2] . '"';
                    break;
                } elseif ($post_data['billerID'] == 867 || $post_data['biller_id'] == 867 || $biller_id == 267) {
                    //$additionalParamsStr.= '"ad'.$i.'":"'.$post_data['params'][$i].'"';
                    $additionalParamsStr = '"paymentType":"' . $post_data['params'][1] . '"';
                    break;
                } elseif ($i + 1 == count($post_data['params'])) {
                    $additionalParamsStr .= '"ad' . $i . '":"' . $post_data['params'][$i] . '"';
                } else {
                    $additionalParamsStr .= '"ad' . $i . '":"' . $post_data['params'][$i] . '",';
                }
        ...
        // 			if($post_data['billerID'] == 821 || $post_data['biller_id'] == 821 )
        // 			{
        // 				$biller_id = 1160;
        // 			}

        $body = '{"adParams": {' . $additionalParamsStr . '},"uid":"' . MOBIKWIK_USER_ID . '","pswd":"' . MOBIKWIK_USER_PWD . '","cir": "","cn": "' . $canumber . '","op": "' . $biller_id . '"}';

        $httpUrl = MOBIKWIK_BILL_FETCH_API;
        ...
        log_message('debug', 'Bill fetch api response data - ' . $output);
        ...
        $get_operator = $this->db->get_where('mobikwik_bbps_service_category', ['id' => $post_data['operator_id']])->row_array();

        $billerName = isset($get_operator['billerName']) ? $get_operator['billerName'] : '';
        ...
        if (isset($responseData)) {
            if (isset($responseData['success']) && $responseData['success'] == "true" && $responseData['data']) {
                $final_response = [
                    'status' => 1,
                    'amount' => isset($responseData['data'][0]['billnetamount']) ? round($responseData['data'][0]['billnetamount'], 2) : 0,
                    'accountHolderName' => isset($responseData['data'][0]['userName']) ? $responseData['data'][0]['userName'] : '',
                    'duedate' => isset($responseData['data'][0]['dueDate']) ? $responseData['data'][0]['dueDate'] : 'NA',
                    'billDate' => isset($responseData['data'][0]['billdate']) ? $responseData['data'][0]['billdate'] : 'NA',
                    'billerName' => $billerName,
                    'canumber' => $canumber,
                ];
            } else {
                $errors = isset($responseData['message']['text']) ? $responseData['message']['text'] : '';
                $final_response = [
                    'status' => 0,
                    'errors' => $errors,
                ];
    ...
    public function call_mobikwik_bbps_electricity_bill_pay_api($member_id = 0, $biller_id = '', $service_id = 0, $post_data = [], $recharge_unique_id = '', $billerName = '')
    {
        $account_id = 1;
        ...
        if ($post_data['para1']) {
            $canumber = $post_data['para1'];
        } else {
            $canumber = isset($post_data['params'][0]) ? $post_data['params'][0] : '';
        }

        // if($post_data['operator_id'] == 604)
        // {
        // 	$biller_id = 208;
        // 	$canumber = isset($post_data['canumber']) ? $post_data['canumber'] : '';
        // }
        ...
        if ($service_id == 12) {
            if ($post_data['billerID'] == 137) {
                $additionalParamsStr = '&ad1=11';
            } elseif ($post_data['billerID'] == 138) {
                $additionalParamsStr = '&ad1=12';
            } elseif ($post_data['billerID'] == 139) {
                $additionalParamsStr = '&ad1=13';
            } elseif ($post_data['billerID'] == 140) {
                $additionalParamsStr = '&ad1=14';
            } elseif ($post_data['billerID'] == 141) {
                $additionalParamsStr = '&ad1=15';
            } elseif ($post_data['billerID'] == 142) {
                $additionalParamsStr = '&ad1=30';
            } elseif ($post_data['billerID'] == 143) {
                $additionalParamsStr = '&ad1=16';
            } elseif ($post_data['billerID'] == 144) {
                $additionalParamsStr = '&ad1=17';
            } elseif ($post_data['billerID'] == 145) {
                $additionalParamsStr = '&ad1=18';
            } elseif ($post_data['billerID'] == 146) {
                $additionalParamsStr = '"&ad1":"19"';
            } elseif ($post_data['billerID'] == 147) {
                $additionalParamsStr = '&ad1=20';
            } elseif ($post_data['billerID'] == 148) {
                $additionalParamsStr = '&ad1=21';
            } elseif ($post_data['billerID'] == 149) {
                $additionalParamsStr = '&ad1=22';
            } elseif ($post_data['billerID'] == 150) {
                $additionalParamsStr = '&ad1=23';
            } elseif ($post_data['billerID'] == 151) {
                $additionalParamsStr = '&ad1=24';
            } elseif ($post_data['billerID'] == 152) {
                $additionalParamsStr = '&ad1=25';
            } elseif ($post_data['billerID'] == 153) {
                $additionalParamsStr = '&ad1=26';
            } elseif ($post_data['billerID'] == 154) {
                $additionalParamsStr = '&ad1=27';
            } elseif ($post_data['billerID'] == 155) {
                $additionalParamsStr = '&ad1=31';
            } elseif ($post_data['billerID'] == 423) {
                $additionalParamsStr = '&ad1=28';
            } elseif ($post_data['billerID'] == 441) {
                $additionalParamsStr = '&ad1=29';
            } elseif ($post_data['billerID'] == 540) {
                $additionalParamsStr = '&ad1=34';
            } elseif ($post_data['billerID'] == 541) {
                $additionalParamsStr = '&ad1=32';
            } elseif ($post_data['billerID'] == 542) {
                $additionalParamsStr = '&ad1=33';
            } elseif ($post_data['billerID'] == 554) {
                $additionalParamsStr = '&ad1=35';
            }
        }
        ...
        if ($post_data['params']) {
            ...
            for ($i = 1; $i < count($post_data['params']); $i++) {
                if ($i == 2 || $i == 3) {
                    $additionalParamsStr .= '&ad' . $i . '=""';
                } elseif ($i == 4) {
                    $additionalParamsStr .= '&ad' . $i . '=' . $post_data['params'][2];
                } else {
                    $additionalParamsStr .= '&ad' . $i . '=' . $post_data['params'][$i];
                }
            }
        ...
        if ($post_data['billerID'] == 821) {
            $additionalParamsStr = '&ad1=' . $post_data['params'][1];
            $additionalParamsStr .= '&ad2=';
            $additionalParamsStr .= '&ad3=';
            $additionalParamsStr .= '&ad4=' . $post_data['params'][2];
            $biller_id = 1160;
        }

        if ($post_data['operator_id'] == 604 || $post_data['biller_id'] == 604) {
            if ($post_data['para1']) {
                $post_data['canumber'] = $post_data['para1'];
            }
            // credit card api
            $biller_id = 208;
            $canumber = isset($post_data['canumber']) ? $post_data['canumber'] : '';
        ...
        $httpUrl = MOBIKWIK_BILL_PAY_API_URL . 'uid=' . MOBIKWIK_USER_ID . '&pwd=' . MOBIKWIK_USER_PWD . '&reqid=' . $recharge_unique_id . '&cn=' . $canumber . '&op=' . $biller_id . '&amt=' . $amount . $additionalParamsStr;

        log_message('debug', 'Mobikwik Bill pay api url - ' . $httpUrl);

        $req_data = '{"amt":"' . $amount . '","cir":"","cn":"' . $canumber . '","op":"' . $biller_id . '","pwd":"' . MOBIKWIK_USER_PWD . '","reqid":"' . $recharge_unique_id . '","uid":"' . MOBIKWIK_USER_ID . '"}';

        log_message('debug', ' Mobikwik Bill pay body data - ' . $req_data);

        //$checksum = hash_hmac('sha256', $req_data, 'abcd@123');
        $checksum = hash_hmac('SHA256', $req_data, '5DYJS4686C79M48QT68M6QLDFFT2TY25', true);

        $header = ['Content-Type:application/json', 'X-MClient:14', 'Checksum:' . base64_encode($checksum)];
        ...
        log_message('debug', ' Mobikwik Bill pay response data - ' . $output);
        ...
        /*
		    ...
		    [txId] => MockVD5IHy8osY
		    [balance] => 0
		    [discountprice] => 75945.0
		    [couponstatus] => null
		    [mobikwikstamp] => MK01766703673
		    [opRefNo] => MockOCVMThe8VT
		    [googleVoucherCode] => null
        ...
        */
        ...
        if (isset($responseData['status']) && $responseData['status'] == "SUCCESS") {
            $final_response = [
                'status' => 1,
                'msg' => isset($responseData['statusDetails']) ? $responseData['statusDetails'] : 'Bill paid successfully.',
                'txnid' => isset($responseData['mobikwikstamp']) ? $responseData['mobikwikstamp'] : '',
                'operator_ref' => isset($responseData['opRefNo']) ? $responseData['opRefNo'] : '',
                'response_id' => $response_id,
            ];
        } elseif (isset($responseData['status']) && $responseData['status'] == "SUCCESSPENDING") {
            $final_response = [
                'status' => 1,
                'msg' => isset($responseData['statusDetails']) ? $responseData['statusDetails'] : 'Bill is in pending.',
                'txnid' => isset($responseData['mobikwikstamp']) ? $responseData['mobikwikstamp'] : '',
                'response_id' => $response_id,
            ];
        } elseif (!$responseData) {
            $final_response = [
                'status' => 0,
                'msg' => 'Sorry!! bill payment failed.',
                'response_id' => $response_id,
            ];
        } else {
            $errors = isset($responseData['errorMsg']) ? $responseData['errorMsg'] : 'Bill payment failed.';
            $final_response = [
                'status' => 0,
                'msg' => $errors,
                'response_id' => $response_id,
    ...
    public function call_mobikwik_bbps_electricity_bill_pay_api_new($member_id = 0, $biller_id = '', $service_id = 0, $post_data = [], $recharge_unique_id = '', $billerName = '')
    {
        // echo $service_id;
        // echo "<br>";
        // echo "<pre>";
        ...
        $additionalParamsStr = "";
        if ($service_id == 12) {
            if ($post_data['billerID'] == 137 || $post_data['biller_id'] == 137) {
                $additionalParamsStr = '&ad1=11';
            } elseif ($post_data['billerID'] == 138 || $post_data['biller_id'] == 138) {
                $additionalParamsStr = '&ad1=12';
            } elseif ($post_data['billerID'] == 139 || $post_data['biller_id'] == 139) {
                $additionalParamsStr = '&ad1=13';
            } elseif ($post_data['billerID'] == 140 || $post_data['biller_id'] == 140) {
                $additionalParamsStr = '&ad1=14';
            } elseif ($post_data['billerID'] == 141 || $post_data['biller_id'] == 141) {
                $additionalParamsStr = '&ad1=15';
            } elseif ($post_data['billerID'] == 142 || $post_data['biller_id'] == 142) {
                $additionalParamsStr = '&ad1=30';
            } elseif ($post_data['billerID'] == 143 || $post_data['biller_id'] == 143) {
                $additionalParamsStr = '&ad1=16';
            } elseif ($post_data['billerID'] == 144 || $post_data['biller_id'] == 144) {
                $additionalParamsStr = '&ad1=17';
            } elseif ($post_data['billerID'] == 145 || $post_data['biller_id'] == 145) {
                $additionalParamsStr = '&ad1=18';
            } elseif ($post_data['billerID'] == 146 || $post_data['biller_id'] == 146) {
                $additionalParamsStr = '"&ad1":"19"';
            } elseif ($post_data['billerID'] == 147 || $post_data['biller_id'] == 147) {
                $additionalParamsStr = '&ad1=20';
            } elseif ($post_data['billerID'] == 148 || $post_data['biller_id'] == 148) {
                $additionalParamsStr = '&ad1=21';
            } elseif ($post_data['billerID'] == 149 || $post_data['biller_id'] == 149) {
                $additionalParamsStr = '&ad1=22';
            } elseif ($post_data['billerID'] == 150 || $post_data['biller_id'] == 150) {
                $additionalParamsStr = '&ad1=23';
            } elseif ($post_data['billerID'] == 151 || $post_data['biller_id'] == 151) {
                $additionalParamsStr = '&ad1=24';
            } elseif ($post_data['billerID'] == 152 || $post_data['biller_id'] == 152) {
                $additionalParamsStr = '&ad1=25';
            } elseif ($post_data['billerID'] == 153 || $post_data['biller_id'] == 153) {
                $additionalParamsStr = '&ad1=26';
            } elseif ($post_data['billerID'] == 154 || $post_data['biller_id'] == 154) {
                $additionalParamsStr = '&ad1=27';
            } elseif ($post_data['billerID'] == 155 || $post_data['biller_id'] == 155) {
                $additionalParamsStr = '&ad1=31';
            } elseif ($post_data['billerID'] == 423 || $post_data['biller_id'] == 423) {
                $additionalParamsStr = '&ad1=28';
            } elseif ($post_data['billerID'] == 441 || $post_data['biller_id'] == 441) {
                $additionalParamsStr = '&ad1=29';
            } elseif ($post_data['billerID'] == 540 || $post_data['biller_id'] == 540) {
                $additionalParamsStr = '&ad1=34';
            } elseif ($post_data['billerID'] == 541 || $post_data['biller_id'] == 147) {
                $additionalParamsStr = '&ad1=32';
            } elseif ($post_data['billerID'] == 542 || $post_data['biller_id'] == 542) {
                $additionalParamsStr = '&ad1=33';
            } elseif ($post_data['billerID'] == 554 || $post_data['biller_id'] == 554) {
                $additionalParamsStr = '&ad1=35';
            }
        }
    ...
    public function getTotalUnreadRequest()
    {
        $domain_account_id = $this->User->get_domain_account();
        $totalTicket = $this->db->get_where('member_fund_request', ['is_read' => 0, 'account_id' => $domain_account_id])->num_rows();
        return $totalTicket;
    }

    public function getTotalUnreadUtiRequest()
    {
        $domain_account_id = $this->User->get_domain_account();
        $totalTicket = $this->db->get_where('uti_balance_request', ['is_read' => 0, 'account_id' => $domain_account_id])->num_rows();
        return $totalTicket;
    }
    ...
    public function call_mobikwik_recharge_electricity_bill_pay_api($member_id = '', $post = '', $recharge_unique_id = '')
    {
        if ($post['operator'] == 31) {
            $op = 140;
        ...
        $httpUrl = MOBIKWIK_BILL_PAY_API_URL . 'uid=' . MOBIKWIK_USER_ID . '&pwd=' . MOBIKWIK_USER_PWD . '&reqid=' . $recharge_unique_id . '&cn=' . $post['mobile'] . '&op=' . $op . '&amt=' . $post['amount'] . '&cir=19';
        ...
        log_message('debug', ' Mobikwik Recharge Bill  response data - ' . $output);
        ...
        if (isset($responseData['status']) && $responseData['status'] == "SUCCESS") {
            $final_response = [
                'status' => 1,
                'msg' => isset($responseData['statusDetails']) ? $responseData['statusDetails'] : 'Bill paid successfully.',
                'txnid' => isset($responseData['opRefNo']) ? $responseData['opRefNo'] : '',
                'operator_ref' => isset($responseData['opRefNo']) ? $responseData['opRefNo'] : '',
                'response_id' => $response_id,
            ];
        } elseif (isset($responseData['status']) && $responseData['status'] == "SUCCESSPENDING") {
    ...
    {
        $get_active_api = $this->db->get_where('member_upi_active_api', ['user_id' => $loggedAccountID])->row_array();
    ...
    public function get_scan_pay_surcharge($amount = 0, $loggedAccountID = 0)
    {
        $member_package_id = $this->User->getMemberPackageID($loggedAccountID);
        $member_role_id = $this->User->getMemberRoleID($loggedAccountID);
        $surcarge_amount = 0;
        $domain_account_id = $this->User->get_domain_account();
        // get surcharge
        $getSurcharge = $this->db->get_where('scan_pay_commision', ['account_id' => $domain_account_id, 'package_id' => $member_package_id, 'start_range <=' => $amount, 'end_range >=' => $amount])->row_array();
    ...
    public function get_open_money_surcharge($amount = 0, $loggedAccountID = 0, $mode = '')
    {
        $member_package_id = $this->User->getMemberPackageID($loggedAccountID);
        $member_role_id = $this->User->getMemberRoleID($loggedAccountID);
        $surcarge_amount = 0;
        ...
        // get surcharge
        $getSurcharge = $this->db
            ->get_where('open_money_payout_commision', ['account_id' => $domain_account_id, 'package_id' => $member_package_id, 'start_range <=' => $amount, 'end_range >=' => $amount, 'com_type' => strtoupper($mode)])
            ->row_array();
    ...
    public function getBBPSLogo($biller_id = '')
    {
        if ($biller_id == '111') {
            $biller_id = '1160';
        }
        $img_url = 'https://static.mobikwik.com/appdata/operator_icons/op' . $biller_id . '.png';

        return $img_url;
    }

    public function getPayoutAPIBalance()
    {
        $curl = curl_init();

        curl_setopt_array($curl, [
            CURLOPT_URL => 'https://api.zwitch.io/v1/accounts/va_FRn6pSZp3oZ8kZehNul1eEUWP/balance',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
            ...
            CURLOPT_CUSTOMREQUEST => 'GET',
            CURLOPT_HTTPHEADER => ['Authorization: Bearer ak_live_bq0SO69ZdaATI2dabwpeuF7GPfWw09XAIsOP:sk_live_L0TiS0BSbJVeMR6oiEYJ16zc49bfQErxuMai'],
        ]);
        ...
        return isset($api_response['amount']) ? $api_response['amount'] : 0;
    }

    public function CheckTranscationPin($member_id = '', $txn_pass = '')
    {
        $domain_account_id = $this->User->get_domain_account();

        $user_data = $this->db->get_where('users', ['account_id' => $domain_account_id, 'id' => $member_id])->row_array();

        $user_txn_pass = $user_data['decoded_transaction_password'];

        if ($txn_pass != $user_txn_pass) {
            return 0;
        } else {
            return 1;
        }
    ...
    public function cosmosBankQrStatusCheckApi($account_id, $loggedAccountID, $txnid = '', $refid = '')
    {
        $finalResponse = [];
        ...
        $header = ['Content-type: text/plain', 'cid:' . $cid];
        $req = [
            'source' => 'PAYOL1760',
            'channel' => 'api',
            'terminalId' => 'PAYOL-1760',
            'extTransactionId' => $refid,
        ];
        ...
        $cipher = 'AES-128-ECB';
        $data = openssl_encrypt(json_encode($req), $cipher, $key);
        ...
        // TO decrypt
        $decrypted_string = openssl_decrypt($result, $cipher, $key);

        /*$decrypted_string = '{"source":"PAYOL1760","channel":"api","terminalId":"PAYOL-1760","extTransactionId":"PAYOLDGAP52125879","checksum":"4d2526fda3bd9e86aaa89b044d932876b1692d324291599b4f9bb7f5b42ae240","status":"SUCCESS","txnType":"TXNSTATUS","data":[{"customerName":"SANJAY KUMAR","respCode":" ","respMessge":"SUCCESS","upiTxnId":"HDF2E4EAF6E6A2841759D8701EA5D2693EC","txnTime":"2024-03-17T23:55:32+05:30","amount":"300.00","upiId":"99842636@ikwik","extTransactionId":"PAYOLDGAP52125879","custRefNo":"407796883127","remark":"Product Amount"}],"transactionList":[]}';*/
        ...
        if (isset($decodeResult['status']) && $decodeResult['status'] == 'SUCCESS') {
            if (isset($decodeResult['data'][0]['respMessge']) && $decodeResult['data'][0]['respMessge'] == 'SUCCESS') {
                $bank_rrno = $decodeResult['data'][0]['custRefNo'];
                $PayerAmount = $decodeResult['data'][0]['amount'];
                $PayerName = $decodeResult['data'][0]['customerName'];
                $PayerVA = $decodeResult['data'][0]['upiId'];
                $TxnInitDate = $decodeResult['data'][0]['txnTime'];
                $TxnInitDate = str_replace('T', ' ', $TxnInitDate);
                $TxnInitDate = str_replace('+05:30', '', $TxnInitDate);
                ...
                if (isset($procResponse['msg']) && $procResponse['msg'] == 'SUCCESS') {
                    ...
                    $finalResponse = [
                        'status' => 1,
                        'msg' => 'Transaction credited successfully.',
                    ];
                } else {
                    $finalResponse = [
                        'status' => 0,
                        'msg' => 'Transaction already credited or duplicate RRN found.',
                    ];
                }
            } elseif (isset($decodeResult['data'][0]['respMessge']) && $decodeResult['data'][0]['respMessge'] == 'FAILURE') {
                $this->db->where('txnid', $txnid);
                $this->db->update('upi_dynamic_qr', ['is_callback' => 2, 'callback_datetime' => date('Y-m-d H:i:s')]);
                $finalResponse = [
                    'status' => 0,
                    'msg' => 'Transaction failed from bank side.',
                ];
            } else {
                $this->db->where('txnid', $txnid);
                $this->db->update('upi_dynamic_qr', ['is_callback' => 2, 'callback_datetime' => date('Y-m-d H:i:s')]);
                $finalResponse = [
                    'status' => 0,
                    'msg' => 'No transaction found on this qr code.',
                ];
            }
        } else {
            $this->db->where('txnid', $txnid);
    ...
    public function get_new_money_transfer_surcharge($amount = 0, $loggedAccountID = 0, $mode = '')
    {
        $member_package_id = $this->User->getMemberPackageID($loggedAccountID);
        $member_role_id = $this->User->getMemberRoleID($loggedAccountID);
        $surcarge_amount = 0;
        ...
        // get surcharge
        $getSurcharge = $this->db
            ->get_where('new_money_transfer_charge', ['account_id' => $domain_account_id, 'package_id' => $member_package_id, 'start_range <=' => $amount, 'end_range >=' => $amount, 'com_type' => strtoupper($mode)])
            ->row_array();
    ...
    public function get_add_fund_commission($loggedAccountID = 0, $amount = 0)
    {
        $member_package_id = $this->User->getMemberPackageID($loggedAccountID);
        $member_role_id = $this->User->getMemberRoleID($loggedAccountID);
        $is_surcharge = 0;
        ...
        $commission_amount = 0;
        // get surcharge
        $getSurcharge = $this->db->get_where('add_fund_commision', ['account_id' => $domain_account_id, 'package_id' => $member_package_id])->row_array();
        if ($getSurcharge) {
            $is_flat = $getSurcharge['is_flat'];
            $is_surcharge = $getSurcharge['is_surcharge'];
            if ($member_role_id == 3) {
    ...
    public function get_aeps_transcation_surcharge($member_id = 0)
    {
        $member_package_id = $this->User->getMemberPackageID($member_id);
        $member_role_id = $this->User->getMemberRoleID($member_id);
        ...
        $domain_account_id = $this->User->get_domain_account();
        // get surcharge
        $getSurcharge = $this->db->get_where('aeps_transcation_charge', ['account_id' => $domain_account_id, 'package_id' => $member_package_id])->row_array();
    ...
    public function yesBankQrStatusCheckApi($account_id, $loggedAccountID, $txnid = '', $refid = '')
    {
        $finalResponse = [];
        ...
        $decrypted_string = $this->yesDecryptValue($response, $enckey);
        ...
        $apiData = [
            ...
            'response' => json_encode($decrypted_string),
            'created' => date('Y-m-d H:i:s'),
        ];
        $this->db->insert('upi_check_status_api_response', $apiData);

        $decodeResult = explode('|', $decrypted_string);

        if (isset($decodeResult[4]) && $decodeResult[4] == 'SUCCESS') {
            if (isset($decodeResult[5]) && $decodeResult[5] == 'Transaction success') {
                $chk_dynamic_qr = $this->db->get_where('upi_dynamic_qr', ['txnid' => $txnid])->row_array();
                $is_add_fund = isset($chk_dynamic_qr['is_add_fund']) ? $chk_dynamic_qr['is_add_fund'] : 0;

                $bank_rrno = $decodeResult[11];
                $PayerAmount = $decodeResult[2];
                $PayerName = $decodeResult[14];
                $PayerVA = $decodeResult[8];
                $TxnInitDate = $decodeResult[3];
                $TxnCompletionDate = date('Y-m-d H:i:s');
                $member_role_id = $this->User->getMemberRoleID($loggedAccountID);
                $api_id = 2;
            ...
            } elseif (isset($decodeResult[3]) && $decodeResult[3] == 'FAILURE') {
                $this->db->where('txnid', $txnid);
                $this->db->update('upi_dynamic_qr', ['is_callback' => 2, 'callback_datetime' => date('Y-m-d H:i:s')]);
                $finalResponse = [
                    'status' => 0,
