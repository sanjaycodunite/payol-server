...
class User extends CI_Model{
    ...
    public function getAdminLoggedUser($sessionID = ''){
       ...
       if($user){
            $user = $this->db->select('id,account_id,role_id,user_code,name,is_active,mobile,instantpay_outlet_id')->get_where('users',array('id'=>$user['id']))->row_array();
            return $user;
        }
	...
	public function getMemberWalletBalanceSP($account_id = 0, $walletType = 1)
	{
        ...
        $procResponse = $this->db->query("SELECT ROUND(SUM((CASE WHEN type = 1 THEN amount ELSE CONCAT('-',amount) END)),2) as actualBalance FROM tbl_member_wallet WHERE member_id = ".$account_id." and wallet_type = ".$walletType)->row_array();
	...
	{
		$domain_account_id = $this->User->get_domain_account();
		$chk_member = $this->db->get_where('users',array('id'=>$member_id,'account_id'=>$domain_account_id,'fingpay_aeps_status'=>1))->num_rows();
		return $chk_member;
	}
	...
	{
		$domain_account_id = $this->User->get_domain_account();
		$chk_member = $this->db->get_where('users',array('id'=>$member_id,'account_id'=>$domain_account_id,'fingpay_2fa_aeps_status'=>1))->num_rows();
		return $chk_member;
	}
	...
	{
		$domain_account_id = $this->User->get_domain_account();
		$chk_member = $this->db->get_where('users',array('id'=>$member_id,'account_id'=>$domain_account_id,'fingpay_2fa_ap_status'=>1))->num_rows();
		return $chk_member;
	}
	...
	{
		$domain_account_id = $this->User->get_domain_account();
		$chk_member = $this->db->limit(1)->get_where('aeps_member_registration',array('member_id'=>$member_id,'account_id'=>$domain_account_id,'status'=>1))->num_rows();
		return $chk_member;
	}
	...
	{
		$domain_account_id = $this->User->get_domain_account();
		$chk_member = $this->db->limit(1)->get_where('aeps_member_login_status',array('member_id'=>$member_id,'account_id'=>$domain_account_id,'status'=>1))->num_rows();
		return $chk_member;
	}
	...
	public function get_member_2fa_instantpay_aeps_status($member_id = 0)
	{
        ...
        $accountData = $this->User->get_account_data($domain_account_id);
			...
			$get_outlet_id = $this->db->select('instantpay_outlet_id')->get_where('users',array('account_id'=>$domain_account_id,'id'=>$member_id))->row_array();
		           ...
		           // curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($request));
		            ...
		            log_message('debug', 'ICICI AEPS api response - '.json_encode($output)); 
						        ...
						        {	
						        	...
						        	$this->db->update('users',array('is_2fa_register'=>1));	
	...
	{
		$domain_account_id = $this->User->get_domain_account();
		$chk_member = $this->db->get_where('instantpay_aeps_member_login_status',array('member_id'=>$member_id,'account_id'=>$domain_account_id,'status'=>1))->num_rows();
		return $chk_member;
	}
	...
	public function get_money_transfer_surcharge($amount = 0, $loggedAccountID = 0,$mode ='')
	{
		...
		$domain_account_id = $this->User->get_domain_account();
		// get surcharge
	$getSurcharge = $this->db->get_where('money_transfer_commision',array('account_id'=>$domain_account_id,'package_id'=>$member_package_id,'start_range <='=>$amount,'end_range >='=>$amount,'com_type'=>strtoupper($mode)))->row_array();
	...
	public function generate_api_url($member_id = 0,$opt_id = 0,$amount = 0,$loggedAccountID = 0)
	{
		...
		if($api_id)
		{
			// get api data
		 	...
		 	return array('status'=>1,'msg'=>'Success','api_url'=>$api_url,'api_id'=>$api_id,'post_data'=>$post_data,'header_data'=>$header_data,'response_type'=>$apiData['response_type'],'responsePara'=>$responsePara,'seperator'=>$apiData['response_seperator'],'is_active_pathway_api'=>$apiData['is_active_pathway_api']);
	...
	{
		return $this->db->get_where('mobikwik_bbps_service_category',array('service_id'=>$service_id,'status'=>1))->result_array();
	}
	...
	{	
		...
		return $this->db->get_where('mobikwik_bbps_service_category',array('id'=>$biller_id))->row_array();
	}

	public function get_bbps_pmr_service_id($biller_id = 0)
	{
		return $this->db->get_where('mobikwik_bbps_service',array('id'=>$biller_id))->row_array();
	}
	...
	{		
		...
		return $this->db->get_where('mobikwik_bbps_category_params',array('service_id'=>$service_id,'cat_id'=>$biller_id))->result_array();
	}
...
public function distribute_pancard_commision($dmt_id, $transaction_id, $account_id, $surcharge_amount)
	...
	public function saveMemberLiveStatus($member_id = 0, $status = 0, $club_id = 0)
	{
		...
		if($status == 0)
		{
			...
			$this->db->update('club_live_member',array('status'=>0,'updated'=>date('Y-m-d H:i:s')));
		}
		else
		{
			// check record exits or not
			$chkRecord = $this->db->get_where('club_live_member',array('account_id'=>$account_id,'member_id'=>$member_id,'club_id'=>$club_id))->num_rows();
			if($chkRecord)
			{
				$this->db->where('account_id',$account_id);
				$this->db->where('member_id',$member_id);
				$this->db->where('club_id',$club_id);
				$this->db->update('club_live_member',array('status'=>1,'updated'=>date('Y-m-d H:i:s')));
			}
			else
			{
				$data = array(
	...
	public function getClubNotification($member_id = 0)
	{
		...
		$getUserCreatedBy = $this->db->select('created_by')->get_where('users',array('id'=>$member_id))->row_array();
		$userCreatedBy = isset($getUserCreatedBy['created_by']) ? $getUserCreatedBy['created_by'] : 0 ;
		$recordList = $this->db->where_in('to_member_id',array(0,$member_id))->get_where('club_notification',array('is_read'=>0))->result_array();
		if($recordList)
		{
			foreach($recordList as $key=>$list)
			{
				// check member already request submitted or not
				$chkRequest = $this->db->get_where('club_member_request',array('member_id'=>$member_id,'club_id'=>$list['club_id']))->num_rows();
				if($chkRequest)
				{
				...
				{
					$getClubCreatedBy = $this->db->select('created_by')->get_where('club_list',array('id'=>$list['club_id']))->row_array();
	...
	{
		return $this->db->get_where('account',array('id'=>$account_id, 'is_otp_login' => 1))->num_rows();
	}
	...
	public function get_uti_balance_charge($member_id = 0)
	{
		...
		$domain_account_id = $this->User->get_domain_account();
		// get surcharge
		$getSurcharge = $this->db->get_where('uti_balance_charge',array('account_id'=>$domain_account_id,'package_id'=>$member_package_id))->row_array();
	...
	{
		$domain_account_id = $this->User->get_domain_account();
		$totalTicket = $this->db->get_where('web_notification',array('is_read'=>0,'account_id'=>$domain_account_id))->num_rows();
		return $totalTicket;
	}
    
    
    public function call_mobikwik_bbps_electricity_bill_fetch_api($member_id = 0,$biller_id = '',$service_id = 0,$post_data = array())
	{	
...
// 		die;
			...
			if($service_id == 12){
				...
				else if($post_data['billerID'] == 138){

					$additionalParamsStr = '"bankName":"12"';
				}

				else if($post_data['billerID'] == 139){

					$additionalParamsStr = '"bankName":"13"';
				}
				else if($post_data['billerID'] == 140){

					$additionalParamsStr = '"bankName":"14"';
				}
				else if($post_data['billerID'] == 141){

					$additionalParamsStr = '"bankName":"15"';
				}
				else if($post_data['billerID'] == 142){

					$additionalParamsStr = '"bankName":"30"';
				}
				else if($post_data['billerID'] == 143){

					$additionalParamsStr = '"bankName":"16"';
				}
				else if($post_data['billerID'] == 144){

					$additionalParamsStr = '"bankName":"17"';
				}
				else if($post_data['billerID'] == 145){

					$additionalParamsStr = '"bankName":"18"';
				}
				else if($post_data['billerID'] == 146){

					$additionalParamsStr = '"bankName":"19"';
				}
				else if($post_data['billerID'] == 147){

					$additionalParamsStr = '"bankName":"20"';
				}
				else if($post_data['billerID'] == 148){

					$additionalParamsStr = '"bankName":"21"';
				}

				else if($post_data['billerID'] == 149){

					$additionalParamsStr = '"bankName":"22"';
				}
				else if($post_data['billerID'] == 150){

					$additionalParamsStr = '"bankName":"23"';
				}
				else if($post_data['billerID'] == 151){

					$additionalParamsStr = '"bankName":"24"';
				}
				else if($post_data['billerID'] == 152){

					$additionalParamsStr = '"bankName":"25"';
				}
				else if($post_data['billerID'] == 153){

					$additionalParamsStr = '"bankName":"26"';
				}
				else if($post_data['billerID'] == 154){

					$additionalParamsStr = '"bankName":"27"';
				}
				else if($post_data['billerID'] == 155){

					$additionalParamsStr = '"bankName":"31"';
				}
				else if($post_data['billerID'] == 423){

					$additionalParamsStr = '"bankName":"28"';
				}
				else if($post_data['billerID'] == 441){

					$additionalParamsStr = '"bankName":"29"';
				}
				else if($post_data['billerID'] == 540){

					$additionalParamsStr = '"bankName":"34"';
				}
				else if($post_data['billerID'] == 541){

					$additionalParamsStr = '"bankName":"32"';
				}
				else if($post_data['billerID'] == 542){

					$additionalParamsStr = '"bankName":"33"';
				}
				else if($post_data['billerID'] == 554){
...
// 			}
			...
			$get_operator = $this->db->get_where('mobikwik_bbps_service_category',array('id'=>$post_data['operator_id']))->row_array();
	...
	public function call_mobikwik_bbps_electricity_bill_pay_api($member_id = 0,$biller_id = '',$service_id = 0,$post_data = array(),$recharge_unique_id = '',$billerName = '')
	{	
	...
	public function call_mobikwik_bbps_electricity_bill_pay_api_new($member_id = 0,$biller_id = '',$service_id = 0,$post_data = array(),$recharge_unique_id = '',$billerName = '')
	{	
	...
	//	$canumber = isset($post_data['canumber']) ? $post_data['canumber'] : '';
		...
		if($service_id == 12){
			...
			if($post_data['billerID'] == 137 || $post_data['biller_id'] == 137){
				...
				else if($post_data['billerID'] == 138 || $post_data['biller_id'] == 138){

					$additionalParamsStr = '&ad1=12';
				}

				else if($post_data['billerID'] == 139 || $post_data['biller_id'] == 139){

					$additionalParamsStr = '&ad1=13';
				}
				else if($post_data['billerID'] == 140 || $post_data['biller_id'] == 140){

					$additionalParamsStr = '&ad1=14';
				}
				else if($post_data['billerID'] == 141 || $post_data['biller_id'] == 141 ){

					$additionalParamsStr = '&ad1=15';
				}
				else if($post_data['billerID'] == 142 || $post_data['biller_id'] == 142 ){

					$additionalParamsStr = '&ad1=30';
				}
				else if($post_data['billerID'] == 143 || $post_data['biller_id'] == 143 ){

					$additionalParamsStr = '&ad1=16';
				}
				else if($post_data['billerID'] == 144 || $post_data['biller_id'] == 144 ){

					$additionalParamsStr = '&ad1=17';
				}
				else if($post_data['billerID'] == 145 || $post_data['biller_id'] == 145 ){

					$additionalParamsStr = '&ad1=18';
				}
				else if($post_data['billerID'] == 146 || $post_data['biller_id'] == 146 ){

					$additionalParamsStr = '"&ad1":"19"';
				}
				else if($post_data['billerID'] == 147 || $post_data['biller_id'] == 147){

					$additionalParamsStr = '&ad1=20';
				}
				else if($post_data['billerID'] == 148 || $post_data['biller_id'] == 148 ){

					$additionalParamsStr = '&ad1=21';
				}

				else if($post_data['billerID'] == 149 || $post_data['biller_id'] == 149 ){

					$additionalParamsStr = '&ad1=22';
				}
				else if($post_data['billerID'] == 150 || $post_data['biller_id'] == 150 ){

					$additionalParamsStr = '&ad1=23';
				}
				else if($post_data['billerID'] == 151 || $post_data['biller_id'] == 151 ){

					$additionalParamsStr = '&ad1=24';
				}
				else if($post_data['billerID'] == 152 || $post_data['biller_id'] == 152 ){

					$additionalParamsStr = '&ad1=25';
				}
				else if($post_data['billerID'] == 153 || $post_data['biller_id'] == 153 ){

					$additionalParamsStr = '&ad1=26';
				}
				else if($post_data['billerID'] == 154 || $post_data['biller_id'] == 154 ){

					$additionalParamsStr = '&ad1=27';
				}
				else if($post_data['billerID'] == 155 || $post_data['biller_id'] == 155 ){

					$additionalParamsStr = '&ad1=31';
				}
				else if($post_data['billerID'] == 423 || $post_data['biller_id'] == 423 ){

					$additionalParamsStr = '&ad1=28';
				}
				else if($post_data['billerID'] == 441 || $post_data['biller_id'] == 441 ){

					$additionalParamsStr = '&ad1=29';
				}
				else if($post_data['billerID'] == 540 || $post_data['biller_id'] == 540 ){

					$additionalParamsStr = '&ad1=34';
				}
				else if($post_data['billerID'] == 541 || $post_data['biller_id'] == 147 ){

					$additionalParamsStr = '&ad1=32';
				}
				else if($post_data['billerID'] == 542 || $post_data['biller_id'] == 542 ){

					$additionalParamsStr = '&ad1=33';
				}
				else if($post_data['billerID'] == 554 || $post_data['biller_id'] == 554 ){
	...
	{
		$domain_account_id = $this->User->get_domain_account();
		$totalTicket = $this->db->get_where('member_fund_request',array('is_read'=>0,'account_id'=>$domain_account_id))->num_rows();
		return $totalTicket;
	}
	...
	{
		$domain_account_id = $this->User->get_domain_account();
		$totalTicket = $this->db->get_where('uti_balance_request',array('is_read'=>0,'account_id'=>$domain_account_id))->num_rows();
		return $totalTicket;
	}
	...
	public function get_scan_pay_surcharge($amount = 0, $loggedAccountID = 0)
	{
		...
		$domain_account_id = $this->User->get_domain_account();
		// get surcharge
		$getSurcharge = $this->db->get_where('scan_pay_commision',array('account_id'=>$domain_account_id,'package_id'=>$member_package_id,'start_range <='=>$amount,'end_range >='=>$amount))->row_array();
	...
	public function CheckTranscationPin($member_id = '', $txn_pass='')
	{
		...
		$user_data = $this->db->get_where('users',array('account_id'=>$domain_account_id,'id'=>$member_id))->row_array();
	...
	public function cosmosBankQrStatusCheckApi($account_id,$loggedAccountID,$txnid = '',$refid = '')
	{
        ...
        if(isset($decodeResult['status']) && $decodeResult['status'] == 'SUCCESS')
        {
        	if(isset($decodeResult['data'][0]['respMessge']) && $decodeResult['data'][0]['respMessge'] == 'SUCCESS')
        	{
                ...
                $queryData = array('account_id' => $account_id, 'member_id' => $loggedAccountID, 'txnid' => $txnid, 'amount'=>$PayerAmount, 'bankRRN' => $bank_rrno, 'payerVA' => $PayerVA, 'member_role_id'=>$member_role_id,'api_id'=>$api_id,'is_callback'=>2,'is_add_fund'=>$is_add_fund);
                $procQuery = $this->db->query($callStoreProc, $queryData);
                $procResponse = $procQuery->row_array();
			...
			elseif(isset($decodeResult['data'][0]['respMessge']) && $decodeResult['data'][0]['respMessge'] == 'FAILURE')
        	{
        		...
        		$this->db->update('upi_dynamic_qr',array('is_callback'=>2,'callback_datetime'=>date('Y-m-d H:i:s')));
        		$finalResponse = array(
                    	'status' => 0,
	...
	public function get_new_money_transfer_surcharge($amount = 0, $loggedAccountID = 0, $mode = '')
	{
		...
		$domain_account_id = $this->User->get_domain_account();
		// get surcharge
		$getSurcharge = $this->db->get_where('new_money_transfer_charge',array('account_id'=>$domain_account_id,'package_id'=>$member_package_id,'start_range <='=>$amount,'end_range >='=>$amount,'com_type'=>strtoupper($mode)))->row_array();
	...
	public function get_add_fund_commission($loggedAccountID = 0, $amount = 0)
	{
		...
		$commission_amount = 0;
		// get surcharge
		$getSurcharge = $this->db->get_where('add_fund_commision',array('account_id'=>$domain_account_id,'package_id'=>$member_package_id))->row_array();
		if($getSurcharge)
		{
			$is_flat = $getSurcharge['is_flat'];
			$is_surcharge = $getSurcharge['is_surcharge'];
	...
	public function get_aeps_transcation_surcharge($member_id = 0)
	{
		...
		$domain_account_id = $this->User->get_domain_account();
		// get surcharge
		$getSurcharge = $this->db->get_where('aeps_transcation_charge',array('account_id'=>$domain_account_id,'package_id'=>$member_package_id))->row_array();
