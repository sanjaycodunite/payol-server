...
class IciciAeps_model extends CI_Model {
    ...
    public function activeAEPSMember($post,$aadhar_photo,$pancard_photo,$userID)
    {
        ...
        $wallet_data = array(
            ...
            'is_otp_verify'  => 0,
            'status'         => 0,
            'account_no'                =>$post['bank_account_no'],
    ...
    public function aepsOTPAuth($post,$memberID,$otpReferenceID,$hash,$aadhar)
    {
        ...
        if(isset($otpResponseData['statuscode']) && $otpResponseData['statuscode'] == 'TXN' && $otpResponseData['status'] == 'Transaction Successful')
        {
            ...
            $this->db->update('users',array('instantpay_aeps_status'=>1,'instantpay_outlet_id'=>$outlet_id));

            return array('status'=>1,'msg'=>'success');
        }
        else
        {
            return array('status'=>0,'msg'=>$otpResponseData['status']);
        }
    }

    public function addBalance($txnID,$aadharNumber,$iin,$amount,$userID,$recordID,$serviceType = '')
    {       
        $account_id = $this->User->get_domain_account();
        $accountData = $this->User->get_account_data($account_id);
    ...
    public function addStatementCom($txnID,$aadharNumber,$iin,$amount,$recordID,$memberID)
    {       
        $account_id = $this->User->get_domain_account();
    ...
    public function saveAepsTxn($txnID,$service,$aadhar_no,$mobile,$amount,$iinno,$api_url,$api_response,$message,$status,$api_response_data = array(),$balanceAmount = '',$bankRRN = '',$transactionAmount = '',$userID = '')
    {
