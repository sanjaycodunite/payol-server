...
class Dmt_model extends CI_Model {
    ...
    public function memberActivation($post,$loggedAccountID)
    {
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Sender Register API Called.]'.PHP_EOL;
...
<senderPin>'.$post['pin_code'].'</senderPin>
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Register Sender API Post Data - '.$postJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<dmtServiceResponse>
    ...
    <senderMobileNumber>9289210048</senderMobileNumber>
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Register Sender API Response - '.$responseJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
    ...
    public function memberActivationOtpAuth($post,$loggedAccountID)
    {
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Sender Verify OTP API Called.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<additionalRegData>'.$memberData['stateresp'].'</additionalRegData>
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Sender Verify OTP API Post Data - '.$postJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<dmtServiceResponse>
    ...
    <senderMobileNumber>9289210048</senderMobileNumber>
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Sender Verify API Response - '.$responseJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
    ...
    public function memberActivationResendOtpAuth($token,$loggedAccountID)
    {
        $account_id = $this->User->get_domain_account();
        $accountData = $this->User->get_account_data($account_id);
        $loggedUser = $this->db->get_where('users',array('id'=>$loggedAccountID))->row_array();
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Sender Resend OTP API Called.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<txnType>IMPS</txnType>
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Sender Resend OTP API Post Data - '.$postJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<dmtServiceResponse>
    ...
    <senderMobileNumber>9289210048</senderMobileNumber>
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Sender Resend OTP API Response - '.$responseJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
    ...
    public function verifyIfscCode($ifsc,$loggedAccountID)
    {
        $account_id = $this->User->get_domain_account();
        $accountData = $this->User->get_account_data($account_id);
        $loggedUser = $this->db->get_where('users',array('id'=>$loggedAccountID))->row_array();
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT IFSC Verify API Called.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<ifsc>'.$ifsc.'</ifsc>
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT IFSC Verify API Post Data - '.$postJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<dmtServiceResponse>
    ...
    public function memberFetchDetail($post,$loggedAccountID)
    {
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Get Sender API Called.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Get Sender API Post Data - '.$postJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<dmtServiceResponse>
    ...
    <usedLimit>0.0</usedLimit>
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Get Sender API Response - '.$responseJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<dmtServiceResponse>
    ...
    public function addBeneficiary($post,$mobile,$loggedAccountID)
    {
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Register Ben API Called.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<ifsc>'.$post['ifsc'].'</ifsc>
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Register Ben API Post Data - '.$postJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<dmtServiceResponse>
    ...
    <senderMobileNumber>9289210048</senderMobileNumber>
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Register Ben API Response - '.$responseJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
    ...
    public function verifyBen($benId,$mobile,$loggedAccountID,$total_wallet_charge,$admin_total_wallet_charge,$admin_id)
    {
        $account_id = $this->User->get_domain_account();
        $accountData = $this->User->get_account_data($account_id);
        $loggedUser = $this->db->get_where('users',array('id'=>$loggedAccountID))->row_array();
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Verify Ben API Called.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<initChannel>AGT</initChannel>
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Verify Ben API Post Data - '.$postJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<dmtServiceResponse>
    ...
    public function deleteBen($benId,$mobile,$loggedAccountID)
    {
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Delete Ben API Called.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<recipientId>'.$benId.'</recipientId>
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Delete Ben API Post Data - '.$postJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<dmtServiceResponse>
    ...
    public function transferFund($post,$total_wallet_charge,$admin_total_wallet_charge,$surcharge_amount,$admin_surcharge_amount,$loggedAccountID)
    {
        $account_id = $this->User->get_domain_account();
        $accountData = $this->User->get_account_data($account_id);
        $loggedUser = $this->db->get_where('users',array('id'=>$loggedAccountID))->row_array();
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Transaction #'.$txnid.' Saved.]'.PHP_EOL;
        $this->User->generateLog($log_msg);

        // start convinence fees api
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Transaction Conv Fees API Called.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<txnAmount>'.($amount*100).'</txnAmount>
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Transaction Conv Fees API Post Data - '.$postJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Transaction Conv Fees API Response - '.$responseJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Transaction API Called.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<txnType>IMPS</txnType>
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Transaction API Post Data - '.$postJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
...
<dmtTransactionResponse>
    ...
    <uniqueRefId>PGMIZPKHIVERA2ETS5ZF5E2LDMI4XXIU46U</uniqueRefId>
        ...
        // save system log
        $log_msg = '['.date('d-m-Y H:i:s').' - API('.$loggedUser['user_code'].') - DMT Transaction API Response - '.$responseJson.'.]'.PHP_EOL;
        $this->User->generateDMTLog($log_msg);
