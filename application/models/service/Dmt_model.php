...
class Dmt_model extends CI_Model
{
    ...
    public function memberActivation($post, $loggedAccountID)
    {
        ...
        if (isset($responseData['response_code']) && $responseData['response_code'] == 1) {
            ...
            $this->db->update('user_paysprint_dmt_activation', ['stateresp' => $responseData['stateresp']]);
    ...
    public function memberActivationOtpAuth($post, $loggedAccountID)
    {
        ...
        $token = $post['token'];
        $memberData = $this->db
            ->select('mobile,first_name,last_name,dob,address,pin_code,stateresp')
            ->get_where('user_paysprint_dmt_activation', ['account_id' => $account_id, 'member_id' => $loggedAccountID, 'token' => $token])
            ->row_array();
        ...
        if (isset($responseData['response_code']) && $responseData['response_code'] == 1) {
            // update status
            $this->db->where('token', $token);
            $this->db->update('user_paysprint_dmt_activation', ['status' => 1, 'updated' => date('Y-m-d H:i:s'), 'updated_by' => $loggedAccountID]);
    ...
    public function memberFetchBen($mobile = '')
    {
        ...
        if (isset($responseData['responseReason']) && $responseData['responseReason'] == 'Successful') {
            if (isset($responseData['recipientList']['dmtRecipient']['bankAccountNumber'])) {
                ...
                $benList = isset($responseData['recipientList']['dmtRecipient']) ? $responseData['recipientList']['dmtRecipient'] : [];
            }
            $api_response = [
                'status' => 1,
    ...
    public function addBeneficiary($post, $mobile, $loggedAccountID)
    {
        ...
        $memberData = $this->db
            ->select('user_code,name,mobile,wallet_balance')
            ->get_where('users', ['id' => $loggedAccountID])
            ->row_array();
        $before_balance = $memberData['wallet_balance'];

        $addressData = $this->db
            ->select('dob,address,pin_code')
            ->get_where('user_paysprint_dmt_activation', ['account_id' => $account_id, 'member_id' => $loggedAccountID, 'mobile' => $mobile, 'status' => 1])
            ->row_array();

        $getBankName = $this->db
            ->select('title')
            ->get_where('paysprint_dmt_bank_list', ['id' => $post['bankID']])
            ->row_array();
        ...
        $log_msg =
            ...
            ' - DT(' .
            $loggedUser['user_code'] .
            ') - DMT Beneficiary Auth Beneficiary Saved into
        System.]' .
            PHP_EOL;
        $this->User->generateDMTLog($log_msg);
        ...
        $log_msg =
            ...
            ' - DT(' .
            $loggedUser['user_code'] .
            ') - DMT Beneficiary Auth Paysprint Post Data -
        ' .
        ...
        $log_msg =
            ...
            ' - DT(' .
            $loggedUser['user_code'] .
            ') - DMT Beneficiary Auth Paysprint Register
        ...
        if (isset($responseData['response_code']) && $responseData['response_code'] == 1) {
            ...
            $this->db->update('user_dmt_beneficiary', ['status' => 1, 'beneId' => $responseData['data']['bene_id']]);
    ...
    public function deleteBen($benId, $mobile, $loggedAccountID)
    {
        ...
        $log_msg =
            ...
            ' - DT(' .
            $loggedUser['user_code'] .
            ') - DMT Delete Ben Auth Paysprint Post Data -
        ' .
            json_encode($post_data) .
            '.]' .
            PHP_EOL;
        ...
        $log_msg =
            ...
            ' - DT(' .
            $loggedUser['user_code'] .
            ') - DMT Delete Ben Auth Paysprint Delete Ben API
        Response - ' .
            $output .
            '.]' .
            PHP_EOL;
    ...
    public function transferFund($post, $surcharge_amount, $loggedAccountID, $active_api_id)
    {
        ...
        $beneficiaryData = $this->db->get_where('user_dmt_beneficiary', ['beneId' => $benId])->row_array();
        $register_mobile = $beneficiaryData['register_mobile'];

        // get member activation data
        $memberActivationData = $this->db->get_where('user_paysprint_dmt_activation', ['account_id' => $account_id, 'member_id' => $loggedAccountID, 'status' => 1, 'mobile' => $register_mobile])->row_array();
        ...
        $log_msg =
            ...
            ' - DT(' .
            $loggedUser['user_code'] .
            ') - DMT Transfer Auth Member Wallet Deducation,
            Updated Balance - ' .
        ...
        if ($active_api_id == 2) {
            ...
            $log_msg =
                ...
                ' - DT(' .
                $loggedUser['user_code'] .
                ') - DMT Transfer Auth Paysprint DMT Txn API
                Response - ' .
                $output .
                '.]' .
                PHP_EOL;
            ...
            $log_msg =
                ...
                ' - DT(' .
                $loggedUser['user_code'] .
                ') - DMT Transfer Auth Payu DMT Txn API Response -
                ' .
        ...
        if (isset($responseData['response_code']) && $responseData['response_code'] == 1) {
            if (isset($responseData['txn_status']) && $responseData['txn_status'] == 1) {
                ...
                $this->db->update('user_dmt_transfer', ['status' => 3, 'op_txn_id' => $responseData['ackno'], 'rrn' => $responseData['utr']]);
                ...
                // update status
                $this->db->where('transaction_id', $transaction_id);
                $this->db->update('user_dmt_transfer', ['op_txn_id' => $responseData['ackno'], 'rrn' => $responseData['utr']]);
                ...
                // update status
                $this->db->where('transaction_id', $transaction_id);
                $this->db->update('user_dmt_transfer', ['status' => 4, 'op_txn_id' => $responseData['ackno'], 'rrn' => $responseData['utr']]);
                ...
                $log_msg =
                    ...
                    #' .
                    $transaction_id .
                    ' Member Wallet Reunded, Updated Balance - ' .
                    $after_wallet_balance .
                    '.]' .
                    PHP_EOL;
                $this->User->generateDMTLog($log_msg);
            ...
            if ($responseData['status'] == 'FAILED') {
                // update status
                $this->db->where('transaction_id', $transaction_id);
                $this->db->update('user_dmt_transfer', ['status' => 4]);
