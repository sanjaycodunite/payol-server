...
$("#aadhar_verify").click(function(){
		var siteUrl = $("#siteUrl").val();
		var aadhar_no = $("#signatory_aadhar").val();
		$(".large-loader").css('display','block');
		if(aadhar_no != 0)
		{
			$.ajax({                
				url:siteUrl+'portal/kyc/aadharVerify/'+aadhar_no,                        
				success:function(r){
					...
					if(data["status"] == 1){
			            ...
			            $(".large-loader").css('display','none');
	             		...
	             		// 		 button: "OK!",
				            ...
				            swal({
						  	title: 'Otp Sent Successfully',
						  	//text: 'Are you sure want tot delete',
						  	icon: 'success',						  	
						  	confirmButtonText: 'OK',
							}).then((result) => {
							  	
							  		//window.location.href= siteUrl+'/user/buyer/deletePurchaseMargin/'+recordID;
		...
		{
			$(".recharge-comm-loader").html('<font color="red">Please Enter Aadhar No.</font>');
		}
		
		
	});

	$("#aadhar_otp_verify").click(function(){
		var siteUrl = $("#siteUrl").val();
		var aadhar_request_id = $("#aadhar_request_id").val();
		...
		{
			$.ajax({                
				url:siteUrl+'portal/kyc/aadharVerifyOtp/'+aadhar_request_id+'/'+aadhar_otp,                        
				success:function(r){
					...
					if(data["status"] == 1){
			            ...
			            $("#bankModal").modal('hide');
			              ...
			              icon: "success",
							 ...
							 //$("#bankModal").modal('none');
							 $("#aadhar_verify").css('display','none');
							  $('#signatory_aadhar').prop("readonly", true);
		...
		{
			$(".recharge-comm-loader").html('<font color="red">Please Enter OTP.</font>');
		}
	...
	$("#pan_verify").click(function(){
		...
		{
			$.ajax({                
				url:siteUrl+'portal/kyc/panVerify/'+pan_no+'/'+signatory_name,                        
				success:function(r){
					...
					if(data["status"] == 1){
			            ...
			            $(".large-loader").css('display','none');
				            swal({
				              title: 'Pan Verified Successfully  Verified Name - '+data['msg'],           
				              icon: "success",
	             				 button: "OK!",
				            });

				           $("#pan_verify").css('display','none');
