        ...
        function CaptureDepositeMorpho() {
                //debugger;
                var url = "http://127.0.0.1:11100/capture";
                if($('#serviceType4').is(':checked')) { 
                    var PIDOPTS = '<PidOptions ver=\"1.0\">' + '<Opts fCount=\"1\" fType=\"2\" iCount=\"\" iType=\"\" pCount=\"\" pType=\"\" format=\"0\" pidVer=\"2.0\" timeout=\"10000\"  env=\"P\" otp=\"\" wadh=\"\" posh=\"\"/>' + '</PidOptions>';
                    ...
                    if (xhr.readyState == 4) {
                        ...
                        if (status == 200) {
                            ...
                            var Service = 'Cash';
                                ...
                                var Amount = $("#amount").val();
                                if (Amount == "0") {
                                    CallDepositeApi(Service, xhr.responseText);
                                }
                                else {
                                    alert("Wrong Input Amount");
                                    ClearCtrl()
...
function CallApi(ServiceType, BiometricData) {
    ...
    jQuery.ajax({
        type: "POST",
        url: siteUrl+"retailer/newaeps/apiAuth",
...
function CallVerifyApiLoginNewAuthTransaction(ServiceType, BiometricData) {
    ...
    jQuery.ajax({
        type: "POST",
        url: siteUrl+"retailer/newaeps/merchantAuthenticity",
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify(jsonData),
        dataType: "json",
        success: function (Result) {
            if(Result.status == true)
            {
                ...
                setTimeout(function() {
                  window.location.replace(siteUrl+'retailer/newaeps');
                });
            }
            else
            {
...
function CallKYCVerifyApi(BiometricData) {
    var siteUrl = $("#siteUrl").val();
    //var encodeFPTxnId = $("#encodeFPTxnId").val();
    var jsonData = {BiometricData:BiometricData, bankType:$('.bankType').val()};
    jQuery.ajax({
        type: "POST",
...
function CallKYCVerifyApiLogin(BiometricData) {
    ...
    jQuery.ajax({
        type: "POST",
        url: siteUrl+"retailer/newaeps/kycBioAuthNew",
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify(jsonData),
        dataType: "json",
        success: function (Result) {
...
function CallDepositeApi(ServiceType, BiometricData) {
    var siteUrl = $("#siteUrl").val();
    var Device = GetDevice();
    var Service = 'Cash';
    var AAdharNumber = $("#aadhar_no").val();
    ...
    jQuery.ajax({
        type: "POST",
        url: siteUrl+"retailer/newaeps/depositeApiAuth",
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify(jsonData),
        dataType: "json",
        success: function (Result) {
